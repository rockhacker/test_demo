...
/**合约风险率控制器
 * Class UStandardHazardController
 *
 * @package App\Http\Controllers\Admin
 */
class UStandardHazardController extends Controller
{
    public function index()
    {
        $currencies = Currency::where('is_quote', 1)->get();
        return view('admin.uStandard.hazard.index')->with('currencies', $currencies);
    }

    public function handle()
    {
        $id = request('id', 0);
        $lever_trade = LeverUStandard::find($id);
        return view('admin.uStandard.hazard.handle')
    ...
    public function super_handle()
    {
        $id = request('id', 0);
        $lever_trade = LeverUStandard::find($id);
        return view('admin.uStandard.hazard.super_handle')
            ->with('lever_trade', $lever_trade);
    }
    ...
    public function postHandle(Request $request)
    {
        try {
            ...
            for ($i=0; $i < $forMaxNum; $i++){
                ...
                $trade = LeverUStandard::where('status', LeverUStandard::STATUS_TRANSACTION)->find($trade_id);
                if (!$trade) {
                    break;
                }
...
//            return $this->error($e->getMessage() . $e->getFile() . $e->getLine());
    ...
    public function list(Request $request)
    {
        ...
        $uid = request('uid', '');
        $user_hazard = LeverUStandard::where('status', LeverUStandard::STATUS_TRANSACTION)
    ...
    {
        $currencies = Currency::where('is_quote', 1)->get();
        return view('admin.uStandard.hazard.total')
    ...
    public function totalLists(Request $request)
    {
        ...
        GROUP BY user_id
         */
        $user_hazard = LeverUStandard::where('status', LeverTransaction::STATUS_TRANSACTION)
