...
class CurrencyUpdateRequest extends BaseRequest
{
    /**
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $exists = Currency::where('code', $code)->where('id', '<>', $id)->exists();
            if ($exists) {
                return $validator->errors()->add('code', '此币种已存在');
            }

            $accounts_display = $this->input('accounts_display', []);
            if (!$accounts_display) {
                $this->request->set('accounts_display', []);
            }
