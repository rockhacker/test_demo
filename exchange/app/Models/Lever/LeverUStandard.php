...
use App\Jobs\LeverUStandardPushTrade;
...
class LeverUStandard extends Model
{
    protected $table = 'Lever_u_standard';
...
//        $multiple_number = bc_mul($number, $multiple);
         ...
         *
         */
        $diff = $type == self::WAY_BUY ? bc(bc($update_price, '-', $price),'/',$price) : bc(bc($price, '-', $update_price),'/',$price);
     ...
     * @return void
     */
    public static function newPrice($currency_match, $timestamp = null)
    {
        ...
        $price = CurrencyQuotation::where('currency_match_id', $currency_match->id)->value('close');
        ...
        LeverUStandard::where("currency_match_id", $currency_match->id)
...
//        LeverUStandard::tradeHandle($currency_match,$price, $timestamp);
//        LeverHandle::dispatch($params)->onQueue('lever:handle');
     ...
     * @return void
     */
    public static function tradeHandle($currency_match)
    {
        $lever_users = LeverUStandard::where("currency_match_id", $currency_match->id)
            ->where("status", self::STATUS_TRANSACTION)
        ...
        $sub_user_ids = SocketPusher::getUidListByGroup(SocketPusher::LEVER_U_STANDARD_TRADE);
...
//                HandleUserLever::dispatch($user_id, $currency_match)->onQueue('handle:user:lever:' . $queue_num);
            ...
            if (in_array($user_id, $sub_user_ids)) {
                LeverUStandardPushTrade::dispatch($lever->user_id, $currency_match)->onQueue('leverUStandard:push:trade');
            }
    ...
    protected static function handleUserLever($user_id, $currency_match)
    {
        ...
        try {
            ...
            $quote_wallet->changeBalance(AccountLog::LEVER_TRANSACTION_FROZEN, $change, $data);
    ...
    public static function getUserProfit($user_id, $quote_currency_id = 0, $base_currency_id = 0)
    {
        ...
        try {
            //优先让数据库计算盈亏和保证金
            $user_profit = LeverUStandard::where('status', self::STATUS_TRANSACTION)
                ...
                ->select('user_id')
                ->selectRaw('SUM((CASE `type` WHEN 1 THEN `update_price` - `price` WHEN 2 THEN `price` - `update_price` END) / `price` * `multiple` * `caution_money`) AS `profits_total`')
                ->selectRaw('SUM(`caution_money`) AS `caution_money_total`')
                ->selectRaw('SUM(`origin_caution_money`) AS `origin_caution_money_total`')
                ->groupBy('user_id')
                ->first();
            if (!$user_profit) {
                $levers = LeverUStandard::where(function ($query) use ($base_currency_id, $quote_currency_id) {
                    $base_currency_id > 0 && $query->where("base_currency_id", $base_currency_id);
                    $quote_currency_id > 0 && $query->where("quote_currency_id", $quote_currency_id);
...
//            echo $e->getMessage();
    ...
    {
        DB::beginTransaction();
        $trades = LeverUStandard::where(function ($query) use ($quote_currency_id, $base_currency_id) {
            $quote_currency_id > 0 && $query->where("quote_currency_id", $quote_currency_id);
            $base_currency_id > 0 && $query->where("base_currency_id", $base_currency_id);
     ...
     * 平仓
     *
     * @param \App\Models\Lever\LeverUStandard $lever_transaction
...
//            }
    ...
    public static function getAccountHazardRate($account)
    {
        ...
        $total_money = bc($balance, '+', $caution_money_total);
        if (bc($caution_money_total, '<>', '0')) {
            $hazard_rate = bc(bc(bc($total_money, '+', $profits_total), '/', $caution_money_total), '*', 100, 2);
        }
        return $hazard_rate;
    }
     ...
     * @return float
     */
    public static function getAccountAllBalance($account)
    {
        $allBalance = 0;
        $total_money = 0;
        if (!$account) {
        ...
        $total_money = bc($balance, '+', $caution_money_total);
        if (bc($caution_money_total, '<>', '0')) {
            $allBalance = bc($total_money, '+', $profits_total, 2);
        }
        return $allBalance;
    }
...
//            }
    ...
    public static function checkNeedStopPriceTrade($currency_match, $user_id = 0)
    {
        ...
        $need_check_lever = LeverUStandard::where('status', self::STATUS_TRANSACTION)
...
//        }
    ...
    public static function entrustActivate()
    {
        ...
        */
        $trades = LeverUStandard::where('status', LeverUStandard::STATUS_ENTRUST)
            ->where(function ($query) {
                $query->orWhere(function ($query) {
                    $query->where('type', LeverUStandard::WAY_BUY)
                        ->whereRaw('`update_price` <= `origin_price`');
                })->orWhere(function ($query) {
                    $query->orWhere('type', LeverUStandard::WAY_SELL)
                        ->whereRaw('`update_price` >= `origin_price`');
                });
            });
        ...
        $result = $trades->update([
            ...
            'status' => LeverUStandard::STATUS_TRANSACTION,
        ]);
    ...
    public static function pushDeletedTrade(Collection $lever_trades)
    {
        ...
        $grouped->each(function (Collection $item, $key) {
            ...
            //推送深度
            SocketPusher::leverUStandardClosed($item->pluck('id')->all(), $key);
        });
    }
