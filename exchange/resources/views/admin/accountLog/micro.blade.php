...
<script>
    layui.config({
        ...
        table.render({
            elem: '#list'
            , url: '/admin/account_log/micro_list?type={{request('type',0)}}' //模拟接口
            , cols: [[
                ...
                {fixed: 'right', field: '', title: '操作', width: 70, templet: '#operateBar'}
            ]]
            , page: true
            , limit: 20
            , height: 'full-220'
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (layEvent === 'del') {
                layer.confirm('是否删除该记录', {
                    ...
                    $.post('/admin/account_log/micro_del_log?id=' + data.id, {}, function (res) {
                        layer.msg(res.msg);
                        if (res.code) {
