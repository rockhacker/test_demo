        ...
        <div class="layui-card-body">
            <form class="layui-form" lay-filter="layuiadmin-form-useradmin" id="layuiadmin-form-useradmin">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="name" lay-verify="name" placeholder="请输入字段名称"
                               class="layui-input" value="{{$info->field_name}}">
                    </div>
                </div>
                <div class="layui-form-item">
                ...
                <div class="layui-form-item" id="if_may_has_children" hidden>
                    ...
                    <div class="layui-input-inline">
                        <input type="checkbox" id="has_children" name="has_children" lay-skin="switch"
                               lay-filter="if_has_children" lay-text="是|否" {{isset($info->field_object['has_children'])? 'checked':''}}>
                    </div>
                ...
                <script id="div_of_attribute_tpl" type="text/html">
                    <div class="layui-form-item div_of_children" id="div_of_attribute">
                        ...
                        <div class="layui-input-block">
                            ...
                            {{--@{{# console.log(d.added_arr_by_id[item.option_id])}}--}}
                            <div class="layui-form-item">
                                <div class="layui-input-inline" style="width: 150px;">
                                    <select name="property[@{{index}}][option_id]" lay-search lay-filter="select_property" id="select_property">
                                        <option value="@{{item.option_id}}" selected>@{{d.added_arr_by_id[item.option_id].property_name}}</option>;
                                        @{{#  layui.each(d.options, function(o_index, o_item){ }}
                                        <option value="@{{o_item.id}}">@{{o_item.property_name}}</option>;
                                        @{{#  }); }}
                                    </select>
                                </div>
                                <div class="layui-form-mid">：</div>
                                <div class="layui-input-inline" style="width: 300px;">
                                    <input type="text" name="property[@{{index}}][value]" placeholder="值" autocomplete="off" class="layui-input" value="@{{item.value}}">
                                </div>
                        ...
                        <div class="layui-input-block">
                            ...
                            <div class="layui-form-item">
                                <div class="layui-input-inline" style="width: 100px;">
                                    <input type="text" name="children_value[@{{index}}][real]" placeholder="实际值" autocomplete="off" class="layui-input" value="@{{item.real}}">
                                </div>
                                <div class="layui-form-mid">-</div>
                                <div class="layui-input-inline" style="width: 100px;">
                                    <input type="text" name="children_value[@{{index}}][show]" placeholder="显示值" autocomplete="off" class="layui-input" value="@{{item.show}}">
                                </div>
                                <button type="button" class="layui-btn layui-btn-danger" lay-submit lay-filter="del_element">删除</button>
                            </div>
                            @{{#  }); }}
...
<script>
    layui.config({
        ...
        var added_property = @json(isset($info->field_object['property'])?$info->field_object['property']:[]);//模板赋值，选择过的属性
        var children_value = @json(isset($info->field_object['children_value'])?$info->field_object['children_value']:[]);//模板赋值，已保存的子元素
        var element = '';//字段元素

        var max_children_id = 0;//当前最大子元素数量
        var max_attribute_id = 0;//当前最大属性数量s
        for (i in added_property) {
            max_attribute_id = i;
        }
        ...
        console.log(max_children_id)
        console.log(max_attribute_id)
        check_if_may_has_children();//对默认下拉元素的检查
        reload_div_of_attribute();//重载【添加字段属性框】

        check_if_has_children_div();//检查是否需要打开子元素添加框
         ...
         * 初始化已添加和添加过的属性
         */
        function init_addable_and_added(){

            var added_property_id = [];
            for(x in added_property){
                //console.log(added_property[x]);
                added_property_id.push(added_property[x]['option_id'])
            }

            //可添加名单数组
            addable_arr = $.grep(addable_arr, function(value) {
                //添加到已添加名单数组
                if(added_property_id.indexOf(String(value.id)) > -1){
                    added_arr.push(value);
                    return false;
                }else{
                    return true;
        ...
        function reload_div_of_attribute(){
            $.ajaxSettings.async = false;//设ajax同步请求
            $.get('/admin/field_property/list', {element:element,limit:100}, function (res) {
                if(res.code == 0){
                    addable_arr = res.data;
                    init_addable_and_added();
                }else{
                    layer.msg('获取字段属性列表失败');
                }
            });
            $.ajaxSettings.async = true;//设ajax异步请求
            var getTpl = div_of_attribute_tpl.innerHTML
                ,view = document.getElementById('view');

            var added_arr_by_id = [];
            added_arr.forEach((item,index,array)=>{
                //执行代码
                added_arr_by_id[item.id] = item
            });
            laytpl(getTpl).render({'list':added_property,'options':addable_arr,'added_arr_by_id':added_arr_by_id}, function(html){
                view.innerHTML = html;
            });
            form.render('select', 'layuiadmin-form-useradmin');//动态生成的表单元素需要重新渲染
        }
        ...
        function reload_div_of_children(show){
            ...
            laytpl(getTpl).render(children_value, function(html){
               //console.log(children_value)
                view.innerHTML = html;
            });
        }
        //检查是否需要打开子元素框
        function check_if_has_children_div(){
            var is_checked = check_has_children_input_value();//检查【是否有子元素开关】的状态
            if(is_checked ==false){
                //关的话，隐藏【添加子元素框】
                reload_div_of_children(false);
        ...
        form.on('submit(submit)', function (data) {
            $.post('/admin/field/update', data.field, function (res) {
                layer.msg(res.msg);
                if (res.code) {
                    setTimeout(function () {
                        location.reload()
