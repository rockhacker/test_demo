        ...
        <div class="layui-card-body">
            <form class="layui-form" lay-filter="layuiadmin-form-useradmin" id="layuiadmin-form-useradmin">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="code" lay-verify="code" placeholder="请输入币种code"
                               class="layui-input" value="{{$info->code}}">
                    </div>
                </div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="radio" name="trade_status" value="0" title="关闭" {{$info->trade_status == 0 ?'checked': ''}}>
                        <input type="radio" name="trade_status" value="1" title="开启" {{$info->trade_status == 1 ?'checked': ''}}>
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="forex_point_rate" lay-verify="forex_point_rate" placeholder="请输入点差率"
                               class="layui-input" value="{{$info->forex_point_rate}}">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="forex_fee_rate" lay-verify="forex_fee_rate" placeholder="请输入手续费率"
                               class="layui-input" value="{{$info->forex_fee_rate}}">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="forex_min_cont"  placeholder="请输入最小张数"
                               class="layui-input" value="{{$info->forex_min_cont}}">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="forex_max_cont"  placeholder="请输入最大张数"
                               class="layui-input" value="{{$info->forex_max_cont}}">
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="forex_cont_num"  placeholder="请输入每张数量"
                               class="layui-input" value="{{$info->forex_cont_num}}">
                    </div>
                </div>
                <div class="layui-form-item">
...
<script>
    layui.config({
        ...
        @foreach($info->market_start_day as $key => $start_day)
            laydate.render({
                elem: '#market_day_'+ {{$key}} //指定元素
                ,range:true
                ,type:"time"
                ,format: 'HH:mm:ss' //可任意组合
                ,value: "{{ $start_day['start'] . ' - ' . $start_day['end'] }}" //参数即为：2018-08-20 20:08:08 的时间戳
            });
        @endforeach

        form.on('submit', function (data) {
            $.post('/admin/forex/trade_list/update', data.field, function (res) {
                layer.msg(res.msg);
                if (res.code) {
                    setTimeout(function () {
