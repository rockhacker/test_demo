...
<div class="layui-fluid">
    <div class="layui-card">
        <div class="layui-card-header">编辑表单组</div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="name"  placeholder="请输入组名称"
                               class="layui-input" value="{{$info->group_name}}">
                    </div>
                </div>
                <div id="view"></div>
                        ...
                        <div class="layui-input-block">
                            ...
                            <div class="layui-form-item">
                                <div class="layui-input-inline" style="width: 150px;">
                                    <select name="fields[@{{index}}]" lay-search lay-filter="select_field" id="select_field">
                                        <option value="@{{item}}" selected>@{{d.added_arr_by_id[item].field_name}}</option>;
                                        @{{#  layui.each(d.options, function(o_index, o_item){ }}
                                        <option value="@{{o_item.id}}">@{{o_item.field_name}}</option>;
                                        @{{#  }); }}
                                    </select>
                                </div>
...
<script>
    layui.config({
        ...
        var addable_arr = [];
        var added_arr = [];
        var added_fields = @json(isset($info->group_field)?$info->group_field:[]);//模板赋值，添加过的字段
        ...
        function reload_div_of_field(){
            ...
            $.ajaxSettings.async = true;//设ajax同步请求
            ...
            laytpl(getTpl).render({'list':added_fields,'options':addable_arr,'added_arr_by_id':added_arr_by_id}, function(html){
                view.innerHTML = html;
            });
        ...
        function init_addable_and_added(){
            ...
            for(x in added_fields){
                ...
                added_fields_id.push(added_fields[x])
            }

            //可添加名单数组
            addable_arr = $.grep(addable_arr, function(value) {
                //添加到已添加名单数组
                if(added_fields_id.indexOf(String(value.id)) > -1){
                    added_arr.push(value);
                    return false;
                }else{
                    return true;
        ...
        form.on('submit(submit)', function (data) {
            $.post('/admin/model_group/update', data.field, function (res) {
                setTimeout(function () {
                    location.reload()
                }, 500)
            });
