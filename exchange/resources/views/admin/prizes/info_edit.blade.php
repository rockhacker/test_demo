            ...
            <form class="layui-form" lay-filter="layuiadmin-form-useradmin" id="layuiadmin-form-useradmin">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <select name="prizes_id" id="">
                            @foreach($prizes as $p)
                                <option value="{{$p->id}}" {{ $p->id == $info->prizes_id ? 'selected' : '' }}>{{$p->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="layui-input-block">
                        <select name="lang_id" id="">
                            @foreach($langs as $lang)
                                <option value="{{$lang->id}}" {{ $lang->id == $info->lang_id ? 'selected' : '' }}>{{$lang->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        ...
                        <input type="hidden" id="upload-input" name="img" value="{{ $info->img }}">
                        <div>
                            <img width="200px" id="upload-img"  src="{{ $info->img }}">
                        </div>
                    </div>
                </div>
...
<script>
    layui.config({
        ...
        form.on('submit', function (data) {
            $.post('/admin/prizes/info_edit', data.field, function (res) {
                layer.msg(res.msg);
                if (res.code) {
                    setTimeout(function () {
                        location.reload()
