...
<script>
    layui.config({
        ...
        form.on('submit(search)', function (data) {
            ...
            $.post('/admin/setting_type/save', field, function (res) {
                layer.msg(res.msg);
                if (res.code) {
                    setTimeout(function () {
                        location.reload()
        ...
        table.render({
            elem: '#list'
            , url: '/admin/setting_type/list' //模拟接口
            , cols: [[
                {field: 'id', width: 100, title: 'ID'},
                {field: 'name', title: '分类'},
                {field: 'settings_count', title: '设置数量'},
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (obj.event === 'del') {
                ...
                layer.prompt({
                    ...
                    $.post('/admin/setting_type/update', {
                        id: data.id,
                        name: value
                    }, function (res) {
                        layer.close(index);
