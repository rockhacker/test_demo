...
func (f *ForexTradeLists) CheckMarketStatus() (marketStatus int) {
	...
	//为了方便设置，这块统一使用UTC+8时区（东八区）北京时间进行设置

	//统一计算时区
	//zone, _ := t.Zone()
	carbonZC := carbon.SetTimezone(carbon.PRC)
...
type ForexTransactions struct {
	...
	CompleteTimeHandler string          `gorm:"-"`
...
func (f *ForexTransactions) AfterFind(tx *gorm.DB) (err error) {
	...
	f.CompleteTimeHandler = time.Unix(f.CompleteTime, 0).Format("2006-01-02 15:04:05")
	return
}
