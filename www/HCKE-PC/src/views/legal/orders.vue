<template>
    <div>
        <div class="w100 mauto baselight2 mt20 bgwhite dark-bg-1C2532 ptb10 flex alcenter between ft16 plr20 radius2">
        ...
        <div class="plr20 ptb20">
            ...
            <div class="flex alcenter baselight2 ft14 ">
                <div class="mr30 pointer bd2px ptb10" v-for="(item,i) in statusList"  :class="[status==item.status?(selectType=='BUY'?'bdbgreen':'bdbred'):'',status==item.status?(selectType=='BUY'?'green':'red'):'']" @click="changeStatu(item.status)" :key="i">{{item.name}}</div>
            </div>
            <div class="mt20">
