  ...
  <div class="p-10">
    ...
    <div class="mt20">
      ...
      <ul class="flex wraps times-list tc mt10">
        <li class="mb10 bde9 w30 ptb10 mr10 dark-bg-1C2532 posRelt pointer gray9 ft14" v-for="(item,i) in wallet_list" :key="i" @click="changeModel(i)">
          <span>{{item.code}}</span>
    ...
    <div class="name">{{ parseFloat(item.number) }}</div>
      ...
      <div class="bg-main-liner ptb20 flex hidden-xs radius6">
        <div class="flex flex1 alcenter jscenter">
          ...
          <span class="black bold ft18">{{$t('second.profitRate')}}</span>
        </div>
       <div class="flex1 alcenter flex  jscenter gold ft36">
          <span class="bold text-clip-yellow" v-if="second_List.length&&second_List[timeIndex].profit_ratio">{{parseFloat(Number(second_List[timeIndex].profit_ratio)*100)}}%</span>
          <span></span>
        </div>
      </div>
      ...
      <div class="mt20 nmb_mt20 flex" v-else>
        <div class="flex1 bg-main-liner tc ptb10 ft16 white radius40 flex alcenter jscenter pointer mb_btn mr20" @click="submits(1)" >
