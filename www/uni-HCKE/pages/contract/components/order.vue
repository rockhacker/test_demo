<template>
  <view>
    <view @click="showDetail" class="p-20 border-0 border-solid border-b-2 border-gray-600">
      <view class="flex">
        <view>{{
          info.status == 1 ? $t("second.inTransaction") : $t("second.closed")
        }}</view>
        <image
    ...
    <uni-popup :show="showOrderDetail" :btnShow="false" :msg="info.symbol_name" @hidePopup="showOrderDetail=false">
        ...
        <view class="text-52 pt-30" v-else>{{remain_milli_seconds}} {{ $t('second.miao') }}</view>
        ...
        <view  class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('second.loss')}}</view>
        ...
        <view class="flex justify-between w-full px-30 mb-20">
          ...
          <view class="text-base" :class="[ploss>=0?'text-success':'text-danger']">{{ ploss }}</view>
				</view>
        </template>
