		...
		<view class="bg-white dark:bg-floor">
			...
			<view class="px-30 py-20">
				...
				<scroll-view scroll-x class="w-full whitespace-nowrap" :show-scrollbar="false">
					<view class="px-20 py-10 mr-40 flex-col justify-center items-center text-lg inline-flex border-2 border-solid border-gray-200 dark:border-gray-700" hover-class="opacity-50"
						:class="{'bg-primary border-primary dark:border-primary text-black':index==i}" v-for="(item,i) in coinList" :key="i" 
			...
			<view class="px-30 py-20">
					...
					<view v-if="coinList[index].currency_protocols.length > 1">
						...
						<view class="flex mb-30">
							<view v-for="(item, index) in coinList[index].currency_protocols" :key="index" 
								class="text-lg mr-20 py-10 px-20  border-2 border-solid border-gray-200 dark:border-gray-700" :class="{'text-primary border-primary dark:border-primary': name == item.chain_protocol.code}"
				...
				<view class="mt-100 mb-60 bg-primary rounded-10 p-20 text-black text-lg text-center" @tap="bindAddress" hover-class="opacity-50">{{ $t('bind.bind') }}</view>
			</view>
			<!-- 验证码 -->
			<!-- <view class="flex items-center">
