<template>
	<view :class="theme">
		<view class="text-black dark:text-gray-400 bg-gray-100 dark:bg-gray-900">
			...
			<!--轮播-->
			<swiper active-class="activeClass" class="h-300 w-full" :indicator-dots="true" :autoplay="true"
				:interval="3000" :circular="true" indicator-color="#aaa" indicator-active-color="#eee">
		  ...
		  <view class="flex items-center flex-1 justify-center flex-col text-center" @click="toPath('/pages/legal/legal')">
			...
			<view class="w-full my-20 h-120 flex justify-between bg-white dark:bg-floor items-icon">
				<view v-if="false" class="flex-1 flex justify-center items-center border-0 border-solid border-gray-100 dark:border-gray-900 border-r-10"
					@click="linkWallet">
					<view class="leading-120 font-bold text-lg">
						{{$t('new.ljqb')}}
					</view>
				...
				<view class="flex-1 flex items-center justify-center border-0 border-solid border-gray-100 dark:border-gray-900 border-l-10"
			...
			<swiper display-multiple-items="3"
				class="w-full h-200 bg-white dark:bg-floor border-0 border-b-2 border-solid border-gray-100 dark:border-gray-800"
				v-if="quoList.length > 2">
				<swiper-item class="flex items-center justify-around" v-for="(item, i) in quoList" :key="i">
					<view @click="toUrl(item)" class="flex flex-col items-center">
			...
			<swiper class="w-full h-200 bg-white dark:bg-floor border-0 border-b-2 border-solid border-gray-800" v-else>
				<swiper-item class="flex items-center justify-around" v-for="(item, i) in quoList" :key="i">
					...
					<view @click="toUrl(item)" class="flex column items-center">
						<text class="text-sm">{{ item.base_currency_code }}/{{ item.quote_currency_code }}</text>
						<text class="ft16 bold" v-if="item.currency_quotation"
							:class="{
			...
			<!--行情-->
			<view class="pb-100 w-full bg-white dark:bg-floor">
				...
				<view class="flex px-40 pt-20">
					<view class="trading-item text-center mr-40 " v-for="(item, index) in allList" :key="index"
						@click="setItem(item)">
						<view class="pb-10 text-lg" :class="itemCode == item.code ? 'text-primary' : ''">{{ item.code }}
						</view>
				...
				<view @click="toUrl(item)" class="flex items-center text-sm text-gray-400 tableitem px-40 py-30"
					...
					<view class="flex items-center" style="flex: 1.5">
						<image class="w-40 h-40 mr-16 rounded-half bg-white" :src="host + item.base_currency.logo">
						</image>
					...
					<view v-if="item.base_currency_code=='TEST' && !isOnline(item.market_time)" class="w-200">
						{{$t('trade.soon')}}</view>
					<template v-else>
