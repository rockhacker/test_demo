<template>
	<view :class="theme">
		<view class="p-30 bg-white dark:bg-floor">
		...
		<view class="mt-20 py-20 bg-white dark:bg-floor">
			...
			<view class="px-30 py-20 mt-40">
				<view class="bg-primary text-black rounded-10 p-30 text-lg text-center" @tap="transfer" hover-class="opacity-50">{{ $t('assets.transfer') }}</view>
			</view>
		</view>
		
		<view class="py-30 mt-20 bg-white dark:bg-floor" style="min-height:50vh">
	...
	methods: {
        ...
        getAssets() {
				this.$utils.initDataToken({url:'account/list'},res=>{
					...
					this.wallet_data = res[0];
                    this.accountTypeId =this.wallet_data.id;
                    this.ids = this.wallet_data.accounts[0].id;
                    this.getDetail()
				})
