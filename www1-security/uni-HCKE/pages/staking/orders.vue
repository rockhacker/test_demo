<template>
	<view :class="theme">
		<view class="text-black dark:text-white dark:bg-gray-600 dark:bg-opacity-30 w-full overflow-hidden">

			<view v-if="tableData.length">
				<view v-for="(item, index) in tableData" :key="index"
					class="p-40 m-40 rounded-10 text-xl bg-white dark:bg-floor">
					...
					<view class="mt-20 rounded-3xl p-20 text-sm text-center"
						:class="!item.status?'bg-primary text-black':'bg-danger text-white'"
