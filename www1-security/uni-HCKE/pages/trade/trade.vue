<template>
	<view class="pb-100" :class="theme">
		<!-- 头部 -->
		<view class="h-88 w-full header bgPart fixed flex alcenter jscenter plr15 zdx100">
			<view class="flex alcenter jscenter" @click="showLeft=true">
				<!-- <image src="/static/image/list.png" class="h20 wt20"></image> -->
				<text class="dark:text-white text-black font-bold text-xl">{{currency_name}}/{{legal_name}}</text>
		...
		<template v-else>
			<view class="flex pt-110 pb15 bgPart">
				<!-- 左侧 -->
				<view class="flex1 plr15 left-flex">
					
					<view class="flex alcenter tc overxy posRelt  ">
						<view :class="['flex1  flex-after  text-xl',{'active text-new-blue':type=='buy'},{'text-333333 dark:text-999FAA':type!='buy'}]" @click="changeType('buy')">{{$t('trade').buy}}</view>
						<!-- <view class="bor abstrot mt5 bg-white dark:bg-1C2532"></view> -->
						<view :class="['flex1  flex-after text-xl',{'active text-new-blue':type=='sell'},{'text-333333 dark:text-999FAA':type!='sell'}]" @click="changeType('sell')">{{$t('trade').sell}}</view>
					</view>
					...
					<view class="">
						<view class="" v-show="priceType=='limit'">
							<view class="bd_input rounded-8 input-pd">
						...
						<view class=""  v-show="priceType=='market'">
							<view class="bd_input flex alcenter radius2 bgBlack">
								<input type="text" :value="$t('trade.best')" disabled class="h40 flex1 plr10 text-gray-500" />
							</view>
						</view>
						...
						<view class="py-10">
							...
							<view class="flex alcenter around tc tab-box gray7 input-title">
								<view :class="['bd_input flex1 radius2',{'bg-k-green text-white bd_green': percent==item&&type=='buy'},{'bg-k-red text-white bd_red': percent==item&&type=='sell'},{'ml10':index>0}]" v-for="(item,index) in percentList" :key='index' @click="changePercent(item)">{{item}}%</view>
							</view>
						</view>
						<view class="py-10">
							<view class="text-838B99 input-title ft12 flex justify-between"> 
				...
				<view class="trade-left flex column">
					...
					<view class="text-white bg-gray-400 p-10 rounded-8 text-center mt-10 mr-10" @click="toDeal">{{$t('new.deal')}}></view>
			...
			<view class="bgPart">
			<view class="flex alcenter between ptb10  plr15 footer pb-10 border-0 border-solid border-b-2 border-gray-300">
				<view class="flex flexend">
					<text class="text-xl text-new-blue">{{$t('trade.delegate')}}</text>
				...
				<view class="flex alcenter gray7">
					...
					<navigator class="ml-20 text-lg text-white bg-gray-400 py-10 px-40 rounded-8 text-center" url="entrust">{{$t('new.newAll')}}</navigator>
				</view>
			</view>
			<!-- 委托类型切换弹窗 -->
