...
const forexStore = defineStore(
    ...
    () => {
        ...
        const initMarket = () => {
            if (getTradeListPromise.value) return;
            getTradeListPromise.value = new Promise((resolve, reject) => {
                getTradeList()
                    .then(({ data }) => {
            ...
            return getTradeListPromise.value
        };
        ...
        const getCurrentTradeItem = computed(() => {
            ...
            const Id =
                typeof currentTradeId.value === "number"
                    ? currentTradeId.value
                    : tradeList.value[0].Id;
            return tradeList.value.find((item) => item.Id === Id) || null;
        });
