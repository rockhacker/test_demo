    ...
    <van-popup v-model:show="show" round position="bottom">
        <div class="p-20">
            ...
            <div class="mt-4 py-4 px-10 flex items-center justify-between border-1 border-solid border-#888 rounded-4">
                ...
                <van-button size="small" type="primary" @click="number = maxNumber">
            ...
            <div class="flex justify-between items-center mt-10">
                <van-button class="w-[48%] rounded-8" color="#999" :disabled="true">
                    {{ current.seconds + 1 }}s{{ $t('legal.auto_cancel') }}
                </van-button>
                <van-button class="w-[48%] rounded-8" color="#2fdab0" @click="onSubmit">
                    {{ $t('legal.submit') }}
                </van-button>
            </div>
        </div>
