...
const useMicro = () => {
    ...
    const getTradeMode = () => {
        getMicroTradeMode().then(({ data }) => {
            ...
            !useCurrency.value && (useCurrency.value = payableCurrency.value[0] || undefined)
        })
    }
...
export const useMicroOrder = () => {
    ...
    const orderTypeList = [
        {
            label: 'trade.in_transaction',
            value: 1 // 交易中
        },
        {
            label: 'trade.closed_position',
            value: 3 // 已平仓
        }
    ]
    ...
    let timer: number | undefined // 秒合约倒计时任务
