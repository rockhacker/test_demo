...
const marketStore = defineStore(
    "market",
    () => {
        const marketInitPromise = ref<Promise<any>>();
        ...
        const initMarket = async () => {
            marketInitPromise.value = getCurrencyMatches()
            await marketInitPromise.value.then(({ data }) => {
                currencyMatches.value = data
            }).finally(() => {
                marketInitPromise.value = undefined
            })
        };
