      ...
      <div v-for="(item,i) in coinlist" :key="i">
        <div class="flex alcenter ptb10 bdbheader plr20">
          ...
          <div class="flex1 tr baselight2" v-if="changeData.is_recharge==1">
            <!-- <el-button   class="bg-main-liner black" style="padding: 4px" size="mini" @click="showInfo(item,i,'charge')" :disabled="!(item.currency&&item.currency.open_recharge==1)">{{$t('assets.charge')}}</el-button> -->
            <!-- <span
              class="pointer"
            ...
            <el-button class="bg-main-liner black" style="padding: 4px" size="mini" @click="showInfo(item,i,'mention')" :disabled="!(item.currency&&item.currency.open_draw==1)">{{$t('assets.mention')}}</el-button>
            <!-- <span
              class="pointer"
        ...
        <div class="mlr20 mtb20 radius2 bdheader ptb20 plr20" v-show="curindex==i&&infotype=='mention'">
          ...
          <div class="flex mt20">
            <div class="flex1">
              <p>{{$t('assets.fee')}}</p>
              <p class="ht40 lh40 plr20 bdinput mt10 w100 baselight2">{{mentionInfo.rate}}</p>
            </div>
            <div class="flex1 ml30">
              <p>{{$t('assets.recivenum')}}</p>
              <p
                class="ht40 lh40 plr20 bdinput mt10 w100 baselight2"
              >{{accSub(mentionInfo.numbers ,mentionInfo.rate) <0?0:accSub(mentionInfo.numbers ,mentionInfo.rate) }}</p>
            </div>
          </div>
