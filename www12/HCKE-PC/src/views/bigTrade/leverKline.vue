<template>
  <div>
    <div class="bgheader bdbheader flex alcenter plr10 ht40">
      <div class="baselight2 ft16 bold mr20">
        {{ currency_matchInfo.Code }}
      </div>
      <div class>
        <div
          class="ft16"
          :class="[
            currency_matchInfo.ForexQuotations.Change < 0 ? 'red' : 'green',
          ]"
        >
          {{ currency_matchInfo.ForexQuotations.Close|| "0.00" }}
        </div>
        <!-- <div
          class="ft12"
        >≈{{ currency_matchInfo.currency_quotation.close *cny_usd || "0.00" | filterDecimals}}CNY</div> -->
      </div>
      <div v-if="currency_matchInfo.MarketStatus!=1" class="plr20 ml20 ptb5 radius8" style="border:1px solid #ff6064;color: #ff6064;">{{$t('new.closed')}}</div>
      <div class="flex1 ft12 tc">
        ...
        <div class="flex alcenter">
          <span
            class="flex1"
            :class="[
              currency_matchInfo.ForexQuotations.Change < 0
                ? 'red'
                : 'green',
            ]"
            >{{
              currency_matchInfo.ForexQuotations.Change ||
              "0.00" | filterDecimals
            }}%</span
          >
          <span class="flex1 baselight2">{{
            currency_matchInfo.ForexQuotations.High || "0.00"
          }}</span>
          <span class="flex1 baselight2">{{
            currency_matchInfo.ForexQuotations.Low || "0.00"
          }}</span>
          <span class="flex1 baselight2">{{
            currency_matchInfo.ForexQuotations.Volume ||
            "0.00" | filterDecimals
          }}</span>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    ...
    this.lang= lang || 'en';
    localStorage.setItem("bigTradePeriod", "1hour"); //默认为1分钟;
    this.createWidget();
  },
  destroyed(){
    this.removeWidget();
  ...
  methods: {
    adjustScale() {
      var that = this;
      var str = that.currency_matchInfo.ForexQuotations.Close;
      console.log('str',str);
      if (str) {
        ...
        .initDataToken(
          {
            ...
            data: {
              from: froms,
              to: tos,
              symbol: that.currency_matchInfo.Code,
              period: periods,
              forex_id:
          ...
          false,
          true,false,{baseURL:`${this.API}/v1/api/`}
        )
        .then(res => {
          arr = res;
          return arr;
    ...
    connect(real) {
      ...
      that.$newSocket.connected(socket => {
        socket.send({
          type: "sub",
          params: "KLINE." + that.currency_matchInfo.Code
        });
        socket.on("KLINE", msg => {
          // console.log(msg,11);
          var klineType = localStorage.getItem("bigTradePeriod");
          if(klineType=='1hour'){
              klineType = '60min'
          }
    ...
    unbindSocket(symbol){
      let that = this;
      this.$newSocket.send([{type: "unsub",params: "KLINE."+symbol}]);
    }, 
    // k线
    createWidget() {
            let that = this;
                ...
                widget.onChartReady(function() { //图表方法
                    ...
                    buttonArr.forEach((v, i) => {
                        ...
                        if(v.text=='1min'){
                            ...
                            localStorage.setItem('bigTradePeriod','1min')  //默认为1分钟
                        }
                    ...
                    let handleClick = (e, value,type) => {
                        ...
                        localStorage.setItem('bigTradePeriod',type)
        ...
        createFeed(){
            ...
            Datafeed.Container.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onHistoryCallback, onErrorCallback) {
                ...
                that.$http.initDataToken({url:'forex/kline',
                    data:{from:rangeStartDate,to:rangeEndDate,symbol:symbolInfo.name,period:resolution,forex_id:that.currency_matchInfo.Id}
                },false,true,false,{baseURL:`${that.API}/v1/api/`})
                .then(res=>{
                  console.log('market/kline');
                    if(res.length>0){
                         res.forEach((item,i)=>{
                          item.id = item.Id
                          item.period = item.Period;
                          // item['base-currency'] = '1'
                          // item['quote-currency'] = '1'
                          // item.market_from = 1
                          item.currency_match_id = that.currency_matchInfo.Id
