<template>
  <div>
    <!-- 头部商家信息部分 -->
    <el-card class="mb20 dark-bg-1C2532 dark-text-white">
      ...
      <div class="ft16 flex alcenter between">
        <div class="flex baselight2">
          <div
            ...
            :class="{'bgwhite dark-bg-1C2532':selectType=='BUY'}"
          >
          ...
          <div
            ...
            :class="{'bgwhite dark-bg-1C2532':selectType=='SELL'}"
          >
            <span  :class="{'text-clip-green':selectType=='SELL','baselight':selectType!='SELL'}">{{$t('store.mysell')}}</span>
          </div>
        </div>
      </div>
      <div class="flex alcenter ft16 bold baselight bgwhite dark-bg-1C2532">
        <div
          class="mlr20 ptb10 pointer plr10 mtb10 radius6"
          v-for="(item,i) in statusList"
          :key="i"
          :class="[i==coinIndex?'bg-main-liner black':'']"
          @click="changeStatus(i,item.status)"
        >{{item.name}}</div>
      </div>
    </div>

    <div class="plr20 pt20 baselight2 bgwhite dark-bg-1C2532">
    ...
    <div class="tc ptb40 bgwhite dark-bg-1C2532" v-if="orderlist.length==0">
    ...
    <el-dialog :title="$t('store.fabu')" :visible.sync="dialogVisiblefabu" width="30%" :before-close="handleClose">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmfabu" class="bg-main-liner black radius4">{{$t('legal.e_confrim2')}}</el-button>
      </span>
    </el-dialog>
