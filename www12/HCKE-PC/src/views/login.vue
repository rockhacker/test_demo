<template>
    <div class="flex alcenter jscenter  minvh100 bg-main-liner login-page">
        <div class="bgwhite radius4 plr60 ptb40 mb50 w25 mwidth">
            ...
            <div class="mt40 ">
                <div class="bold">
                    <span class="mr20 ft14" v-show="i==1" :class="{'active2px':index==i}" v-for="(item,i) in infos" :key="i" @click='changeMethod(i)'><span class="text-clip-green">{{$t(item)}}</span></span>
                </div> 
                <div class="mt20 radius2 ht50 lh50 flex alcenter border-b-EEEEEE">
                    ...
                    <input type="text" class="flex1 h100  pl20 black ft14" v-model="account" @change="changeAccount" :placeholder="index == 0 ? $t('register.enterPhone') : $t('register.enterEmail')">
                </div>
                ...
                <div class="mt40 bg-main-liner black tc h48 lh48 radius2 pointer ft20" @click='login'>{{$t('login.login')}}</div>
            </div>
        </div>
    </div>
</template>
