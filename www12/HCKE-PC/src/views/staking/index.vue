<template>
	<div class="minh100 ft16">
		<div class="bgwhite dark-bg-1C2532" v-if="false">
			<div class="mauto wt1200 ptb20">
				<div class="flex between">
					<div class="flex1">
						<div class="ft40 bold black dark-text-white">{{$t('staking.kuanjishangcheng')}}</div>
						<div class="ft14 text-333333 mtb20 dark-text-999FAA" style="width:500px">
							{{$t('staking.describe1')}}
						</div>
						<div class="ft15 text-333333 mt40 dark-text-999FAA">{{$t('staking.chanpinyoushi')}}</div>
						<div class="flex text-333333 mt20 between">
							<div>{{$t('staking.suicunsuiqu')}}</div>
							<div>{{$t('staking.meirixiafa')}}</div>
							<div>{{$t('staking.suocangzhouqi')}}</div>
							<div>{{$t('staking.suishikongtou')}}</div>
						</div>
						<div @click="toPath" class="flex text-333333 mt20 between pointer">
							{{ $t('new.hdjs') }}
						</div>
		...
		<div class="mauto pt20 wt1200 pb80">
					 ...
					 <div class="flex2">
							...
							<div class="flex1 ml20">
								 <div class="text-333333 dark-text-white bold mb30">{{item.title}}</div>
								  <div class="flex flexend">
									...
									<el-tag size="mini" class="bg-main-liner black">{{$t('staking.meiqipaixi')}} {{item.dividend_percentage}}%</el-tag>
								 </div>
							 </div>
						 </div>
						 ...
						 <div class="mt20 ft14 flex text-333333 dark-text-white">
						 	...
						 	<div class="flex1">
						 		...
						 		<span class="ml10">{{item.lock_dividend_days}} {{$t('new.day')}}</span>
						 	</div>
						 </div>
					 </div>
					 <div class="flex1 flex column between">
						 <div class="flex jsend">
							<el-button class="wt100 mr10 bg-main-liner black" style="border-radius: 0;" @click="$router.push({path:`/stakingDetails?id=${item.id}`})">{{$t('staking.shengou')}}</el-button>
						 </div>
			...
			<el-card class="box-card mb20 tc ptb40" v-if="stakingData.length==0">
