<template>
	<view v-if="show">
		...
		<view :class="type == 'middle' ? 'flex justify-center items-center flex-col my-contents' : ''">
			<view :class="['uni-popup', { 'bg-white': bgColor, 'bg-floor': !bgColor }, 'uni-popup-' + type]">
				...
				<view v-if="btnShow" class="uni-popup-bottoms">
					<view class="uni-popup-bottom1 flex justify-center items-center">
						<view class="cannel flex-1" @tap="hide">{{$t('trade.cancel')}}</view>
						<view class="comfirms flex-1" @tap="comfirms">{{$t('login.e_confrim')}}</view>
					</view>
				</view>
			</view>
		</view>
