<template>
	<view :class="theme">
		<view class="bg-white dark:bg-main-black text-black dark:text-white">
				...
				<view class="pt-40" v-if="coinInfo.currency_protocols&&coinInfo.currency_protocols.length > 1">
					...
					<view class="flex mt-20">
						<view v-for="(item, index) in coinInfo.currency_protocols" :key="index" class="mr-20 px-20 py-10 border-2 border-solid"
							:class="name == item.chain_protocol.code ? 'text-new-orange border-new-orange' : 'text-gray-500 border-gray-500'" @tap="selectCharge(item)">
			...
			<view class="p-40 ">
				...
				<view class="mb-40">
					...
					<view class="flex items-center justify-between border-0 border-b-2 border-solid border-gray-200 dark:border-gray-700">
						<input type="digit" class="h-80 flex-1" v-model="number" @input="numberChange" :placeholder="$t('assets.minnum') + (coinInfo.draw_min || '0.00')" />
				...
				<view class="mb-40">{{ $t('trade.fee') }}：{{ coinInfo.draw_rate || '--' }} {{ currencyName || '--' }}</view>
				
				<view class="mt-20 rounded-10 h-80 flex justify-center items-center text-black text-sm text-center bg-new-orange" @tap="mention">{{ $t('assets.mention') }}</view>
			</view>
...
export default {
	...
	methods: {
		...
		all() {
			...
			const res = this.accSub(this.balance,this.coinInfo.draw_rate)
			this.reciveNumber = parseFloat(res) < 0 ? 0: res
		},
		numberChange(e) {
			// 到账数量
			...
			const res = this.accSub(e.target.value,this.coinInfo.draw_rate)
			this.reciveNumber = parseFloat(res) < 0 ? 0: res
		},
		// 选择充币类型
