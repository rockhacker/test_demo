<template>
  <view>
    <view @click="showDetail" class="p-20 border-0 border-solid border-b-2 border-gray-600">
      <view class="flex">
        <view>{{
        ...
        <view>{{ info.symbol_name }} {{ info.seconds }}{{ $t('new.m') }}</view> 
      </view>
      <view class="flex justify-between mt-10">
		...
		<view class="mt-10" v-if="remain_milli_seconds==0 && info.status == 1">{{$t('second.settling')}}</view>
          <view class="mt-10" v-else :class="[ploss>=0?'text-success':'text-danger']">{{ ploss>=0? ploss:`-${info.number}` }}</view>
        </view>
      </view>
      <view class="flex justify-between mt-10">
        <view  v-if="(info.status==1)" class="text-left">
          <view>{{ $t("second.Countdown") }}</view>
		<view class="mt-10" v-if="remain_milli_seconds==0">{{$t('second.settling')}}</view>
          <view  class="mt-10" v-else   style="display: block">
            {{ remain_milli_seconds }} {{ $t('new.m') }}
          </view>
        </view>
        <view class="flex flex-col justify-center items-center">
    ...
    <uni-popup :show="showOrderDetail" :btnShow="false" :msg="info.symbol_name" @hidePopup="showOrderDetail=false">
        ...
        <!-- <view class="text-52 pt-30" v-else>{{ secondsFormat(remain_milli_seconds)}}</view> -->
        <view class="text-52 pt-30" v-else>
				<view class="text-52 pt-30" v-if="remain_milli_seconds==0">{{$t('second.settling')}}</view>
		       <view class="text-52 pt-30" v-else>{{remain_milli_seconds}} {{ $t('new.m') }}</view>
		</view>
        <!-- <view class="text-52 py-16" v-else :class="ploss < 0 ? 'text-danger': 'text-success'">{{ ploss >=0? '+':'' }} {{ ploss }}</view> -->

				<view class="py-60 w-full mt-30">
					<view class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('second.jyfx')}}</view>
        ...
        <view  class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('second.closed_price')}}</view>
					<view class="text-base">{{info.end_price}}</view>
				</view>	

        <view  class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('second.miao')}}</view>
					<view class="text-base">{{info.seconds}}</view>
				</view>	

        <view  class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('second.income')}}</view>
					<view class="text-base">{{info.profit_ratio *100}}%</view>
				</view>	
        <view  class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('second.profit_amount')}}</view>
					<!-- <view class="text-base">{{info.fact_profits}}</view> -->
        ...
        <view  class="flex justify-between w-full px-30 mb-20">
					...
					<view class="text-base">{{info.created_at}}</view>
				</view>	
        <view  class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('trade.time_end')}}</view>
					<view class="text-base">{{info.handled_at}}</view>
				</view>	
        </template>
        <template  v-else>
          <view class="flex justify-between w-full px-30 mb-20">
        ...
        <view class="flex justify-between w-full px-30 mb-20">
					<view class="text-base">{{$t('second.profitLoss')}}</view>
						<view class="mt-10" v-if="remain_milli_seconds==0">{{$t('second.settling')}}</view>
          <view class="text-base" v-else :class="[ploss>=0?'text-success':'text-danger']">{{ploss>=0? ploss:`-${info.number}`  }}</view>
				</view>
        </template>
				</view>
...
export default {
    ...
    this.timer = setInterval(() => {
      ...
      if (this.remain_milli_seconds <= 0) {
        this.remain_milli_seconds = 0;
          this.$emit('secondsOut',this.info)
  ...
  methods: {
    ...
    secondsFormat(s) {
      ...
      let se = second >= 0 ? second + this.$t("new.m") : "";
      return d + h + m + se;
    },
  },
  beforeDestroy() {
