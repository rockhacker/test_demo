<template>
	<view :class="theme">
		<view class="bg-my-grey dark:bg-main-black">
			<uni-nav-bar fixed>
				<view class="w-full flex flex-col p-40">
					<view class="w-full flex justify-center">
						<view class="flex w-full h-60 bg-F4F5F7 dark:bg-main-black rounded-2xl overflow-hidden">
							<view class="flex-1 flex justify-center items-center text-center" :class="active==0?'bg-new-orange text-white':'text-333333 dark:text-white'" @tap="setActive(0)">{{$t('new.leverLes')}}</view>
							<view class="flex-1 flex justify-center items-center text-center" :class="active==1?'bg-new-orange text-white':'text-333333 dark:text-white'" @click="setActive(1)">{{$t('new.leverAll')}}</view>
							<!-- <view class="flex-1 flex justify-center items-center text-center" :class="active==2?'bg-new-orange text-white':'text-333333 dark:text-white'" @tap="setActive(2)">{{$t('qqjy.qqjy')}}</view> -->
						</view>
					</view>
			...
			<view v-if="active==1 && !soon" class="min-h-screen bg-gray-100 dark:bg-floor">
				<!-- <view class="flex justify-center items-center text-3xl h-600" v-if="soon">{{$t('trade.soon')}}</view> -->
				<view class="flex justify-between p-20 bg-white dark:bg-main-black dark:text-white">
					<view class="w-3_5">
						...
						<view class="">
							<view class="flex alcenter around tc tab-box gray7 input-title">
							 ...
							 <!-- <slider class="slider-style" :activeColor=" leverTradeType == 'sell'  ? '#d74e5a' : '#12b886'"  block-size="10" :value="value1" @change="changePercent" show-value /> -->
						</view>
				...
				<!-- 当前委托列表 -->
				<view class="mt-20 p-20 bg-white dark:bg-main-black pb-120 min-h-half dark:text-white">
					<view class="flex justify-between">
	...
	methods: {
		...
		chart(){
			this.$refs.second.show({
				...
				symbol:this.symbol,
				decimal:this.currencyMatch.decimal,
		...
		calculation(prices) {
			...
			submits2(){
			var that = this;
			console.log(that.leverOrder.mult)
			console.log(that.leverOrder.share)
			if (!that.leverOrder.mult || !that.leverOrder.share) {
				that.leverOrder.bond = 0
				return false;

			}
			if (that.priceStatus == 0) {
				console.log(that.priceValue)
				if (!that.priceValue) {
		...
		handicap() {
			...
			this.$ws.Sub([`MARKET_DEPTH.${this.symbol}`,'DAY_MARKET'])
			this.$ws.registerCallBack(this.handleCallBack)
		},
		handleCallBack(msg){
