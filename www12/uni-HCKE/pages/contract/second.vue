<template>
  <view class="pb-300 bg-white dark:bg-main-black dark:text-white">
    ...
    >
      <view
        class="flex1 text-center py-20 border-0 border-solid border-new-orange"
    ...
    <view class="px-30 mt-20">
      <view class="flex flex-wrap">
        ...
        >
          <view class="text-gray-600">
            {{ item.seconds }}{{ $t("new.m") }}
          </view>
          <view style="transform: scale(0.8); color: #e27046">
            {{ $t("second.income") }}{{ item.profit_ratio * 100 }}%
          </view>
          <image
            v-if="item == second"
            class="w-50 h-40 absolute bottom-0 right-0"
      ...
      <view class="mt-30">
        ...
        >
          <input class="flex1" v-model="num" type="text" />
          <picker
    ...
    <view class="border-0 border-t-2 border-solid border-gray-300 dark:border-gray-600">
      <view class="flex m-20">
        <view
          class="text-base border-0 border-solid border-new-orange py-10"
        ...
        <view
          class="text-base ml-40 border-0 border-solid border-new-orange py-10"
      ...
      <template v-if="orderType == 1">
        <order @secondsOut="secondsOut" v-for="item in list" :key="item.id" :info="item"></order>
      </template>
      <template v-else>
    ...
    <!-- 下单弹窗 -->
			<uni-popup v-if="showModal" :show="showModal" @hidePopup="hideBtn" @comfirmPopup="confirmBtn" :msg="$t('trade.confrim_order')" :btnShow="true" key="comfirmPopup" :lang="langs">
        <view class="w-full p-40 ">
					<view class="text-lg">
						<view class="">{{ currencyMatch.symbol }}</view>
						<view class="flex justify-between mt-20 ">
							<view class="text-lg">{{ $t('trade.types') }}</view>
							<view class="text-lg">{{ type == '1' ? $t('second.buy') : $t('second.sell') }}</view>
						</view>
						<view class="flex justify-between mt-20">
							<view class="text-lg">{{ $t('second.numOpen') }}</view>
							<view class="text-lg">{{ num }}</view>
						</view>
						<view class="flex justify-between mt-20">
							<view class="text-lg">{{ $t('second.miao') }}</view>
							<view class="text-lg">{{ second.seconds }}</view>
						</view>
						<view class="flex justify-between mt-20">
							<view class="text-lg">{{ $t('second.income') }}</view>
							<view class="text-lg">{{ second.profit_ratio * 100 }}%</view>
						</view>
					</view>
				</view>
			</uni-popup>
