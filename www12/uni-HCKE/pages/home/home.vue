  ...
  <view class="overflow-x-hidden" :class="theme">
	  <view class="bg-white dark:bg-main-black">
		  ...
		  <uni-nav-bar fixed>
			...
			<view class="absolute right-0 top-0 z-10 h-100 flex items-center mx-20">
				<view class="py-10 px-20 bg-new-orange rounded-full text-black" @tap="showRight = true">
					{{$t('new.yy')}}
				</view>
				<!-- <navigator  open-type="switchTab" url="/pages/assets/assets" class="py-10 px-20 mx-20 bg-new-orange rounded-full text-white">
		...
		<uni-swiper-dot :dots-styles="dotsStyles" :info="bannerList" :current="current" field="content" :mode="mode">
				<swiper  :autoplay="true" :interval="3000" active-class="activeClass" class="swiper h-300 w-full" @change="change">
		...
		<view class="flex py-40 text-my-black mt-20 dark:text-white">
			<!-- <view class="flex items-center flex-1 justify-center flex-col text-center" @click="toPath('/pages/subscription/index')">
			<image class="w-84 h-84" src="/static/image/position.png"></image>
		 ...
		 <!-- <view class="flex items-center flex-1 justify-center flex-col text-center" @click="toLink('https://exchange-hk.oss-cn-hongkong.aliyuncs.com/ethfx/IMG_0637.MP4')">
			<image class="w-84 h-84" src="/static/image/whitepaper.png"></image>
			<view class="text-lg mt-20">{{ $t("new.spjs") }}</view>
		  </view> -->
		</view>

		<!--涨幅榜-->
		<swiper v-if="quoList.length" :display-multiple-items="quoList.length>2?3:quoList.length" class="w-full h-170 bg-white dark:bg-main-black pl-20 pr-20">
		  <swiper-item class="flex items-start justify-around"  v-for="(item, i) in quoList" :key="i">
			<view @click="toUrl(item,allList[1])" class="flex flex-col items-center w-200 h-170 justify-center rounded-10 relative">
				<text class="text-1A222B mb-20 dark:text-white" style="font-size: 28rpx;">{{ item.base_currency_code }}/{{ item.quote_currency_code }}</text>
      ...
      <view
        class="w-full rounded-full h-90 flex items-center justify-between px-60 bg-k-red dark:bg-new-orange"
        @click="toPath('/pages/home/recharge')"
      >
	  	<view class="flex items-center">
			<image src="/static/image/kscz.png" class="w-70 h-70 mr-10"></image>
      ...
      <!-- <view
        class="mt-20 w-full rounded-full h-90 flex items-center justify-between px-60 bg-new-orange"
      ...
      >
		...
		<!--行情-->
	<view class="pb-100 mt-20 bg-mg-gray">
		 <view  v-for="(list, index) in allList" :key="index">
			  <!-- <view @click="toUrl(allList[index].matches[0],allList[index])" class="flex items-center text-sm text-gray-400 tableitem mx-40 mb-30 py-30 bg-new-orange px-20">
		  ...
		  <view class="flex items-center text-my-black text-lg mx-40 px-10 py-20 bg-EDEDED dark:bg-main-black dark:text-white">
		  ...
		  <view @click="toUrl(item,allList[index])" class="flex items-center text-sm text-gray-400 tableitem mx-40 py-30" v-for="(item, i) in list.matches" :key="i">
			<view class="flex items-center" style="flex: 1.5">
				<image class="w-40 h-40 mr-16 rounded-half bg-white" :src="host + item.base_currency.logo"></image>
				<view class="text-sm text-my-black dark:text-white"><text class="text-sm text-mg-black">{{ item.base_currency_code }}</text>/{{ item.quote_currency_code }}</view>
			</view>
			<view v-if="item.base_currency_code=='MUT' && !isOnline(item.market_time)" class="w-200">{{$t('trade.soon')}}</view>
			<template v-else>
				<view class="flex-1 text-my-black text-sm dark:text-white" v-if="item.currency_quotation">{{ parseFloat(item.currency_quotation.close) }}</view>
		...
		<uni-popup :show="announcementModal.showModal" @hidePopup="hideBtn" :msg="announcementModal.title" :btnShow="announcementModal.tipbtnShow" :lang="lang">
			<view class="w-full p-40 h-700 overflow-auto" v-html="announcementModal.content">
			</view>
	...
	<uni-drawer :visible="showRight" style="background: rgb(16, 32, 48,.5);" mode="right" @close="showRight = false">
		<view class="flex column ht100">
			<view class="flex items-center p-40 border-0 border-b-2 border-solid border-gray-100 dark:border-gray-900 justify-between dark:text-white"
			v-for="(item, i) in languages" :key="i" @tap="changeLang(i)">
...
export default {
	...
	data() {
		return {
			...
			languages: {
					...
					pt: { name: 'Portugal', img: '/static/image/pt.png' },
					// hi: {
					// 	name: 'हिन्दी',
			...
			dotsStyles:{
				...
				selectedBackgroundColor:'#FFCE1C',
				bottom:-20
			}
		};
	},
	...
	methods: {
		...
		topo(){
			...
			window.open(`${this.$utils._DOMAIN}/wp/ETHFXX-${lang}.pdf`);
