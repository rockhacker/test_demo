<template>
	<view :class="theme" class="rechargeOpen">
         <view class="bg-my-grey dark:bg-main-black flex justify-center items-center flex-col p-30 dark:text-white">
			  ...
			  <button class="w-full rounded-10 bg-new-orange text-black text-xl" @click="setCopy">{{$t('feed.copyadd')}}</button>
			  <view class="open-form w-full">
			       <text class="text">{{$t('feed.depositsAddress')}}</text>
			       <input class="uni-input bg-my-grey dark:bg-main-black  dark:text-white border-0 border-solid border-b-2 border-C9C9C9"  v-model="depositsAddress" :placeholder="$t('feed.depositsAddressUp')"/>
				   <text class="text">{{$t('feed.deposits')}}</text>
				   <input class="uni-input bg-my-grey dark:bg-main-black  dark:text-white border-0 border-solid border-b-2 border-C9C9C9"  v-model="deposits" :placeholder="$t('feed.depositsUp')"/>
			  ...
			  <button class="w-full rounded-10 bg-new-orange text-black text-xl" @click="submit">{{$t('feed.submit_transaction_screenshot')}}</button>
           ...
           uploadImg(i){
				...
				uni.chooseImage({
					...
					success: (chooseImageRes) => {
						...
						uni.uploadFile({
							url: helper._DOMAIN +'/api/common/image_upload', //仅为示例，非真实的接口地址
							// #ifdef APP-PLUS
