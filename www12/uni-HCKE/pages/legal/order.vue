<template>
	<view class="" :class="{'light':theme=='light'}">
		<view class="status_bar bg-new-orange">
			<view class="top_view bg-new-orange"></view>
		</view>
		<view class="header fixed flex alcenter between plr15 bg-new-orange">
			<view>
				<image src="../../static/image/forward.png" mode="aspectFit" class="wt20 h20" @click="back()"></image>
			</view>
		</view>
		<view class="">
			<view class="done flex between alcenter pt60 plr15 pb15 bg-new-orange">
				...
				<view class="flex column alcenter" @click="call()">
					...
					<text class="ft12 white mt5">{{$t('new.lianxikefu')}}</text>
				</view>
			</view>
			<view class="price flex column plr15 ptb15">
				<view class="flex alcenter">
					<text class="mr5 text-new-orange ft18 bold" v-if="data.area_info">{{data.amount}} {{data.area_info.currency}}</text>
			...
			<view class="info flex column plr15 ft13  pt10 pb150">
				<view v-if="data.way" class="text-xl text-red-500">
					{{ data.way=='SELL'? $t('new.wait_handle') : $t('new.lxkf')}}
				</view>
				<template v-if="false">
				...
				<view v-if="false" class="flex between alcenter ptb10 bdb1f">
				...
				<view class="flex alcenter ptb10" v-if="data.status==0 && data.way=='BUY' && false">
		...
		<uni-popup ref="ceil" :type="type" :custom="true">
			...
			<view class="uni-tip" v-if="payType=='ceil'">	
				...
				<view class="uni-tip-content flex column white ft12">
					<text class="text-new-orange">{{$t('store.t_tip1')}}</text>
				...
				<view class="uni-tip-group-button">
					...
					<view class="uni-tip-button text-new-orange" @click="cancel('ceil'),submit()">{{$t('login.e_confrim2')}}</view>
				</view>
			</view>
			<view class="uni-tip" v-if="payType=='ipay'">	<!-- 确认付款弹窗 -->
				...
				<view class="uni-tip-content white tc ft12">
					<text class="text-new-orange">{{$t('store.donotconfrim')}}。</text>
			...
			<view class="uni-tip" v-if="payType=='depay'">	<!-- 延期确认弹窗 -->
				...
				<view class="uni-tip-content white tc ft12">
					<text class="text-new-orange">{{$t('store.notpayconfrim')}}。</text>
		...
		methods: {
			...
			downTime(startTime,endTime){//倒计时
				...
				that.setTimer=setInterval(function(){
					...
					if(ts>0){
						if(dd>0){
							that.timer = dd + that.$t('new.day') + hh + that.$t('new.s') + mm + that.$t('new.f') + ss + that.$t('new.m') ;
						}
						else if(hh>0){
