<template>
	<view :class="theme">
		<view class="bg-white dark:bg-main-black min-h-screen text-gray-700 dark:text-gray-100">
			...
			<view class="flex items-center justify-between text-sm border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700">
				<view class="px-40 py-20" :class="status == item.id ? 'text-new-orange border-0 border-b-2 border-solid border-new-orange' : ''" v-for="item in tabs" :key="item.id" @tap="selectType(item.id)">{{ item.texts }}</view>
			</view>
			
			<view class="px-40">
				<view v-for="(item, index) in orderList" @click="clickItem(item)" :key="index" class=" border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 py-20">
					...
					<view class="flex flex-col">
						...
						<view class="flex mb-20 justify-between">
							...
							<view class="flex-1 text-center" v-if="item.status == 3">
								<view class="text-gray-400 mb-10">
									{{$t('new.ykb')}}
								</view>
								<view class="text-black dark:text-white text-sm">
									{{ item.profits/item.caution_money*100 | toFixed4 }}%
								</view>
							</view>
							<view class="flex-1 text-right" >
								<view v-if="item.handle_time" class="text-gray-500">{{ $t('trade.time_end') }}</view>
								<view v-if="item.handle_time" class="mt-10">{{ item.handle_time }}</view>
							</view>
						</view>
					</view>
					<!-- 按钮 -->
					<view class="flex text-right mt-30 justify-end" v-if="status == 0 || status == 1">
						<view class="bg-main-linear px-30 py-20 rounded-8 text-white" @tap="loss(item,index)">{{$t('lever.setys')}}</view>
						<view class="bg-main-linear px-30 py-20 rounded-8 text-white ml-20" @tap="closes(item.id, index)">{{ status == 0 ? $t('trade.chedan') : $t('trade.pingcang') }}</view>
					</view>
				</view>
			</view>
			...
			<uni-popup :show="lossData.shows" @hidePopup="hideBtn" @comfirmPopup="lossClose" :msg="lossData.title" :lang="langs">
			 ...
			 <uni-popup :show="showOrderDetail" :btnShow="false" :msg="item.currency_code" @hidePopup="showOrderDetail=false">
							<view class="text-52 py-16" v-if="item.status ==3" :class="item.fact_profits < 0 ? 'text-danger': 'text-success'">{{ item.fact_profits >=0? '+':'' }} {{ item.fact_profits }}</view>
							<view class="flex justify-between w-full px-30 mb-20" v-if="item.status == 3">
								<view class="text-gray-500 mb-10">
									{{$t('new.ykb')}}
								</view>
								<view class="text-black dark:text-white text-sm">
									{{ item.profits/item.caution_money*100 | toFixed4 }}%
