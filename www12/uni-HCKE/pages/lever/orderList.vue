<template>
	<view :class="theme">
		<uni-nav-bar backIcon fixed>
			<view class="flex justify-between items-center py-20 pr-20 w-full bg-white dark:bg-main-black dark:text-white">
				<view>
					<view>{{$t('lever.risk')}}：{{ risk || '0.00' | toFixed2 }} %</view>
					<view class="pt5">
						{{$t('lever.totalyk')}}：<text :class="totalRisk < 0?'text-k-red':'text-k-green'">{{ totalRisk || '0.0000' |toFixed4 }}</text>
					</view>
				</view>
				<view class="bg-new-orange text-black py-20 text-center rounded-10 w-200" @tap="allClose.shows=true">{{$t('lever.yijian')}}</view>
			</view>
		</uni-nav-bar>
		
		<view class="list text-xl px-30 bg-white dark:bg-main-black mt-20" style="min-height: calc(100vh - 70px - var(--status-bar-height));">
			...
			<template v-else>
				<view v-for="(item, index) in orderList" :key="index" class="py-30 border-0 border-solid border-b-2 border-gray-100 dark:border-gray-900">
					<view class="flex justify-between mb-20">
						<view class="">
							<text class="text-lg" :class="item.type == 1 ? 'text-k-green' : 'text-k-red'">{{ item.type == 1 ? $t('trade.buy') : $t('trade.sell') }}</text>
							<text class="ml-10">{{ item.symbol }}</text>
							<text class="ml-10">x{{ item.share }}{{$t('trade.hand')}}</text>
						</view>
						<view :class="item.profits < 0 ? 'text-k-red' : 'text-k-green'">{{ item.profits || '0.0000' | toFixed4 }}</view>
					</view>
					...
					<view class="flex mb-20">
						...
						<view class="flex-1 text-center">
							...
							<view class=" dark:text-white text-sm" :class="item.profits < 0 ? 'text-danger' : 'text-success'">
					...
					<view class="flex text-right mt-30 justify-end">
						<view class="bg-new-orange px-40 py-20 rounded-8 text-white" @tap="loss(item,index)">{{$t('lever.setys')}}</view>
						<view class="bg-new-orange px-40 py-20 rounded-8 text-white ml-20" @tap="closes(item.id,index)">{{$t('trade.pingcang')}}</view>
					</view>
				</view>
			</template>
		</view>
...
export default {
	...
	onHide() {
		this.$ws.unSub(['LEVER_TRADE','LEVER_CLOSED'])
	...
	methods: {
		...
		orderTrade(){
			...
			this.$ws.Sub(['LEVER_TRADE','LEVER_CLOSED'])
