<template>
	<view :class="theme" class="flex flex-col">
		...
		<view class="w-full flex flex-col">
			<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
				<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
					...
					:class="isActive==i?'text-new-orange':'text-838B99'" v-for="(item,i) in marketLists" :key="i" 
					@click="changeTab(i,item.code,item)">
					{{item.code}}
					<view class="w-70 h-6 mt-10" :class="{'bg-new-orange':isActive==i}"></view>
				</view>
			</scroll-view>
			
			<view class="bg-white dark:bg-main-black text-838B99 dark:text-9BA4AD px-36 rounded-tl-3xl rounded-tr-3xl">
		...
		<view class="bg-white dark:bg-main-black pb-100" style="min-height: calc(100vh - 100px);">
