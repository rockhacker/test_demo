<template>
	<view :class="theme">
		 <view class="p-30 bg-my-grey dark:bg-main-black text-black dark:text-white" style="min-height: 100vh;">
			<view>
			<view class="ft24 text-new-orange" v-if="loginActive==0">
				{{$t('login').r_mobile}}
			</view>
			<view class="ft24 text-new-orange" v-else>
				{{$t('login').r_email}}
			</view>
			...
			<view class="mt40 ">
				<view class="mb10 ">
					...
					<view class="flex bgwhite alcenter bdb_myblue ">
						...
						<button size="mini" type="primary" class="px-40 bg-new-orange text-white"  :disabled="disable" :loading="load" @click="send">{{ codeText }}</button>
					</view>
					<view class="flex alcenter flex1 bdb_myblue">

						<view class="flex  alcenter between mr10" >
							<picker :value="index" :range="array" class=" flex1"  @change="bindPickerChange" range-key="code">
								<!-- <view class="uni-input">{{array[index].name_cn}}</view> -->
								<view class="flex items-center pr20">
									<view class="ft12" :class="index===''?'w-10 h-10':''" >{{ index!=='' ? array[index].global_code:''}}</view>
							        <image v-if="index===''" src="/static/image/trade_arrow_down.png" class="wt15 h10 ml5"></image>
								</view>
							</picker>
							
						</view>
						<input type="text" v-model="mobile" class="h40 lh40  flex1" :placeholder="$t('login').p_mobile">
					</view>
					<view class="flex bgwhite alcenter bdb_myblue ">
					...
					<view class="mt20 flex alcenter">
						...
						<navigator :url="'/pages/home/agreement'">
							<view class="ml10 text-new-orange">《{{$t('login').p_private}}》</view>
						</navigator>
					</view>
					<view class="mt45 radius4 ptb10 ft14 tc mb10 bg-new-orange text-white" @tap="register">{{$t('login.register')}}</view>
				</view>
			</view>
			<view class="mt20 flex justify-start alcenter" style="justify-content: end;">
				<!-- <view class="text-new-orange" v-if="loginActive==1" @click="loginActive=0;user_string=''">
					{{$t('login.r_mobile')}}
				</view>
				<view class="text-new-orange" v-else @click="loginActive=1;user_string=''">
					{{$t('login.r_email')}}
				</view> -->
				<!-- #ifdef H5 -->
		...
		methods:{
			...
			getCountry(){
				this.$utils.initData({url:'default/area_list'},(res,msg)=>{
					...
					this.index = res.findIndex((item)=>item.id == 2) || 0
					this.area_code  =this.array[this.index].id;
				})
			},
			// 区号选择
			bindPickerChange(e){
			...
			register(){
				...
				var data={
					...
					mobile:this.mobile ? `${this.array[this.index].global_code}-${this.mobile}`:''
				}
				if(this.loginActive==0){
					// 手机
					data.type='mobile';
