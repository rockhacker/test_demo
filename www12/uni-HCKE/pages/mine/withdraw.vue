<template>
	<view :class="theme">
		<view class="min-h-screen text-black dark:text-white bg-white dark:bg-main-black">
			<view class="flex items-center justify-between p-30 border-0 border-b-2 border-solid border-gray-100 dark:border-gray-900" v-for="(item,index) in coinList" :key="index" @click="toUrl(item)">
		...
		methods:{
            ...
            getDetail(item) {
                ...
                that.$utils.initDataToken({ url: 'account/detail', data: { account_type_id: 1, id: item.id } }, res => {
                    ...
                    let url='/pages/assets/mention?currency=' + item.currency.currency_protocols[0].currency_id + '&name=' + item.code + '&id=' + item.id +'&account_type_id='+res.account_type_id
