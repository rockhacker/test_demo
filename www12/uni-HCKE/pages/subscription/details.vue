<template>
	<view :class="theme">
		<view class="p-30 bg-my-grey dark:bg-my-black ">
			<view class="bg-white dark:bg-main-black p-20 mb-20 rounded-8">
				
				<view class="flex flex-col text-xl py-20" v-if="coin.list[coin.active]">
					<text class="mr-10 text-gray-400">{{ $t('subscription.balance')}}</text>
				...
				<view class="border-0 border-b-2 border-solid border-gray-200 py-30">
					...
					<view class="text-2xl font-bold text-clip-green">
				...
				<view class="border-0 border-b-2 border-solid border-gray-200 py-30">
					<view class="flex justify-between text-lg mb-10">
						<text class="text-gray-400">{{$t('subscription.faxingshuliang')}}</text>
						<text class="text-clip-green">{{subscription.issue_number}} {{subscription.currency_name}}</text>
					</view>
					<view class="flex justify-between text-lg">
						<text class="text-gray-400">{{$t('subscription.shengyushuliang')}}</text>
						<text class="text-clip-green">{{subscription.issue_number - subscription.subscribed || ''}} {{subscription.currency_name}}</text>
					</view>
				</view>
				
				<view class="flex text-lg text-gray-400 my-40">
					<view class="flex-1">
				...
				<view class="flex h-80 my-40">
					<view class="w-120 mr-20 text-gray-400 border-0 border-b-4 border-solid border-my-green h-full flex justify-center items-center">
					...
					<view class="flex-1 border-0 border-b-4 border-solid border-my-green h-full">
						<input type="number" :placeholder="$t('legal.p_amout')" class="h-full text-xl px-20 text-black" v-model="amount">
					</view>
				</view>
				
				<view class="mt-20 rounded-10 p-20 text-white text-sm text-center bg-main-linear my-40" @tap="submit">{{$t('subscription.lijishenggou')}}</view>
				<view v-if="subscription.currency && subscription.currency.desc">
					<view class="flex between">
						<view class="text-xl  text-black dark:text-white">{{$t('subscription.bizhongjieshao')}}</view>
						<view v-if="subscription.link" class="bg-main-linear text-white p-10" @click="baip">{{$t('new.baip')}}</view>
					</view>
					<view class="flex1 mt20 text-black dark:text-white break-all">{{subscription.currency.desc}}</view>
				</view>
			</view>
		</view>
    </view>
...
<script>
	...
	export default{
		...
		methods:{
			baip(){
				window.location.href = this.subscription.link
			},
			// 修改支付币种
			changeCoin(e){
				...
				try {
					...
					that.$utils.initDataToken({ 
						...
						uni.setNavigationBarTitle({
							title: `${this.subscription.currency_name} ${this.$t('subscription.new_subscription')}`
						});
					});
				} catch (e) {
					console.log(e);
