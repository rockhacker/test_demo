<template>
	<view class="bg-main-black min-h-screen text-white" :class="{'light':theme=='light'}">
		<view class="flex flexend between plr15 ptb10 w100 bdb_1e fixed lf0 zdx100">
			<view class="flex flexend">
				<view :class="['ft14 gray7',{'ft18 text-new-orange':current==0}]" @click="change(0)">{{$t('trade.all_delegate')}}</view>
			    <view :class="['pl10 ft14 gray7',{'ft18 text-new-orange':current==1}]" @click="change(1)">{{$t('trade.his_delegate')}}</view>
			</view>
			...
			<view class="ptb10 bdb_1e pr15" v-for="(item,index) in enList" :key="item.id">
				<view class="flex between alcenter">
					<view class="flex flex flexend">
						...
						<text class=" pl5 ft12">{{item.created_at.substring(10,16)}} {{item.created_at.substring(5,7)}}/{{item.created_at.substring(8,11)}}</text>
					</view>
					<view class="blueColor bgBlack radius2 plr20 ptb5" @click="revoke(index,item.id)">{{$t('trade.back')}}</view>
				</view>
				<view class="mt15 flex">
					<view class="flex1">
						<text class="" v-if="item.currency_match">{{$t('trade.price')}}({{item.currency_match.quote_currency_code}})</text>
						<view class="mt5">{{item.price}}</view>
					</view>
					<view class="flex1 tc">
						<text class="" v-if="item.currency_match">{{$t('trade.num')}}({{item.currency_match.base_currency_code}})</text>
						<view class="mt5">{{item.number}}</view>
					</view>
					<view class="flex1 tr">
						<text class="" v-if="item.currency_match">{{$t('trade.real_num')}}({{item.currency_match.base_currency_code}})</text>
			...
			<view class="ptb10 bdb_1e pr15" v-for="item in hisList" :key="item.id">
				<view class="flex between alcenter">
					<view class="flex flex flexend">
						...
						<text class="text-my-black pl5 ft14">{{item.currency_match.base_currency_code}}/{{item.currency_match.quote_currency_code}}</text>
				...
				<view class="mt15 flex">
					<view class="flex1">
						<text class="">{{$t('trade.fee')}}</text>
						<view class="mt5">{{item.fee}}</view>
					</view>
					<view class="flex1 tc">
						<text class="">{{$t('trade.delegate_price')}}({{item.currency_match.quote_currency_code}})</text>
						<view class="mt5">{{item.price}}</view>
					</view>
					<view class="flex1 tr">
						<text class="">{{$t('trade.w_num')}}({{item.currency_match.base_currency_code}})</text>
						<view class="mt5">{{item.number}}</view>
					</view>
				</view>
				<view class="mt10 flex">
					<view class="flex1">
						<text class="">{{$t('trade.cj_total')}}({{item.currency_match.quote_currency_code}})</text>
						<view class="mt5">{{item.transacted_volume}}</view>
					</view>
					<view class="flex1 tc">
						<text class="">{{$t('trade.cj_price')}}({{item.currency_match.quote_currency_code}})</text>
						<view class="mt5">{{item.avg_price}}</view>
					</view>
					<view class="flex1 tr">
						<text class="">{{$t('trade.cj_vol')}}({{item.currency_match.base_currency_code}})</text>
						<view class="mt5">{{item.transacted_amount}}</view>
					</view>
				</view>
			</view>
