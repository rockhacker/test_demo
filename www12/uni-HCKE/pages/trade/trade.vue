<template>
	<view class="pb-100" :class="theme">
		<!-- 头部 -->
		<view class="h-88 w-full bg-main-black fixed flex alcenter jscenter plr15 zdx100">
			<view class="flex alcenter jscenter" @click="showLeft=true">
				<!-- <image src="/static/image/list.png" class="h20 wt20"></image> -->
				<text class="text-white font-bold text-xl">{{currency_name}}/{{legal_name}}</text>
		...
		<template v-else>
			<view class="flex pt-110 pb15 bg-white dark:bg-main-black">
				<!-- 左侧 -->
				<view class="flex1 plr15 left-flex">
					
					<view class="flex alcenter tc overxy posRelt  ">
						<view :class="['flex1  flex-after  text-xl',{'active text-new-orange':type=='buy'},{'text-white dark:text-999FAA':type!='buy'}]" @click="changeType('buy')">{{$t('trade').buy}}</view>
						<!-- <view class="bor abstrot mt5 bg-white dark:bg-main-black"></view> -->
						<view :class="['flex1  flex-after text-xl',{'active text-new-orange':type=='sell'},{'text-white dark:text-999FAA':type!='sell'}]" @click="changeType('sell')">{{$t('trade').sell}}</view>
					</view>
					<view class="flex alcenter text-lg justify-between py-10 bg-main-black rounded-8 mb-10 px-10"  @click="togglePopup('bottom-share')" >
						<image @click.stop="showTip = true" src="/static/image/warning.png" class="w-40 h-40 mr-10"></image>
						<text class="text-white py-6" data-position="bottom">{{priceType=='limit' ? $t('trade').limit : $t('trade').shi}}</text>
					...
					<uni-popup :show="popType === 'bottom-share'" position="bottom" @hidePopup="togglePopup('')">
						<view class="bottom-content">
							<view class="bottom-content-box tc ">
								<view :class="['bdb_1e ptb10 ft14 bg-white dark:bg-main-black',{'blueColor':priceType=='limit'}]" @click="changePriceType('limit')">{{$t('trade').limit}}</view>
								<view :class="['ptb10 ft14 bg-white dark:bg-main-black',{'blueColor':priceType=='market'}]" @click="changePriceType('market')">{{$t('trade').shi}}</view>
								<view class="pt5 pb10">
									<view class="ptb10 ft14 bg-white dark:bg-main-black" @click="togglePopup('')">{{$t('trade').cancel}}</view>
								</view>
							</view>
						</view>
					</uni-popup>
					<view class="">
						<view class="" v-show="priceType=='limit'">
							<view class="bg-main-black rounded-8 input-pd">
								<view class="text-838B99 text-lg">{{$t('trade.price')}}</view>
								<input type="number" v-model='price' :placeholder="$t('trade.p_price')" class="text-white h40 flex1 ft14" />
						...
						<view class=""  v-show="priceType=='market'">
							<view class="bd_input flex alcenter radius2 bgBlack">
								<input type="text" :value="$t('trade.best')" disabled class="h40 flex1 plr10 text-white" />
							</view>
						</view>
						
						<view class="mt-6 bg-main-black rounded-8 input-pd">
							<view class="input-title">
								<text>{{$t('trade.num')}}</text>
								<text class="fee">{{$t('trade.fee')}}:{{parseFloat(fee)}}%</text>
							</view>
							<input type="number" v-model="number" :placeholder="$t('trade.p_num')" class="h40 flex1 text-white" @input="inputNum" />
						...
						<view class="py-10">
							...
							<view class="flex alcenter around tc tab-box gray7 input-title">
								<view :class="['bg-main-black flex1 radius2',{'bg-k-green text-white bd_green': percent==item&&type=='buy'},{'bg-k-red text-white bd_red': percent==item&&type=='sell'},{'ml10':index>0}]" v-for="(item,index) in percentList" :key='index' @click="changePercent(item)">{{item}}%</view>
							</view>
						</view>
						<view class="py-10">
							...
							<view class="mt10 input-title ft12 flex justify-between">
								...
								<view class="mt-4 text-838B99 dark:text-white font-extrabold">{{tradeNum || '--'}}{{legal_name}}</view>
							</view>
						</view>
				...
				<view class="trade-left flex column">
					...
					<view class="text-black bg-main-black p-10 rounded-8 text-center mt-10 mr-10" @click="toDeal">{{$t('new.deal')}}></view>
			...
			<view class="bg-white dark:bg-main-black">
			<view class="flex alcenter between ptb10  plr15 footer pb-10 border-0 border-solid border-b-2 border-E5E5E5 dark:border-999FAA">
				<view class="flex flexend">
					<text class="text-xl text-new-orange">{{$t('trade.delegate')}}</text>
				...
				<view class="flex alcenter gray7">
					<image  src="/static/image/datamap.png" mode="aspectFit" class="w-40 h-40" @tap="linkKline"></image>
					<!-- <image src="/static/image/all.png" class="h20 wt20 ml5"></image> -->
					<navigator class="ml-20 text-lg text-black bg-main-black dark:text-white py-10 px-40 rounded-8 text-center" url="entrust">{{$t('new.newAll')}}</navigator>
				</view>
			</view>
			<!-- 委托类型切换弹窗 -->
			<uni-popup :show="popType === 'bottom-entrust'" position="bottom" @hidePopup="togglePopup('')">
				<view class="bottom-content">
					<view class="bottom-content-box tc gray7">
					    <view :class="['bdb_1e ptb10 ft14 bg-white dark:bg-main-black',{'blueColor':enType=='in'}]" @click="changeEntrustType('in')">{{$t('trade.buy')}}</view>
						<view :class="['ptb10 ft14 bg-white dark:bg-main-black',{'blueColor':enType=='out'}]" @click="changeEntrustType('out')">{{$t('trade.sell')}}</view>
			            <view class="pt5 pb10">
			            	<view class="ptb10 ft14 bg-white dark:bg-main-black" @click="togglePopup('')">{{$t('trade.cancel')}}</view>
			            </view>
					</view>
		...
		<view class="left-mode">
			<uni-drawer :visible="showLeft" style="background: rgb(16, 32, 48,.5);" mode="left" @close="showLeft = false">
				<view class="flex column ht100 dark:text-white">
					<view class="pt20">
						<view class="ft18 pl15 ptb10">{{$t('trade.bibi')}}</view>
						<view class="flex plr15 bdb_1e bold gray7">
							<view :class="['mr20 pb5 ft14',{'text-new-orange border-0 border-b-4 border-solid border-new-orange':current==index}]" v-for="(item,index) in quotationList" :key="item.id" @click="changeLegal(index,item.code)">{{item.code}}</view>
						</view>
					</view>
					<view class="flex1 overyscroll" v-for="(market,index) in marketList" :key="index" v-if="current==index">
						<view :class="['flex alcenter between ptb15 plr15  bdb_1e',{' bggray':legal_id==item.quote_currency_id && currency_id==item.base_currency_id}]" :key="idx" v-for="(item,idx) in market" v-if="item.open_change==1" @click="changeCurrency(idx)">
							<view class="flex flexend">
								...
								<text class="ft14">/{{item.quote_currency_code}}</text>
							</view>
							<view v-if="item.base_currency_code=='ETHFXX' && !isOnline(item.market_time)" class="w-200">{{$t('trade.soon')}}</view>
							<template v-else>
