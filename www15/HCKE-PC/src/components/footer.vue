  ...
  <div class="bgfooter">
    <div class="w60 mauto ptb40">
      <div class="flex basecolor ft16 jscenter alb tc">
        <div class="w100 flex basecolor ft16 jscenter alb tc white">
			<p class="textitem flex1 white" @click="toPathPDF('prove_dlzm.pdf')">{{$t('home.dlzm')}}</p>
			<p class="textitem flex1 white" @click="toPathPDF('certificate_zczs.pdf')">{{$t('home.zczs')}}</p>
			<p class="textitem flex1 white" @click="toPathPDF('finance_jrpz.pdf')">{{$t('home.jrpz')}}</p>
		</div> 
      ...
      <div class="tc ptb60">
        <p class="ft24 bold">DEMO</p>
        <p class="baselight mt20">©Copyright 2021 DEMO. All rights reserved.</p>
      </div>
    </div>
  </div>
</template>
...
export default {
    ...
    methods:{
		toPath(){
			...
			window.open(`${this.$http._DOMAIN}/wp/MUT-${lang}.pdf`);
		},
		toPathPDF(urls){
			// let path = `../../static/${urls}.pdf`
      let path = `${this.$http._DOMAIN}/wp/${urls}`
		   window.open(path);
		},
      toPath2(){
