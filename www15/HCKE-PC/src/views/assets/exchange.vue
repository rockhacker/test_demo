<template>
  <div class="mymention pb60">
    <div class="flex between baselight2 ptb20 plr20 ft16 bgwhite">
      <span class="basecolor">{{ $t("new.duih") }}</span>
    </div>
    <div class="mt20 plr30">
      <div class="flex alcenter mb20 " :style="{'flex-direction':show==1?'row':'row-reverse'}">
        <div class="flex alcenter flex1" ref="left">
          <el-select
            v-model="currencyCode"
            class="flex1 selectBor w100"
        ...
        <div class="flex alcenter flex1" ref="right">
          <el-select
      ...
      <div class="flex alcenter mb40 duih mt40">
        <span class="basecolor">{{ $t("new.dhs") }}:</span>
        <input
          type="text"
          v-model="num"
          class="ht40 lh40 plr20 borGray flex1 baselight2"
        />
        <p class="quanb basecolor" @click="setAll">{{ $t("new.quanb") }}</p>
      </div>
      <div class="text-flex">
        <div class="top child basecolor">
          <span>{{ $t("new.dangq") }}</span>
          <span>{{ $t("new.keyong") }}</span>
          <span>{{ $t("new.ked") }}</span>
        </div>
        <div class="bottom child basecolor">
          <span>{{ rate }}</span>
      ...
      <div
        class=" btn mt20 radius2 tc ht48 lh48 w100 white bg-main"
        @click="sub"
      >
        {{ $t("new.duih") }}
      </div>
