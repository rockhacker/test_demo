<template>
  <div class="baselight">
    <div class="flex between baselight2 ptb20 plr20 ft16 bgwhite">
      <span class="basecolor">{{$t('assets.currencyTransfer')}}</span>
      <router-link :to="{name:'transferLog',query:{currency_id:currency_id,currency_code:curcoin}}" class="basecolor">{{$t('assets.transferRecord')}}</router-link>
    </div>
    <div class="plr20 pt20 pb50 cointransfer">
      <div class="flex alcenter">
        <span class="inblock wt80 basecolor">{{$t('assets.transferCurrency')}}:</span>
      ...
      <div class="flex mt30 alcenter">
        <div class="wt80 basecolor">{{$t('assets.transferAccount')}}:</div>
        <div class="flex alcenter flex1">
          <div class="flex alcenter flex1">
            <div class="pr10 basecolor">{{$t('assets.from')}}</div>
            <div class="flex alcenter flex1" style="position: relative;">
              <el-select v-model="fromtype" class="myselect w100" @change="changetransfer1" placeholder="">
                <el-option
          ...
          <div class="pl10 flex alcenter flex1">
            <div class="pr10 basecolor">{{$t('assets.reach')}}</div>
            <div class="flex alcenter flex1">
              <el-select v-model="totype" class="myselect w100" @change="changetransfer2" placeholder="">
                <el-option
                  v-for="item in accoutlist"
                  :key="item.account_code"
                  :label="item.name"
      ...
      <div class="mt10 basecolor" style="margin-left:80px">{{$t('trade.balance')}}: {{balance||'0.00'}}</div>
      <div class="mt10 flex alcenter">
        <span class="inblock wt80 basecolor">{{$t('assets.transfernum')}}:</span>
		<div class="ht40 lh40 radius2 baselight2 w100" style="border:1px solid #80808085">
			<input type="number" class="h100 pl20 baselight2 w100" v-model="number" />
		</div>

			<!-- <span class="plr10" @click="number=balance">{{$t('assets.max')}}</span> -->
      </div>
      <div class="flex mt20">
        <div class="bg-main lh48 ht48 tc white ft16 radius4 w100" @click="transferConfirm">{{$t('login.e_confrim2')}}</div>
      </div>
    </div>
  </div>
</template>
