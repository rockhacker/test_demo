<template>
    <div>
        <!-- <div class="w100 mauto baselight2 mt20 bgwhite ptb10 flex alcenter between ft16 plr20 radius2">
        ...
        <div>
            <div class="plr20 radius2">
                <div class="ft16 ptb20 flex alcenter between">
                    <div class="flex gray6">
                        <span class="bold mr20 pointer" @click="changeType('BUY')" :class="{'text-main':selectType=='BUY'}">{{$t('legal.buy')}}</span>
                        <span class="ht20 bdrfopt25" ></span>
                        <span class="bold ml20 pointer" @click="changeType('SELL')" :class="{'text-main':selectType=='SELL'}">{{$t('legal.sell')}}</span>
                    </div>
                    
                </div>
                <div class="flex alcenter ft16 bold  baselight2 ptb30" style="border-bottom:1px solid #eee;">
                    <div class="ptb5 plr10 pointer radius4" style="border:1px solid white;text-align: center;" v-for="(item,i) in coinlist" :key="i" :class="[i==coinIndex?'text-main border-green':'gray6']" @click="changeCoin(i)">{{item.code}}</div>
                </div>
            </div>
            <div class="plr20 baselight2">
                ...
                <div class="flex alcenter ptb10" v-for="(item,i) in orderlist" :key='i'>
                    ...
                    <div class="flex1 gray6"><span>{{item.remain_qty}}</span><span class="pl5">{{coinlist[coinIndex].code}}</span></div>
                    <div class="flex1 gray6">{{(item.min_number*item.price) | filterDecimals}}-{{(item.max_number*item.price) | filterDecimals}} <span class="pl5">{{item.area_currency}}</span></div>
                    <div class="flex1 gray6">{{item.price | filterDecimals}}<span class="pl5">{{item.area_currency}}</span></div>
                    <div class="flex1 tc flex alcenter gray6">
                        <p class="mr5 ptb2 gray6 " v-for="(itm,inx) in item.payways" :key="inx">
                        <img :src="API + itm.logo" width="20" alt="">
                            <!-- <span :class="[{'blue':itm.code=='alipay','green':itm.code=='wxpay','yellow':itm.code=='bank'}]">{{itm.name}}</span> -->
                        </p>
                    </div>
                    <div class="wt50 tc white radius2 ptb5 pointer gray6" :class="{'bg-main':selectType=='BUY','bg-orange':selectType=='SELL'}" @click="getinfo(item)">{{selectType=='BUY'?$t('legal.buy'):$t('legal.sell')}}</div>
                </div>
            </div>
                    ...
                    <div>
                        <div class=" ht50 lh50 flex alcenter posRelt mb15">
                            ...
                            <div class="abstort rt0 btm0 ht50 flex alcenter">
                                ...
                                <div class="plr10 pointer text-main" @click="dealAll">{{$t('legal.all')}}</div>
                            </div>
                        </div> 
                        ...
                        <p class="">{{$t('legal.allmoney')}}：{{totalPrice | filterDecimals}}</p>
                    </div>
                </div>
                <span slot="footer" class="dialog-footer flex">
                    <el-button class="flex1 mr30 radius4" @click="dialogVisible=false"><span>{{second}}</span> s{{$t('legal.cannel')}}</el-button>
                    <el-button type="primary" @click="confirm" class="flex1 radius4 white" >{{$t('legal.e_confrim2')}}</el-button>
                </span>
            </el-dialog>
            <!-- 交易密码 -->
            <el-dialog
...
export default {
    ...
    methods:{
        ...
        changeCoin(i){
            ...
            this.$store.commit('setCurrencyId',this.currency_id)
            this.currentPage=1;
            this.getlist();
        },
        handleClose(done) {
