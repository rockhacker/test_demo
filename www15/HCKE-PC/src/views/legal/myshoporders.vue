<template>
  <div class="radius2 shop">
    <!-- 头部商家信息部分 -->
    <div class="ptb30 plr30 mb20 bgwhite">
      <div class="flex between alcenter">
        <div class="flex ">
          <div class="flex alcenter ft14 mr30">
            <span
              class="wt50 ht50 radius50p white tc flex alcenter jscenter bgblue"
            >{{personinfo.name.substr(0,1)}}</span>
            <div class="pl10">
              <span class="basecolor">{{personinfo.name}}</span>
               <div class="gray6">
                <span>{{$t('store.regtime')}}</span>
                <span>{{personinfo.created_at}}</span>
              </div>
            </div>
          </div>
        </div>
        <div class="flex flex1 jsend">
          ...
          <div
            class="wt60 tc white radius2 ht30 lh30 pointer bg-main"
    ...
    <!-- 出售求购订单部分 -->
    <div class="radius2 bgwhite">
      <div class="ft16 flex alcenter between" style="background:#f6f7f9;height: 70px;">
        <div class="flex baselight2" style="height:100%">
          <div
            class="bold pointer gray6 plr20"
            style="line-height:70px"
            @click="changeType('BUY')"
            :class="{'green bgwhite':selectType=='BUY'}"
          >{{$t('store.mybuy')}}</div>
          ...
          <div
            ...
            :class="{'green bgwhite':selectType=='SELL'}"
          >{{$t('store.mysell')}}</div>
        </div>
      </div>
      <div class="ptb20 pl20 flex alcenter ft16 bold baselight2">
        <div
          class="plr10 ptb5 pointer bd2px radius4"
          v-for="(item,i) in statusList"
          :key="i"
          :class="[i==coinIndex?'activeType green':'gray6 defaultType']"
          @click="changeStatus(i,item.status)"
        >{{item.name}}</div>
      </div>
    </div>
    <div class="plr20 bgwhite" style="border:0px solid #eeeeee;border-top-width:1px">
      ...
      <div class="flex alcenter ptb10 gray6" v-for="(item,i) in orderlist" :key="i">
        ...
        <div class="flex15 white flex alcenter jsend">
          <div
            class="mr10 pointer plr10 radius2 ptb5 gray6"
          ...
          >{{$t('store.back')}}</div>
          <div class="pointer  plr10 radius2 ptb5 bg-main white" @click="goOrder(item.id)">{{$t('store.lookorder')}}</div>
        </div>
      </div>
    </div>
    <div class="tc ptb40 bgwhite" v-if="orderlist.length==0">
    ...
    <el-dialog :title="$t('store.fabu')" :visible.sync="dialogVisiblefabu" width="30%" :before-close="handleClose">
      <div class="ht40 lh40 flex alcenter mb15">
        ...
        <el-select class="flex1 h100 bde7" v-model="currency_id" :placeholder="$t('store.selectCurrency')">
          <el-option
      ...
      <div class="ht40 lh40 flex alcenter mb15">
        ...
        <el-select class="flex1 h100 bde7" v-model="area_id" :placeholder="$t('store.p_coin')">
          <el-option
      ...
      <div class="ht40 lh40 flex alcenter mb15">
        ...
        <div>
          <el-radio v-model="auto_start" label="1">{{$t('legal.yes1')}}</el-radio>
          <el-radio v-model="auto_start" label="0">{{$t('legal.no1')}}</el-radio>
        </div>
      </div>
      <div class="ht40 lh40 flex alcenter mb15">
        ...
        <input
          type="number"
          class="flex1 h100 bde7 pl20 radius2"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmfabu" class="radius4 white">{{$t('legal.e_confrim2')}}</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    confirmfabu() {
      ...
      if (
        (Number(this.max_number) || Number(this.min_number)) >
