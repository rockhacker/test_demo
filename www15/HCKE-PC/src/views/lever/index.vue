<template>
  <div class="w100 baselight">
    <div class="flex mt5 vh0" style="min-height: 800px;">
      <!-- 左侧行情 -->
      <div class="wt240 bd mr5 ptb10 radius2 h0 bgwhite">
        ...
        <div class="flex alcenter mt10 mb10 gray6">
        ...
        <div class="overyscroll">
          <div v-if="list.length > 0 && list[currentIndex]">
            ...
            >
              <div class="flex1 pl10 gray6">
                {{ itm.base_currency_code }}
              </div>
              <div class="flex1 tr  gray6" v-if="itm.currency_quotation">
                <!-- {{ itm.base_currency_code=='MUT'?!isOnline(itm.market_time)?$t('trade.soon'):itm.currency_quotation.close:itm.currency_quotation.close }} -->
              ...
              <div
                ...
                v-if="(itm.base_currency_code=='MUT'?isOnline(itm.market_time):true) && itm.currency_quotation"
              >
    ...
    <footerComponet class="mt20"></footerComponet>
  </div>
</template>
<script>
...
export default {
  ...
  computed:{
	 ...
	 soon(){
		return this.currency_matchInfo.base_currency_code=='MUT' && !this.isOnline(this.currency_matchInfo.market_time)
	 }
  },
  created() {
          ...
          data2.forEach((item, index) => {
            datas.forEach((itm, inx) => {
              if (
                ...
                if (that.types == 5) {
                  const time = that.historyData.currentPosition[inx].time 
                  // item.time =  that.historyData.currentPosition[idx].time
                  that.historyData.currentPosition.splice(inx, 1, item);
                  that.historyData.currentPosition[inx].time = time
                } else {
