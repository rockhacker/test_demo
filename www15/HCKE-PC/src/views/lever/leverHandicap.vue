<template>
    <div class="flex column">
        <div class="flex">
             <div class="flex1 plr10 ht40 lh40 text-main ft16 bgwhite">{{$t('lever.pankou')}}</div>
             <!-- <div class="bgheader bdbheader flex1 plr10 ht40 lh40 baselight2 ft16 ml5">全站成交 </div> -->
        </div>
        <div class="flex1 flex">
            <div class="flex column w100 flex1 bgwhite">
                <div class="plr10 ptb10 flex1 flex column w100">
                    <div class="flex alcenter gray6">
                ...
                <div class="mt10">
                    <!-- 没有socket推送的时候 -->
                    <div  class="asks">
                        <div class="ptb5 flex alcenter pointer"  v-for='(item,i) in outlist' :key="i" :style="'background-size:'+parseInt((item.amount/sellMax)*100)+'% 100%'" style="transition:0.8s" @click="changeSell(item.price)">
                            <span class="flex1 red">{{$t('trade.sell1')}}{{outlist.length-i}}</span>
                            <span class="flex1 tc gray6">{{item.price}}</span>
                            <span class="flex1 tr gray6">{{item.amount}}</span>
                        </div>  
                    </div>
                ...
                <div class="ptb10" style="border:0 solid #eee;border-top-width: 1px;border-bottom-width: 1px;">
                ...
                <div class="">
                    <div class="bids">
                        <div class="ptb5 flex alcenter pointer"  v-for='(item,i) in inlist' :key="i" :style="'background-size:'+parseInt((item.amount/buyMax)*100)+'% 100%'" style="transition:0.8s"  @click="changeBuy(item.price)">
