<template>
  <div class="bgwhite plr10 ptb10">
    <div class="tc flex mb20 ft14">
      <div class="flex1 hidden-xs ptb20 radius10 white" style="background:#0d202e;">
    ...
    <div class="mt20">
      <div class="flex alcenter ft14 ptb10 pr20">
        <div class="bg-main" style="width:4px;height: 16px;"></div>
        <!-- <img src="../../assets/images/tick.png" alt=""> -->
        <span class="pl10 gray6">{{$t('second.TransactionMode')}}</span>
      </div>
      <ul class="flex wraps times-list tc mt10">
        <li class="mb10 w30 ptb10 mr10 posRelt pointer gray6 ft14 radius4" style="background:#f6f7f9;" v-for="(item,i) in wallet_list" :key="i" @click="changeModel(i)">
          <span>{{item.code}}</span>
          <!-- <img src="../../assets/images/pitch_on.png" alt="" v-if="modelIndex==i" class="abstort btm0 rt0"> -->
          <div v-if="modelIndex==i" class="abstort btm0 rt0 acive-triangle-bottomright"></div>
          <i v-if="modelIndex==i" class="el-icon-check abstort btm0 rt0 white"></i>
        </li>
      </ul>
      <div class="flex alcenter ft14 ptb10">
        <!-- <img src="../../assets/images/tick.png" alt=""> -->
        <div class="bg-main" style="width:4px;height: 16px;"></div>
        <span class="pl10 gray6">{{$t('second.OpenQuantity')}}</span>
      </div>
    ...
    <div class="name">{{ parseFloat(item.number) }}</div>
      ...
      <div class="flex alcenter ft14 ptb10">
        <!-- <img src="../../assets/images/tick.png" alt=""> -->
        <div class="bg-main" style="width:4px;height: 16px;"></div>
        <span class="pl10 gray6">{{$t('second.OpenTime')}}</span>
      </div>
      <ul class="flex wraps times-list tc mt10" v-if="second_List">
        <li class="mb10 w30 ptb10 mr10 bgwhite posRelt pointer gray6 ft14 radius4" style="background:#f6f7f9;" v-for="(item,i) in second_List" :key="i" @click="changeTimes(i)">
          <span>{{item.seconds}}{{$t('new.miao')}}</span>
          <!-- <img src="../../assets/images/pitch_on.png" alt="" v-if="timeIndex==i" class="abstort btm0 rt0"> -->
          <div v-if="timeIndex==i" class="abstort btm0 rt0 acive-triangle-bottomright"></div>
          <i v-if="timeIndex==i" class="el-icon-check abstort btm0 rt0 white"></i>
        </li>
      </ul>
      <div class="ptb20 flex hidden-xs ded5af4" style="background:#fbf8ed;border:1px solid #ded5af;">
        <div class="flex flex1 alcenter jscenter">
          <!-- <img src="../../assets/images/miaoIcon.png" alt=""> -->
          <span class="gray6 ft18">{{$t('second.profitRate')}}</span>
        </div>
      ...
      <div class="mt20 nmb_mt20" v-else>
        <div class="bggreen tc ptb10 ft16 white radius40 flex alcenter jscenter pointer mb_btn mr20" style="width:100%" @click="submits(1)" >
          <span class="white mr10">{{$t('second.buyUp')}}</span> 
          <!-- <img src="../../assets/images/miaoIcon_18.png" alt=""> -->
          <div class="radius50p bgwhite" style="width:24px;height:24px;line-height: 28px;">
            <i class="el-icon-top text-main"></i>
          </div>
        </div>
        <div class="bgred tc ptb10 ft16 mt20 white radius40 flex alcenter jscenter pointer mb_btn" style="width:100%" @click="submits(2)" >
          <span class="white mr10">{{$t('second.buyDown')}}</span> 
    ...
    <el-dialog :title="submitType==1?$t('second.buyUp'):$t('second.buyDown')" :visible.sync="orderShow" :width="wscreenWithdata" :before-close="handleClose">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" size="small" @click="orderSubmit" class="radius4 white">{{$t('lever.e_confrim2')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<style lang="scss">
.acive-triangle-bottomright {
    width: 0;
    height: 0;
    border-bottom: 19px solid #057967;
