<template>
  <div>
    <div class="flex ht50 lh50 tc ft14">
      <div class="plr20 pointer" :class="status==1?'bdbblue blue':'gray6'" @click="changeType(1)">{{$t('lever.dealing')}}</div>
      <div class="plr20 ml10 pointer" :class="status==3?'bdbblue blue':'gray6'" @click="changeType(3)">{{$t('lever.hasping')}}</div>
    </div>
    <div class="plr15 pb10 h180 overyscroll">
      <ul>
      ...
      <ul>
        <li v-if="historyData.currentPosition.length > 0" v-for="(item,index) in historyData.currentPosition":key="index" class="flex between ptb5 gray6">
          ...
          <span class="flex1 tr" v-if="status == 3" :class="item.fact_profits <0?'redColor':'greenColor'">{{item.fact_profits<0?'': '+'}}{{item.fact_profits || '0' |tofixedFour}}</span>
        </li>
        ...
        <li v-if="historyData.currentPositionTotal ==0">
          <div class="tc ptb40">
            ...
            <p class="gray6">{{$t('assets.noData')}}</p>
          </div>
        </li>
      </ul>
    </div>
