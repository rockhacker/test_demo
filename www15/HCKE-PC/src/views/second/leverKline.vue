<template>
  <div>
    <div class="flex alcenter nmb_ptb10">
      ...
      <div class="flex1 tc hidden-xs">
        <div class="flex alcenter ft16">
          <span class="flex1 gray6"
            >${{ currency_matchInfo.currency_quotation.close || "0.00" }}</span
          >
          <p class="flex1 flex alcenter jscenter">
            <img
              src="../../assets/images/red.png"
              class="wt25"
              v-show="currency_matchInfo.currency_quotation.change < 0"
              alt=""
            />
            <img
            ...
            <span
              class="ml10"
              :class="[
                currency_matchInfo.currency_quotation.change < 0
                  ? 'red'
                  : 'green',
              ]"
              >{{ currency_matchInfo.currency_quotation.change < 0 ? "" : "+"
              }}{{
                currency_matchInfo.currency_quotation.change ||
                "0.00" | filterDecimals
              }}%</span
            >
          </p>
          <span class="flex1 gray6"
            >${{ currency_matchInfo.currency_quotation.high || "0.00" }}</span
          >
          <span class="flex1 gray6"
            >${{ currency_matchInfo.currency_quotation.low || "0.00" }}</span
          >
          <span class="flex1 gray6">{{
            currency_matchInfo.currency_quotation.volume ||
            "0.00" | filterDecimals
          }}</span>
        </div>
        <div class="flex alcenter mt5 gray6">
          <span class="flex1">{{ $t("second.newsPrice") }}</span>
          <span class="flex1">{{ $t("second.hoursriseandfall") }}</span>
          <span class="flex1">{{ $t("second.hoursHighest") }}</span>
          <span class="flex1">{{ $t("second.hoursminimum") }}</span>
          <span class="flex1">{{ $t("second.hoursamount") }}</span>
        </div>
      </div>
    </div>
    <div id="tv_chart_container" class="nmb_mt20"></div>
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    ...
    this.lang= lang || 'en';
    localStorage.setItem("secondPeriod", "1hour"); //默认为1分钟;
    this.createWidget();
  },
  destroyed(){
    this.removeWidget();
...
//     next();
//     let that = this;
//     this.unbindSocket(that.currency_matchInfo.symbol);
//   },
  beforeDestroy() {},
  methods: {
    adjustScale() {
    ...
    connect(real) {
      ...
      that.$socket.connected(socket => {
        ...
        socket.on("KLINE", msg => {
          var klineType = localStorage.getItem("secondPeriod");
            if(klineType=='1hour'){
              klineType = '60min'
          }
          if (
            ...
            this.$nextTick(function () {
                ...
                widget.onChartReady(function() { //图表方法
                    ...
                    if(document.body.clientWidth>760){
                        widget.chart().createStudy("MACD", false, true, [12, 26]);
                    }
                    let buttonArr = [
                        {
                            value: "1min",
                    ...
                    let handleClick = (e, value,type) => {
                        ...
                        localStorage.setItem('secondPeriod',type)
