<template>
    <div>
        <div class="bgheader bdbheader flex alcenter plr10 ht40">
            ...
            <div class="">
                <div class="ft16" :class="[currency_matchInfo.change>=0?'green':'red']">{{currency_matchInfo.volume}}</div>
                <!-- <div class="ft12">≈{{currency_matchInfo.volume}}CNY</div> -->
            </div>
            <div class="flex1 ft12 tc">
                <div class="flex alcenter">
                    <span class="flex1">涨幅</span>
                    <span class="flex1">最高价</span>
                    <span class="flex1">最低价</span>
                    <span class="flex1">24H量</span>
                </div>
                <div class="flex alcenter">
                    <span class="flex1" :class="[currency_matchInfo.change>=0?'green':'red']">{{currency_matchInfo.change}}%</span>
                    <span class="flex1 baselight2">{{currency_matchInfo.high}}</span>
                    <span class="flex1 baselight2">{{currency_matchInfo.low}}</span>
                    <span class="flex1 baselight2">{{currency_matchInfo.volume}}</span>
                </div>
            </div>
        </div>
...
<script>
export default {
    props:['symbol','mathid','currency_matchInfo'],
    data() {
        return {
            widget: null,
            symbolInfo: null,
    ...
    methods: {
        connect(real) { //封装推送数据
            var that=this;
            that.$utils.socket.onopen([{type:'sub',name:'DAY_MARKET'}])
            // that.$utils.socket.onopen([{type: "sub", params: "DAY_MARKET"}]);
            that.$utils.socket.onmessage=res=>{
                console.log(res);
            }
