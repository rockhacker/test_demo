<template>
    <div class="flex h100">
        <div class="flex2 flex column mr5">
            <!-- 行情 -->
            <div class="flex2">
                <div class="flex alcenter plr10 ht40">
                    <!-- <div class="baselight2 ft16 bold mr20">{{symbol}}</div> -->
                    <el-popover
                        placement="top-start"
                        popper-class="bgwhite"
                        width="240"
                        trigger="hover">
                        <div>
                    ...
                    <div v-if="tradeList.length>0 && tradeList[currentIndex]">
                        <div class="flex alcenter ptb5 markethover pointer" :class="{'bgshadow':itm.symbol==symbol}"  v-for="(itm,j) in tradeList[currentIndex].matches" v-if="itm.open_change==1"  :key="j" @click="$emit('changeSymbol',j)">
							...
							<div v-if="itm.base_currency_code=='MUT' && !isOnline(itm.market_time)" class="mlr10">{{$t('trade.soon')}}</div>
							<template v-else >
				...
				<div id="tv_chart_container" style="width:100%;height: 90%" v-show="!soon"></div>
            </div>
        ...
        <div class="flex column overy" style="width:300px;height:100%">
            <div class="flex">
                <div class=" flex alcenter bold  flex1 pt7 pl5">
            ...
            <div class="flex1 flex">
                <div class="flex column w100 flex1 " v-show="optionIndex==0">
                    <div class="ptb10 flex1 flex column w100">
                        ...
                        <!-- 买盘 -->
                        <div class=" mt10 flex1 flex column bids">
			...
			<!-- 下单 -->
			<div class="" style="width:290px;height:100%">
                <div class="flex alcenter plr10  baselight2 mb10" >
                ...
                <div class="flex mlr10 pointer" style="border-radius:2px;overflow: hidden;">
                    <!-- <el-button-group class="flex1">
                        ...
                        <div class="flex1 posRelt bggreen" @click="markType=0" style="height:34px;text-align: center;line-height: 34px;color:#fff;font-size:16px">{{$t('trade.buy')}}
                            <div class="abstort top0 rt0 btm0" :style="{'border-top': `16px solid `+(theme=='dark'&& false?'#0B0E11':'#fff'),'border-left': `16px solid transparent`, 'border-bottom':` 18px solid `+(theme=='dark'&& false?'#0B0E11':'#fff')}">
                        ...
                        <div class="flex1 posRelt bgred" @click="markType=1" style="height:34px;text-align: center;line-height: 34px;color:#fff;font-size:16px">{{$t('trade.sell')}}
                            <div class="abstort top0 lf0 btm0" :style="{'border-top': `16px solid `+(theme=='dark'&& false?'#0B0E11':'#fff'),'border-right': `16px solid transparent`, 'border-bottom':` 18px solid `+(theme=='dark'&& false?'#0B0E11':'#fff')}">
                ...
                <div class=" flex  ft12">
                    ...
                    <div class="flex1 plr10 mt10 bdr38" v-show="markType==0">
                        ...
                        <div class="flex alcenter mb10 posRelt">
                            ...
                            <input type="number" v-model="buyprice" class="ht30 bdinput lh30 plr5 radius2 ml10 flex1 nwhite white" v-show="index==0" @input='buypriceInput' @keyup='buypriceInputKeyDown($event)'>
                            <input type="text" :placeholder="$t('trade.goodbuy')" class="trade ht30 bginput lh30 plr5 radius2 ml10 flex1 nwhite white" v-show="index==1" disabled>
                            <span class="abstort ht30 lh30 plr5 rt0 btm0">{{legalName}}</span>
                        </div>
                        <div class="flex alcenter mb10 posRelt">
                            <span class="w15" >{{$t('trade.buyNumber')}}</span>
                            <input type="number" v-model="buynumber" class="ht30 bdinput lh30 pl5 radius2 ml10 flex1 nwhite white"  @input='buynumberInput'>
                        ...
                        <div class="bggreen tc ht30 lh30 white radius2 pointer" style="height: 34px;line-height: 34px;font-size: 16px;" @click="buyin" v-else>{{$t('trade.buy')}}</div>
                    </div>
                    <!-- 卖盘 -->
                    <div class="flex1 plr10 mt10" v-show="markType==1">
                        ...
                        <div class="flex alcenter mb10 posRelt">
                            ...
                            <input type="number" v-model="sellprice" class="ht30 bdinput lh30 plr5 radius2 ml10 flex1 nwhite white" v-show="index==0">
                            <input type="text" :placeholder="$t('trade.goodsell')" class="trade ht30 bginput lh30 plr5 radius2 ml10 flex1 nwhite white" v-show="index==1" disabled>
                            <span class="abstort ht30 lh30 plr5 rt0 btm0">{{legalName}}</span>
                        </div>
                        <div class="flex alcenter mb10 posRelt">
                            <span class="w15">{{$t('trade.sellNumber')}}</span>
                            <input type="number" v-model="sellnumber" class="ht30 bdinput lh30 pl5 radius2 ml10 flex1 nwhite white" @input='sellnumberInput'>
    ...
    //   更新下单
        ...
        createWidget() {
            ...
            this.$nextTick(function () {
                ...
                let cssfile = this.loctheme=='dark' && false?'bundles/new_dark.css':'bundles/new.css';
