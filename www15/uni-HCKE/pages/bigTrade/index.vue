				...
				<view class="flex justify-between p-20 bg-white dark:bg-gray-600 dark:bg-opacity-30">
					<view class="w-3_5">
						...
						<view class="mt-20 text-lg flex items-center justify-between">
							...
							<navigator url="/pages/bigTrade/exchange" class="mt-8 px-10 bg-primary rounded-8 py-4 mr-10">{{$t('new.duih')}}</navigator>
						</view>
					...
					<view class="mt-40">
						...
						<view class="" v-if="orderList.length > 0">
							<view v-for="(item, index) in orderList" :key="index" class="flex justify-between items-center mt-20">
								<view class="flex-1 text-left">{{ item.OrderType ==1 ? $t('trade.rise'): $t('trade.fall') }}</view>
								...
								<view class="flex-2 text-center">{{((parseFloat(item.FactProfits)/parseFloat(item.CautionMoney))*100).toFixed(2)}}%</view>
								<view class="flex-2 text-center">{{ item.FactProfits }}</view>
...
export default {
	...
	methods: {
		...
		linkKline() {
			...
			const url = `/h5/hybrid/html/bigTrade.html?symbol=${symbols}&currency_match_id=${that.currentMatch.Id}`
