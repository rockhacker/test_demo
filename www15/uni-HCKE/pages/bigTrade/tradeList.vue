<template>
	<view :class="theme">
		<view class="bg-white dark:bg-floor min-h-screen text-gray-700 dark:text-gray-100">
			...
			<view class="px-40">
				<view @click="showDetail(item)" v-for="(item, index) in orderList" :key="index" class=" border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 py-20">
					...
					<view class="flex flex-col">
						<view class="flex mb-20">
							<view class="flex-1">
								...
								<view class="mt-10 text-lg">{{ item.OriginPrice || '0.0000' | toFixed4 }}</view>
							</view>
						...
						<view class="flex mb-20">
							<view class="flex-1">
								...
								<view class="text-gray-500">{{ $t('new.avg_price') }}</view>
						...
						<view class="flex mb-20">
							...
							<view class="" v-if="item.CompleteTime">
								...
								<view class="mt-10">{{ item.CompleteTimeHandler }}</view>
							</view>
						</view>
					</view>
					<!-- 按钮 -->
			...
			<uni-popup v-if="orderInfo.ForexTradeLists" :show="showOrderDetail" :btnShow="false" :msg="orderInfo.ForexTradeLists.Code"  @hidePopup="showOrderDetail=false">
				<view class="text-52 py-16" :class="orderInfo.FactProfits < 0 ? 'text-danger': 'text-success'">{{ orderInfo.FactProfits >=0? '+':'' }} {{ parseFloat(orderInfo.FactProfits) }}</view>
				<view class="py-60 w-full mt-30">
					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{$t('new.jyfx')}}</view>
						<view class="text-base">{{orderInfo.OrderType==1?$t('trade.buy'):$t('trade.sell')}}</view>
					</view>

					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{$t('new.cont_vol')}}</view>
						<view class="text-base">{{orderInfo.Quantity}}</view>
					</view>

					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{$t('new.leveage_ratio')}}</view>
						<view class="text-base">{{orderInfo.Multiple}}</view>
					</view>

					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{ $t('new.opprice') }}</view>
						<view class="text-base">{{ orderInfo.OriginPrice || '0.0000' | toFixed4 }}</view>
					</view>

					<view class="flex justify-between w-full px-30 mb-20">
						<!-- <view class="text-base">{{$t('new.closed_price')}}</view> -->
						<view class="text-base">{{$t('new.avg_price')}}</view>
						<view class="text-base">{{orderInfo.UpdatePrice | toFixed4}}</view>
					</view>

					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{$t('new.profit_amount')}}</view>
						<view class="text-base" :class="orderInfo.FactProfits < 0 ? 'text-danger' : 'text-success'">{{orderInfo.FactProfits || '0.0000' | toFixed4}}</view>
					</view>

					<!-- <view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{ $t('trade.price_cur') }}</view>
						<view class="text-base">{{ orderInfo.UpdatePrice || '0.0000' | toFixed4 }}</view>
					</view> -->

					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{ $t('bigTrade.profitRate') }}</view>
						<view class="text-base" :class="orderInfo.FactProfits < 0 ? 'text-danger' : 'text-success'">
							{{((parseFloat(orderInfo.FactProfits)/parseFloat(orderInfo.CautionMoney))*100).toFixed(2)}}%
						</view>
					</view>

					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{ $t('trade.time_start') }}</view>
						<view class="text-base">{{ orderInfo.CreateTimeHandler }}</view>
					</view>
					<view class="flex justify-between w-full px-30 mb-20">
						<view class="text-base">{{ $t('trade.time_end') }}</view>
						<view class="text-base">{{ orderInfo.CompleteTimeHandler }}</view>
					</view>
