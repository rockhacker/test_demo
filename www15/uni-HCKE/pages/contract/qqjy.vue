<template>
  <view class="pb-200">
	  <view>
		<view class="header ww mb-10 mt-4">
		  <view class="item mr20 blue4 rounded-8":class="item.id == active ? 'active' : ''" v-for="(item, index) in list" :key="index" @click="setActive(item)">
				{{ item.symbol }}
		  </view>
		</view>
		...
		<view class="cont mt-20">
		  ...
		  <view class="pb-16" style="background: linear-gradient(90deg, #982F1E, #711E43, #400083, #550C68);">
		 	<view class="gress ww">
			<view class="text"
		  ...
		  <view class="ww flex between alcenter" v-if="working.open_price && price">
			<view>
			  ...
			  >
			  <view class="mt-10">{{$t('trade.price_cur')}} : <text class="price" :class="price>working.open_price?'zhang':'die'"> {{price.toFixed(4)}}</text></view>
			</view>
		 ...
		 <view class="number ww">
		  ...
		  <view class="number ww">
			<view class="items" :class="amount.id == item.id ? 'active' : ''" v-for="(item, index) in amountList" :key="index" @click="setBala(item)">
