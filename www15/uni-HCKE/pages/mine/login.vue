<template>
  <view :class="theme">
	  <view class="bg-white dark:bg-floor text-black dark:text-white" style="min-height: 100vh;">
		  <view class="p-40 flex justify-between">
			<!-- <image src="../../static/image/wewe.png" mode="widthFix" class="w-100 h-94"></image> -->
		...
		<view class="p-40">
			...
			<view class="py-40">
				...
				<view class="mb-20">
				  <view class="flex items-center border-0 border-b-2 border-solid border-gray-200 dark:border-gray-700">
					<input v-if="showPassword" type="text"  v-model="password" @input="passwordInput" class="h-100 flex-1" :placeholder="$t('login').p_pwd" />
					<input v-else type="password"  v-model="password" @input="passwordInput" class="h-100 flex-1" :placeholder="$t('login').p_pwd" />
					<image :src="`/static/image/${theme=='dark'?'password.png':'password_gray.png'}`" @click="showPassword=!showPassword" class="w-30 h-30 mx-20"></image>
				  </view>
				...
				<view class="my-60 bg-primary rounded-10 p-20 text-white text-lg text-center" @tap="login" hover-class="opacity-50">{{ $t("login").login }}</view>
				<view class="my-60 bg-primary rounded-10 p-20 text-white text-lg text-center mt-30" @tap="toWallet" hover-class="opacity-50">{{ $t("wallet.login") }}</view>
			</view>
