<template>
	<view class="p-20" :class="theme">
		<view class="min-h-screen">
			<view class="flex items-center justify-between bg-my-green rounded-8 mb-20 py-30 pl-30 pr-20" v-for="(item,index) in coinList" :key="index" @click="toUrl(item)">
				 <view class="flex items-center">
					<image :src="item.currency.logo" class="w-60 h-60"></image>
					<view class="ml-20 text-lg">{{item.currency.code}} {{$t('assets.mention')}}</view>
				 </view>
				<image src="../../static/image/arrrowr.png" alt="" class="w-16 h-24"></image>
			</view>
	  </view>
	</view>
</template>
