<template>
	<view :class="theme">
		...
		<view class="ptb10 w100 tc flex alcenter between fixed bg-primary head pr10">
		...
		<view class="plr15 pt50">
			<view class="flex alcenter mt10 plr15 h40 bg-my-green rounded-10 between" @tap="actionSheetTap">
			...
			<input type="text" value="" class="h40 plr20 bg-my-green rounded-10 w100" :placeholder="$t('feed.p_title')" v-model="title" />
			<view class="ptb10 ft13 mt15">{{$t('feed.content')}}</view>
			<view class="ptb20 bg-my-green rounded-10">
			...
			<view class="mt-100 mb-60 bg-primary rounded-10 p-20 text-white text-lg text-center" @click="confirm" hover-class="opacity-50">{{$t('login.e_confrim')}}</view>
		</view>
	</view>
</template>
