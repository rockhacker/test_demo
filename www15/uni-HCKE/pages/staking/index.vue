<template>
	<view :class="theme">
		<view class="text-black dark:text-white dark:bg-gray-600 dark:bg-opacity-30 w-full h-screen">
			<uni-nav-bar fixed backIcon>
				<view class="flex-1 p-30 bg-white dark:bg-floor text-2xl">
					{{$t('staking.kuanjishangcheng')}}
				</view>
				<slot name="right">
					<view class="w-100"></view>
					<navigator url="/pages/staking/orders" 
						class="px-10 py-10 border-2 rounded-10 absolute top-24 right-20 z-10 flex items-center" >
						<text class="pr-10 text-lg">{{ $t('new.sgjl') }}</text>
			...
			<scroll-view scroll-y class="w-full" style="height calc(100vh - 50px);">
				<view class="p-30">
					<view class="justify-between rounded-10 bg-white text-black px-20 py-30 mb-30"
						...
						<navigator :url="`/pages/staking/details?id=${item.id}`"
							class="mt-20 rounded-3xl p-20 text-black text-sm text-center bg-primary">
							{{ $t("staking.shengou") }}
						</navigator>
