<template>
	<view :class="theme">
		<view class="p-30">
			<view class="bg-white dark:text-black p-20 mb-20 rounded-8">
				...
				<view class="border-0 border-b-2 border-solid border-gray-200 py-30">
					...
					<view class="text-2xl font-bold text-yellow-500">
				...
				<view class="border-0 border-b-2 border-solid border-gray-200 py-30">
					<view class="flex justify-between text-lg mb-10">
						...
						<text class="text-yellow-500">{{subscription.issue_number}} {{subscription.currency_name}}</text>
					</view>
					<view class="flex justify-between text-lg">
						<text class="text-gray-400">{{$t('subscription.shengyushuliang')}}</text>
						<text class="text-yellow-500">{{subscription.issue_number - subscription.subscribed || ''}} {{subscription.currency_name}}</text>
					</view>
				</view>
				...
				<view class="flex h-80 my-40">
					<view class="w-120 mr-20 text-gray-400 border-0 border-b-4 border-solid border-yellow-500 h-full flex justify-center items-center">
					...
					<view class="flex-1 border-0 border-b-4 border-solid border-yellow-500 h-full">
						<input type="number" :placeholder="$t('legal.p_amout')" class="h-full text-xl px-20 text-black" v-model="amount">
					</view>
				</view>
				
				<view class="mt-20 rounded-10 p-20 text-black text-sm text-center bg-primary my-40" @tap="submit">{{$t('subscription.lijishenggou')}}</view>
				<view v-if="subscription.currency && subscription.currency.desc">
					<view class="text-xl mb20 text-black">{{$t('subscription.bizhongjieshao')}}</view>
					<view class="flex1 text-black break-all">{{subscription.currency.desc}}</view>
				</view>
			</view>
		</view>
    </view>
				...
				try {
					...
					that.$utils.initDataToken({ 
						...
						uni.setNavigationBarTitle({
							title: `${this.subscription.currency_name} `+ that.$t('subscription.new_subscription')
						});
					});
				} catch (e) {
					console.log(e);
