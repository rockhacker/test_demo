<template>
	<view :class="theme">
		<uni-nav-bar fixed backIcon>
			<view class="flex-1 p-30 bg-white dark:bg-floor flex justify-center items-center text-2xl">
				{{$t('subscription.new_subscription')}}
			</view>
			<slot name="right">
				<view class="w-100"></view>
				<navigator url="./orders" class="px-10 py-10 border-solid border-2 border-yellow-500 rounded-10 absolute top-24 right-20 text-yellow-500 z-10 dark:border-primary dark:text-primary" >{{$t('subscription.my_subscription')}}</navigator>
			</slot>
		</uni-nav-bar>
			
		<view class="p-30" v-if="list.length">
			<navigator class="rounded-10 bg-white dark:text-black p-20 mb-20" :url="`./details?id=${item.id}`" hover-class="opacity-50" v-for="(item,index) in list" :key="index">
				<view class="rounded-10">
					<view class="flex justify-between items-center mb-10">
						<text class="text-2xl font-bold">{{item.currency_name}} {{$t('subscription.new_subscription')}}</text>
						<view class="text-sm rounded-8 p-20 text-black bg-primary">
							{{$t('subscription.canjiahuodong')}}
						</view>
					</view>
					...
					<view class="flex justify-between items-center mb-20">
						...
						<view class="text-sm">
							<text>{{$t('subscription.shengyu')}}</text>:
							<text class="ml-10">{{ remainingRatio(item.subscribed,item.issue_number)}}%</text>
						</view>
					</view>
					<view class="w-full h-30 rounded-2xl bg-gray-200 relative overflow-hidden">
						<view class="absolute left-0 h-full leading-30 bg-yellow-500 text-white text-sm text-right" :style="`width: ${remainingRatio(item.subscribed,item.issue_number)}%;`">
