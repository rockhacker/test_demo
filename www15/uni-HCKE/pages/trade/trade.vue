<template>
	<view :class="theme">
		...
		<!-- 头部 -->
		<view class="header bg-floor fixed flex alcenter jscenter plr15 zdx100">
		...
		<template v-else>
			<view class="flex pt70 pb15 bg-floor">
				<!-- 左侧 -->
		...
		<uni-popup-1 v-if="false" :show="announcementModal.showModal" @hidePopup="hideBtn" :closeBtn="true" :msg="announcementModal.title" :btnShow="announcementModal.tipbtnShow">
...
<script>
	...
	import uniPopup1 from '@/components/uni-popup.vue';
	...
	export default {
		...
		hideBtn(){
			...
			changeCurrency(idx){//交易对切换
				...
				this.soon = data.base_currency_code=='MUT' && !this.isOnline(data.market_time)
				...
				that.$utils.initData({ url: 'market/currency_matches' }, res => {
					if (res.length > 0) {
						...
						if(localData){
							...
							that.soon = arrFrist.currency_name=='MUT' && !that.isOnline(localData.market_time);
							...
							that.soon = arrFrist.base_currency_code=='MUT' && !that.isOnline(arrFrist.market_time);
