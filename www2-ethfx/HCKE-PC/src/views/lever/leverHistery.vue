<template>
  <div>
    <div class="flex bgwhite dark-bg-1C2532 ht40 lh40 baselight2 ft16">
      <!-- <div class="plr40 pointer" :class="types==6?'bgpart':''" @click="changeType(6)">{{$t('lever.dedan')}}</div> -->
      <div class="plr40 pointer" :class="types==5?'text-clip-green':''" @click="changeType(5)">{{$t('lever.current')}}</div>
      <div class="plr40 pointer" :class="types==1?'text-clip-green':''" @click="changeType(1)">{{$t('lever.allPosition')}}</div>
      <div class="plr40 pointer" :class="types==0?'text-clip-green':''" @click="changeType(0)">{{$t('lever.allWei')}}</div>
      <div class="plr40 pointer" :class="types==3?'text-clip-green':''" @click="changeType(3)">{{$t('lever.allClose')}}</div>
      <div class="plr40 pointer" :class="types==4?'text-clip-green':''" @click="changeType(4)">{{$t('lever.allBack')}}</div>
    </div>
    <div class="bgwhite dark-bg-1C2532 plr15 pb30">
      <!-- 当前挂单 -->
      <ul v-if="types==6">
        ...
        <li v-for="(item,index) in currentDelegation" :key="index" class="flex alcenter ptb10">
          ...
          <span class="flex1 tr text-clip-green pointer" @click="leverClose(item.id)">{{$t('lever.chedan')}}</span>
        </li>
      ...
      <ul v-else-if="types==5">
        <li class="flex alcenter between">
          <div>
            ...
            <p class="mt10">
              ...
              >{{historyData.totalRisk || '0.0000'}}</span>
            </p>
          </div>
          <p
            class="pointer plr15 tc ft12 baselight2 radius2 bg-main-liner white ptb5"
        ...
        <li class="flex alcenter ptb10">
          ...
          <span class="flex1 tc">{{$t('second.profitRate')}}</span>
        ...
        >
          ...
          <span class="flex1 tc" :class="item.profits < 0 ? 'red' : 'green'">{{item.profits}}</span>
          <span class="flex1 tc" :class="item.profits < 0 ? 'red' : 'green'">
            {{((parseFloat(item.profits)/parseFloat(item.caution_money))*100).toFixed(2)}}%
          </span>
          <div class="flex2 tr text-clip-green">
      ...
      <ul v-else>
        ...
        <li class="flex alcenter ptb10" v-for="(item,index) in historyData.orderList" :key="index">
          ...
          <span class="flex1 tc">{{types == 0 ? item.origin_price : item.price}}</span>
          <!-- <span class="flex1 tc">{{item.update_price}}</span> -->
          <span class="flex1 tc">{{item.target_profit_price}}</span>
          <span class="flex1 tc">{{item.stop_loss_price}}</span>
          <span class="flex2 tc">{{item.time}}</span>
          <span class="flex2 tc" v-if="types==3" >{{item.complete_time}}</span>
          <span class="flex2 tc" v-if="types==3" >{{item.update_price}}</span>
          <span class="flex1 tc">{{item.trade_fee}}</span>
          <span class="flex1 tc">{{item.overnight_money}}</span>
          <span class="flex1 tc">{{item.number}}</span>
          ...
          <span class="flex1 tc" :class="item.profits < 0 ? 'red' : 'green'">{{item.profits}}</span>
          <span
            class="flex1 tr text-clip-green pointer"
    ...
    <el-dialog :title="$t('lever.setys')" :visible.sync="lossShow" width="40%" :before-close="handleClose">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="setLoss" class="radius4 bg-main-liner white">{{$t('lever.e_confrim2')}}</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog :title="$t('lever.confrim_ping')" :visible.sync="allCloseShow" width="40%" :before-close="handleClose">
      <div class="flex jscenter">
        <div
          class="text-clip-green bdblue ptb5 plr15 radius4 pointer"
          :class="closeType==0?'white bg-main-liner':''"
          @click="closeType=0"
        >{{$t('lever.allClose')}}</div>
        <div
          class="green bdgreen ptb5 plr15 radius4 ml25 pointer"
          :class="closeType==1?'white bg-main-liner':''"
          @click="closeType=1"
        >{{$t('lever.duo_ping')}}</div>
        <div
          class="red bdred ptb5 plr15 radius4 ml25 pointer"
          :class="closeType==2?'white bg-main-liner':''"
          @click="closeType=2"
      ...
      <span slot="footer" class="dialog-footer flex jscenter">
        ...
        <el-button type="primary" @click="allClose" class="radius4 wt200 bg-main-liner white">{{$t('lever.e_confrim2')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
