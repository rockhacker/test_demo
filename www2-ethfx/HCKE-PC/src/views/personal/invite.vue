<template>
  <div class=" flex column alcenter">
    ...
    <div class="mt20 radius30 ptb20" style="background:#282842">
      <div class=" flex column between alcenter">
        <div class="tc pt20 white">
          {{ extension_url }}
        </div>
        <div
          style="background: linear-gradient(94deg,#42cbff,#015afe);width:250px"
          class="mt20  white radius30  ptb10 tc"
          id="copy2"
          @click="copy2"
        >
          {{ $t('feed.copyadd') }}
     ...
     <div class="mt20 radius30 flex ptb20 white flex column alcenter" style="background:#282842">
        <span>      {{$t('new.fybl')}}:  {{$t('new.yj')}}/{{set_comm[1]}}%, {{$t('new.ej')}}/{{set_comm[2]}}%, {{$t('new.sj')}}/{{set_comm[3]}}%
</span>
    </div>

    <div class="mt20 radius30 flex ptb10 white" style="background:#282842">
      <div class="flex1">
    ...
    <div class="ptb20 mt30">
      <div class="flex1 plr20 ptb20 ft18 bdbheader">{{ $t('assets.record') }}</div>
     
     <el-table :data="commission_list" :empty-text="$t('staking.noRecords')" style="width: 100%">
