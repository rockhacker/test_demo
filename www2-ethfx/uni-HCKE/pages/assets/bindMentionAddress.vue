	...
	<view :class="theme">
		<view class="bg-my-grey dark:bg-floor">
			...
			<view class="px-30 py-20">
				...
				<scroll-view scroll-x class="w-full whitespace-nowrap" :show-scrollbar="false">
					<view class="px-20 py-10 mr-40 flex-col justify-center items-center text-lg inline-flex" hover-class="opacity-50"
						:class="{'bg-new-blue':index==i,'border-2 border-solid border-E5E5E5':index!=i}" v-for="(item,i) in coinList" :key="i" 
			...
			<view class="px-30 py-20">
					...
					<view v-if="coinList[index].currency_protocols.length > 1">
						...
						<view class="flex mb-30">
							<view v-for="(item, index) in coinList[index].currency_protocols" :key="index" 
								class="text-lg mr-20 py-10 px-20  border-2 border-solid border-gray-200 dark:border-gray-700" :class="{'text-new-blue border-new-blue': name == item.chain_protocol.code}"
				...
				<view class="mt-100 mb-60 bg-new-blue rounded-10 p-20 text-white text-lg text-center" @tap="bindAddress" hover-class="opacity-50">{{ $t('bind.bind') }}</view>
			</view>
			<!-- 验证码 -->
			<!-- <view class="flex items-center">
