	...
	<view :class="theme">
		<view class="bg-white dark:bg-1C2532 text-black dark:text-white">
				...
				<view class="pt-40" v-if="coinInfo.currency_protocols&&coinInfo.currency_protocols.length > 1">
					...
					<view class="flex mt-20">
						<view v-for="(item, index) in coinInfo.currency_protocols" :key="index" class="mr-20 px-20 py-10 border-2 border-solid"
							:class="name == item.chain_protocol.code ? 'text-new-blue border-new-blue' : 'text-gray-500 border-gray-500'" @tap="selectCharge(item)">
			...
			<view class="p-40 ">
				...
				<view class="mt-20 rounded-10 h-80 flex justify-center items-center text-white text-sm text-center bg-new-blue" @tap="mention">{{ $t('assets.mention') }}</view>
			</view>
