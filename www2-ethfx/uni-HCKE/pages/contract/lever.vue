				...
				<view class="w-full flex flex-col p-40">
					<view class="w-full flex justify-center">
						<view class="flex w-full h-60 bg-F4F5F7 dark:bg-333F4A rounded-2xl overflow-hidden">
							<view class="flex-1 flex justify-center items-center text-center" :class="active==0?'bg-new-blue text-white':'text-333333 dark:text-white'" @tap="setActive(0)">{{$t('new.leverLes')}}</view>
							<view class="flex-1 flex justify-center items-center text-center" :class="active==1?'bg-new-blue text-white':'text-333333 dark:text-white'" @click="setActive(1)">{{$t('new.leverAll')}}</view>
							<!-- <view class="flex-1 flex justify-center items-center text-center" :class="active==2?'bg-new-blue text-white':'text-333333 dark:text-white'" @tap="setActive(2)">{{$t('qqjy.qqjy')}}</view> -->
						</view>
					</view>
				...
				<view class="flex justify-between p-20 bg-white dark:bg-1C2532">
					<view class="w-3_5">
						<view class="overflow-hidden flex justify-between rounded-10 bg-gray-100 dark:bg-gray-600 dark:bg-opacity-30 dark:text-white">
							<view class="text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'buy' ? 'bg-k-green text-white' : ''" @tap="tradeType('buy')">
								{{ $t('trade.buy') }}
							...
							<view class="text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'sell' ? 'bg-k-red text-white' : ''" @tap="tradeType('sell')">
								{{ $t('trade.sell') }}
						...
						<view class="h-70 flex justify-center items-center mt-30 rounded-10" :class="leverTradeType == 'buy' ? 'bg-k-green' : 'bg-k-red'" @tap="submits">
					...
					<view class="mt-40">
						...
						<view class="" v-if="orderList.length > 0">
							<view v-for="(item, index) in orderList" :key="index" class="flex justify-between items-center mt-20">
								...
								<view class="flex-1 text-center bg-new-blue py-10 rounded-8 ml-20 text-white" @tap="closeTrade(item.id)">{{ $t('trade.pingcang') }}</view>
							</view>
						</view>
				...
				<view class="pt-bar">
					<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
						<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
							...
							:class="{'text-new-blue':quoteCurrencyIds == item.id}" v-for="(item,i) in quotationList" :key="i" 
							@tap="selectedLegal(item.id)">
							{{item.code}}
							<view class="w-60 h-6 mt-10" :class="{'bg-new-blue':quoteCurrencyIds == item.id}"></view>
						</view>
					</scroll-view>
				</view>
					...
					<view class="pb-60">
						<view class="border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 text-black dark:text-white" v-for="(item, index) in quotationList[quoteIndex].matches" :key="index" v-if="item.open_lever == 1">
							...
							>
								...
								<view v-if="item.base_currency_code=='ETHFX' && !isOnline(item.market_time)">{{$t('trade.soon')}}</view>
								<view :class="item.change < 0 ? 'text-k-red' : 'text-k-green'" v-else>{{ item.currency_quotation.close || '0.00' }}</view>
...
export default {
	...
	onShow() {
		...
		if (that.priceStatus == 1) {
			that.priceValue = that.$t('trade.best');
		}
	...
	methods: {
		setActive(val){
			...
			if(val==0){
				...
				this.$socket.closeSocket()
				that.url=`/hybrid/html/index.html?legal_id=${that.quoteCurrencyId}&currency_id=${that.quoteCurrencyId}&symbol=${that.baseCurrencyCode + '/' + that.quoteCurrencyCode}&currency_match_id=${that.currencyMatchId}&type=lever`
			}else if(val==1){
				that.url=''
				this.init()
			}else if(val ==2){
			...
			that.$utils.initDataToken({ url: 'market/currency_matches' }, res => {
				...
				if (res.length > 0) {
					...
					if (that.quoteCurrencyId) {
								...
								if (lists.length > 0) {
									lists.forEach(function(itm, inx) {
										if (that.baseCurrencyId == itm.base_currency_id) {
											...
											that.soon = itm.base_currency_code=='ETHFX' && !that.isOnline(itm.market_time)
						...
						that.soon = list[0].base_currency_code=='ETHFX' && !this.isOnline(list[0].market_time)
		...
		selectCurrency(options) {
			...
			that.soon = options.base_currency_code=='ETHFX' && !this.isOnline(options.market_time)
		...
		linkFutures() {
			...
			uni.navigateTo({
				url: '/pages/contract/futures?legal_id=' + that.quoteCurrencyId + '&currency_id=' + that.quoteCurrencyId + '&symbol=' + symbols +'&currency_match_id='+that.currencyMatchId+'&type=lever'
			});
		},
		// 跳转k线
		linkKline() {
			...
			var symbols = that.baseCurrencyCode + '/' + that.quoteCurrencyCode;
			uni.navigateTo({
				url: '/pages/market/kline?legal_id=' + that.quoteCurrencyId + '&currency_id=' + that.quoteCurrencyId + '&symbol=' + symbols +'&currency_match_id='+that.currencyMatchId
			});
		},
		// 跳转合约订单记录
		linkLever() {
			...
			if(this.active==0){
				uni.navigateTo({
					url: `/pages/market/order?currency_id=${that.quoteCurrencyId}&currency_match_id=${that.currencyMatchId}&symbol=${symbols}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       
