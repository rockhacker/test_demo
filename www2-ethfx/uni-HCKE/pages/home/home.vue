<template>
  <view class="overflow-x-hidden" :class="theme">
	  <view class="bg-white dark:bg-my-black">
		  ...
		  <uni-nav-bar fixed>
			...
			<view class="absolute right-0 top-0 z-10 h-100 flex items-center">
				...
				<navigator  open-type="switchTab" url="/pages/assets/assets" class="py-10 px-20 mx-20 bg-new-blue rounded-full text-white">
		...
		<uni-swiper-dot :dots-styles="dotsStyles" :info="bannerList" :current="current" field="content" :mode="mode">
				<swiper  :autoplay="true" :interval="3000" active-class="activeClass" class="swiper h-640 w-full" @change="change">
		...
		<view class="flex py-40 text-my-black mt-20">
		...
		<!--涨幅榜-->
		<swiper v-if="quoList.length" :display-multiple-items="quoList.length>2?3:quoList.length" class="w-full h-170 bg-white dark:bg-1C2532 pl-20 pr-20">
		  <swiper-item class="flex items-start justify-around"  v-for="(item, i) in quoList" :key="i">
			<view @click="toUrl(item,allList[1])" class="flex flex-col items-center w-200 h-170 justify-center rounded-10 relative" style="background:#F0F4FB;">
      ...
      <view
        class="w-full rounded-full h-90 flex items-center justify-between px-60 bg-k-red"
      ...
      <view
        class="mt-20 w-full rounded-full h-90 flex items-center justify-between px-60 bg-new-blue"
	...
	<view class="pb-100 mt-20 bg-mg-gray">
		 <view  v-for="(list, index) in allList" :key="index">
			 <view class="text-my-black text-center text-3xl py-30">
			 {{ index==1?$t('new.ysqhq'):list.code }}
		  	</view>

			  <view @click="toUrl(allList[index].matches[0],allList[index])" class="flex items-center text-sm text-gray-400 tableitem mx-40 mb-30 py-30 bg-new-blue px-20">
		  ...
		  <view class="flex items-center text-my-black text-lg mx-40 px-10 py-20 bg-EDEDED">
		  ...
		  <view @click="toUrl(item,allList[index])" class="flex items-center text-sm text-gray-400 tableitem mx-40 py-30" style="border-bottom: 2rpx dashed #E6E6E6;" v-for="(item, i) in list.matches" :key="i">
			<view class="flex items-center" style="flex: 1.5">
				<image class="w-40 h-40 mr-16 rounded-half bg-white" :src="host + item.base_currency.logo"></image>
				<view class="text-sm text-my-black"><text class="text-sm text-mg-black">{{ item.base_currency_code }}</text>/{{ item.quote_currency_code }}</view>
			</view>
			<view v-if="item.base_currency_code=='MUT' && !isOnline(item.market_time)" class="w-200">{{$t('trade.soon')}}</view>
			<template v-else>
				<view class="flex-1 text-my-black text-sm" v-if="item.currency_quotation">{{ parseFloat(item.currency_quotation.close) }}</view>
