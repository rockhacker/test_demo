			...
			<view class="bg-white dark:bg-1C2532">
				<view class="flex justify-between items-center w-full py-30 px-30">
					<view class="flex items-baseline justify-center font-bold">
						<view class="mr-30" :class="Type=='sell'?'text-3xl text-new-blue':'text-gray-500 text-lg'" @click="changetype('sell')">{{$t('legal.ibuy')}}</view>
						<view :class="Type=='buy'?'text-3xl text-new-blue':'text-gray-500 text-lg'" @click="changetype('buy')">{{$t('legal.isell')}}</view>
					</view>
				...
				<view class="w-full border-0 border-b-2 border-solid border-gray-200 dark:border-gray-700">
					<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
						<view class="w-140 h-100 flex-col justify-center items-center text-lg inline-flex rounded-none" 
							...
							:class="{'text-new-blue':coinindex==index}" v-for="(item,index) in legal_list" :key="index" 
							 @click="changeCoin(index,item.id)">
							{{item.code}}
							<view class="w-60 h-6 mt-20" :class="{'bg-new-blue':coinindex==index}"></view>
						</view>
					</scroll-view>
				</view>
			</view>
			<view class="bg-my-grey dark:bg-101625 mt-20">
				<view class="gray75 flex column w100 plr20 ptb10 bdb27 bg-white dark:bg-1C2532" v-for="item in orderlist" :key="item.id">
					...
					<view class="flex justify-between mt5">
						...
						<view class="flex items-center">
							<text class="text-new-blue ft16">{{item.price | toFixed2}}{{item.area_currency}}</text>
						</view>
					</view>
					<view class="flex justify-between mt5">
						<view class="flex items-center">
						...
						<view class="flex items-center jscenter h25 plr20 bg-new-blue radius2" @click="getinfo(item)">
			...
			<view class="layer_box" v-show="box1" @click="isshow">
				<view class="layer_opeation bgHeader" @click.stop>
					<view class="bggray flex justify-between items-center plr15 ptb10">
						<view class=" flex column">
							...
							<view class="mt5 ft14">
								...
								<text class="text-new-blue price pl5" v-if="Data.price"> {{Data.price | toFixed2}} </text> 
								<text class="text-new-blue price pl5">{{Data.area_currency}}</text>
							</view>
						</view>
					...
					<view class="mtb10 tab flex column plr15">
						...
						<view class="flex justify-between items-center Ipt ft13 pl10 mt10 bd3a" v-if="msg==0">
							...
							<view class="ptb10 flex w40 jscenter w40">
								...
								<text class="plr10 text-new-blue tc" @click="all(0)">{{$t('trade.all')}}{{Type=='sell'?$t('trade.sell'):$t('trade.buy')}}</text>
							</view>
						</view>
						<view class="flex justify-between items-center Ipt ft13 pl10 mt10 bd3a" v-if="msg==1">
							...
							<view class="ptb10 flex w40 jscenter w40">
								...
								<text class="plr10 text-new-blue tc" @click="all(1)">{{$t('trade.all')}}{{Type=='sell'?$t('trade.buy'):$t('trade.sell')}}</text>
							</view>
						</view>
						...
						<view class="ft14 flex items-center justify-between mt10">
							...
							<text v-if="legal_list[coinindex]" class="text-new-blue ft16 total">{{totalPrice }} {{legal_list[coinindex].code}}</text>
							<!-- <text class="text-new-blue ft16 total" v-if="msg==0">￥{{money | toFixed2}}</text>
							<text class="text-new-blue ft16 total" v-if="msg==1">￥{{(number*Data.price) | toFixed2}}</text> -->
						</view>
					</view>
					<view class="flex justify-between items-center  plr15 ft14 ptb10">
						...
						<view class="bg-new-blue radius2 ptb10 flex1 tc doit pointer ml5 white" @click="pay(Data.id)">{{$t('legal.do')}}</view>
					</view>
				</view>
			</view>
			<!-- 底部密码弹窗 -->
