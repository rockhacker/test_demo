<template>
	<view :class="theme">
		<view class="bg-white dark:bg-floor p-30">
			...
			<button v-if="!show" @click="show=true" class="w-full rounded-3xl bg-new-blue text-white text-xl" >
				<!-- <image src="../../static/image/card.png" mode="aspectFit" class="w-30 h-30"></image> -->
				{{$t('store.bank')}}	
			</button>

			<button v-if="show" class="w-full rounded-3xl bg-new-blue text-white text-xl mt-100" @tap="confirm">{{$t('login.e_confrim')}}</button>
		</view>
	</view>
</template>
