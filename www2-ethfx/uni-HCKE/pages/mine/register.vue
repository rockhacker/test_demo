<template>
	<view :class="theme">
		 <view class="p-30 bg-my-grey dark:bg-1C2532 text-black dark:text-white" style="min-height: 100vh;">
			<view>
			<view class="ft24 text-new-blue" v-if="loginActive==0">
				{{$t('login').r_mobile}}
			</view>
			<view class="ft24 text-new-blue" v-else>
				{{$t('login').r_email}}
			</view>
			...
			<view class="mt40 ">
				<view class="mb10 ">
					...
					<!-- <view class="flex bgwhite alcenter bdb_myblue ">
						...
						<button size="mini" type="primary" class="px-40 bg-new-blue text-white"  :disabled="disable" :loading="load" @click="send">{{ codeText }}</button>
					</view> -->
					<view class="flex bgwhite alcenter bdb_myblue ">
					...
					<view class="mt20 flex alcenter">
						...
						<navigator :url="'/pages/home/agreement'">
							<view class="ml10 text-new-blue">《{{$t('login').p_private}}》</view>
						</navigator>
					</view>
					<view class="mt45 radius4 ptb10 ft14 tc mb10 bg-new-blue text-white" @tap="register">{{$t('login.register')}}</view>
				</view>
			</view>
			<view class="mt20 flex justify-start alcenter" style="justify-content: end;">
				<view class="text-new-blue" v-if="loginActive==1" @click="loginActive=0;user_string=''">
					{{$t('login.r_mobile')}}
				</view>
				<view class="text-new-blue" v-else @click="loginActive=1;user_string=''">
					{{$t('login.r_email')}}
				</view>
				<!-- #ifdef H5 -->
