<template>
	<view :class="theme">
		<view class="bg-my-grey dark:bg-101625 text-black dark:text-white w-full min-h-screen overflow-hidden">
			
			<view v-if="tableData.length">
				<view class="flex justify-between text-center h-80 items-center text-2xl">
					<view class="flex-1">{{ $t('staking.lixi') }}</view>
					<view class="flex-1">{{ $t('staking.time') }}</view>
				</view>
				<view v-if="tableData.length > 0">
					<view v-for="(item, index) in tableData" :key="index" class="flex justify-between items-center text-center h-80 border-0 border-b-2 border-solid border-gray-300 bg-white dark:bg-floor">
	...
	export default {
		...
		methods:{
			...
			getInsList(id) {
				try {
					const that=this;
					that.$utils.initDataToken({url: "fund/insList" , data:{sub_id:id} },(res,msg) => {
