<template>
	<view class="pb-100" :class="theme">
		<!-- 头部 -->
		<view class="h-88 w-full bg-new-blue fixed flex alcenter jscenter plr15 zdx100">
		...
		<template v-else>
					...
					<view class="">
						...
						<view class="py-10">
							...
							<view class="flex alcenter around tc tab-box gray7 input-title">
								<view :class="['bg-my-grey flex1 radius2',{'bg-k-green text-white bd_green': percent==item&&type=='buy'},{'bg-k-red text-white bd_red': percent==item&&type=='sell'},{'ml10':index>0}]" v-for="(item,index) in percentList" :key='index' @click="changePercent(item)">{{item}}%</view>
							</view>
						</view>
						<view class="py-10">
			...
			<view class="flex alcenter between ptb10  plr15 footer pb-10 border-0 border-solid border-b-2 border-E5E5E5 dark:border-999FAA">
				...
				<view class="flex alcenter gray7">
					...
					<navigator class="ml-20 text-lg text-black bg-my-grey py-10 px-40 rounded-8 text-center" url="entrust">{{$t('new.newAll')}}</navigator>
				</view>
			</view>
			<!-- 委托类型切换弹窗 -->
