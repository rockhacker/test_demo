<template>
  <div>
    <div class="between baselight2 ptb20 plr20 ft16 f-item flex alcenter">
      <p>  <span class="pointer" @click="$router.go(-1)">{{name}}{{$t('assets.account')}} <span>/</span> </span > <span class="blue">{{$t('assets.records')}}</span></p>
      <div>
        <el-select v-model="coinCode" :placeholder="$t('assets.selectCurrency')" class="flex1 bdinput" @change="changeCoin">
    ...
    <div class=" plr15">
      ...
      <!-- 记录列表 -->
      <ul>
        <li class="flex alcenter ptb10 white">
        ...
        <li class="flex alcenter ptb10 white" v-for="item in logList" :key="item.id">
