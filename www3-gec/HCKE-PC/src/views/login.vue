        ...
        <div class="bgpart radius4 plr60 ptb40 mb50 w25 mwidth" style="margin-right:80px;">
            ...
            <div class="mt40 ">
                <div class="bdbea2 flex alcenter bold">
                    <div class="pb10 mr20 pointer ft14" :class="{'active2px':index==i}" v-for="(item,i) in infos" :key="i" @click='changeMethod(i)'>{{$t(item)}}</div>
                </div>
                <div class="mt20 radius4 h46 flex alcenter">
                   ...
                   <!-- <el-select v-model="country_code"  class="w25" @change='changeCountry' v-show="index==0">
                    ...
                    <input type="text" class="flex1 h46 my-input-border pl20 nwhite ft14" v-model="account" @change="changeAccount" :placeholder="index == 0 ? $t('register.enterPhone') : $t('register.enterEmail')">
                </div>
                <div class="mt20 radius4 h46 flex alcenter">
                    <input type="password" class="flex1 h46 my-input-border pl20 nwhite ft14" v-model="password" :placeholder="$t('register.enterPassword')">
                </div>
                ...
                <div class="flex between alcenter mt10">
                    <p class="flex alcenter">
						...
						<el-checkbox v-model="remember" text-color="#fff" fill="#37b6ff">{{$t('login.rem_pwd')}}</el-checkbox>
                    </p>
                    <router-link to="/forgetPassword" class="blue ft16">{{$t('login.forget_pwd')}}？</router-link>
                </div>
                <div class="mt20 white tc h46 lh46 radius2 pointer ft18 bg-my-orange" style="line-height:46px;" @click='login'>{{$t('login.login')}}</div>
                <div class="tc mt30 ft16" style="color:#888888">{{$t('add.myzh')}} 
                     <router-link
                     class="white"
                    :to="{ name: 'register' }" 
                    >{{$t('add.qzc')}}
...
export default {
    ...
    created(){
        ...
        //     type = 2;
        // }
        this.index = localStorage.getItem('actives') || 0;
    ...
    methods:{
        ...
        login(){
            ...
            .then(res=>{
                ...
                this.$emit('getUserinfo',this.$route.query.page || 'home');
