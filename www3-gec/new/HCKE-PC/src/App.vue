    ...
    >
      <div class="flex alcenter between h48 lh48 app">
		<div :class="!value?'bghome':''" class="pointer mr50 ht50 plr10 flex alcenter">
			<img src="../static/logo.png" style="width:30px" @click="goHome" v-if="!value"/>
			<img src="../static/logo.png" style="width:30px" @click="goHome" v-else/>
		</div>
        <router-link
          :to="{ name: item.page }"
          class="mr25"
        ...
        >
        <!-- <div @click="tokefu">{{$t('new.kefu')}}</div> -->
        <!-- <el-menu  @select="handleSelect" background-color="#0B0E11"   text-color="#fff" :default-active="activeIndex" class="el-menu-demo" mode="horizontal">
          <el-submenu index="1">
                <template slot="title">{{ $t('new.kefu') }}</template>
                <el-menu-item index="2-1">{{ $t('home.ywkf') }}</el-menu-item>
                <el-menu-item index="2-2">{{ $t('home.dkkf') }}</el-menu-item>
          </el-submenu>
      </el-menu> -->
      </div>
...
export default {
  ...
  methods: {
    ...
    //      email:this.account
    //    }
    //    window.open(`https://chat.ichatlink.net/widget/standalone.html?eid=ec1ca902e661d35961f562c481072048&language=en&metadata=${JSON.stringify(obj)}`)
    //  }else{
    //   window.open('https://chat.ichatlink.net/widget/standalone.html?eid=ec1ca902e661d35961f562c481072048&language=en')
    //  }
