...
class Socket{
    ...
    close(){
        var that=this;
        this.clearInterval(this.ping_time)
        this.ping_time = null
...
let protocol = location.protocol === 'https:' ? `wss:${Axios._DOMAIN.replace(/^https?\:\/\//i, "")}/v1/ws` : `wss:${Axios._DOMAIN.replace(/^https?\:\/\//i, "")}/v1/ws`
