<template>
  <div class="baselight assets-transfer">
    <div class="flex between baselight2 ptb20 plr20 ft16 f-item">
      <span>{{$t('assets.currencyTransfer')}}</span>
      <router-link :to="{name:'transferLog',query:{currency_id:currency_id,currency_code:curcoin}}" class="blue">{{$t('assets.transferRecord')}}</router-link>
    </div>
    <div class="plr20 pt20 pb50 cointransfer">
      <div class="flex alcenter white">
        <span class="inblock wt80">{{$t('assets.transferCurrency')}}:</span>
        <el-select v-model="curcoin" class="flex1 white bdinput" @change="changeCoin" placeholder="">
          <el-option
      ...
      <div class="flex mt30 white">
        <div class="wt80">{{$t('assets.transferAccount')}}:</div>
        <div class="flex">
          <div style="position: relative;">
            <div class="flex alcenter" style="position: relative;">
              <div class="pr10" style="position: absolute;left:8px">{{$t('assets.from')}}</div>
              <el-select v-model="fromtype" class="white bdinput pl20" @change="changetransfer1" placeholder="">
                <el-option
          ...
          <div class="pl10" style="position: relative;">
            <div class="flex alcenter" style="position: relative;">
              <div class="pr10" style="position: absolute;left:8px">{{$t('assets.reach')}}</div>
              <el-select v-model="totype" class="bdinput pl20" @change="changetransfer2" placeholder="">
                <el-option
                  v-for="item in accoutlist"
                  :key="item.account_code"
                  :label="item.name"
      ...
      <div class="mt30 flex alcenter">
		...
		<div class="bdline ht40 lh40 radius2 baselight2">
			<input type="number" class="h100 pl20 baselight2" v-model="number" />
			<span class="plr10" @click="number=balance">{{$t('assets.max')}}</span>
		</div>
      </div>
      <div class="flex mt60">
        <div class="wt80"></div>
        <div class="bgblue lh48 ht48 tc white ft16 radius4 wt217" @click="transferConfirm">{{$t('login.e_confrim2')}}</div>
      </div>
    </div>
  </div>
</template>
