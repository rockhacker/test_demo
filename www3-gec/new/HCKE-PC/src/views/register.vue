<template>
    <div class="flex alcenter jsend  vh100"  :style="{'background-image': `url(${require('../assets/images/register.png')})`}" >
        <div class="bgpart radius4 plr50 ptb40 mb50 w25 mwidth" style="margin-right:80px;">
            <div class="tc ft20">{{$t('register.welcome')}}</div>
            <div class="mt40 ">
                <div class="bdbea2 flex alcenter bold">
                    <div class="pb10 mr20 pointer"  v-show="i==1" :class="{'active2px':index==i}" v-for="(item,i) in infos" :key="i" @click='changeMethod(i)'>{{$t(item)}}</div>
                </div>
                <div class="mt20 radius4 h46 flex alcenter">
                    ...
                    <input type="text" class="flex1 h46 my-input-border pl20 nwhite" v-model="account" @change="changeAccount" :placeholder="index == 0 ? $t('register.enterPhone') : $t('register.enterEmail')">
                </div>
                <!-- <p class="mt20">
                ...
                <div class="mt20 radius4  h46 flex alcenter">
                    ...
                    <div v-loading="sendLoad" class="w20 my-input-border radius4 h46 pointer tc text-my-orange" style="border-left-width:0;line-height: 46px;border-radius: 0 4px 4px 0;" @click="sendcode" v-show="!hassend">{{$t('login.send')}}</div>
                ...
                <div class="mt20 radius4 h46 flex alcenter">
                    <input type="password" class="flex1 my-input-border h46 pl20 nwhite" v-model="password" :placeholder="$t('register.enterPassword')">
                </div>
                <div class="mt20 radius4 h46 flex alcenter">
                    <input type="password" class="flex1 my-input-border h46  pl20 nwhite" v-model="re_password" :placeholder="$t('register.enterPassword2')">
                </div>
                <div class="mt20 radius4 h46 flex alcenter">
                    <input type="text" class="flex1 h46 my-input-border  pl20 nwhite" v-model="invite_code" :placeholder="$t('register.invitationCode')">
                </div>
                ...
                <!-- </div> -->
                <div class="mt20 white tc h46 radius4 pointer ft18" style="line-height:46px;background: linear-gradient(90deg, #FFB216, #FF9D12);" @click='register'>{{$t('register.register')}}</div>
                <div class="tc mt30 ft16" style="color:#888888">{{$t('add.yyzh')}} 
                     <router-link
                     class="white"
                    :to="{ name: 'login' }" 
                    >{{$t('add.ljdl')}}
...
export default {
    data(){
        return {
            infos:['login.r_mobile','login.r_email'],
            index:1,
            account:'',
            password:'',
            re_password:'',
