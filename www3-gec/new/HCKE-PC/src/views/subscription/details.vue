<template>
	<div class="minh100 bgblack ft16 s-details">
		<div class="mauto wt1200 ptb20">
			<el-card class="box-card mb20 f-item" style="border:0;">
				<el-breadcrumb separator="/">
				  <el-breadcrumb-item :to="{ path: '/subscription' }">{{$t('subscription.new_subscription')}}</el-breadcrumb-item>
				  <el-breadcrumb-item v-if="subscription.currency_name">{{`${subscription.currency_name}${$t('subscription.shenggou')}`}}</el-breadcrumb-item>
				</el-breadcrumb>
			</el-card>
			
			<el-card class="box-card mb20 ft20 f-item" style="border:0;">
				<div class="ft18 mlr20 mtb20 plr70 ptb30 radius10 white">
					...
					<div class="bdbea2 ptb20">
						...
						<div class="text-primary bold">1{{subscription.currency_name}} = {{subscription.sub_price}} USDT</div>
					</div>
					<div class="bdbea2 ptb20">
						<div class="mb10 flex between">
							<div class="">{{$t('subscription.faxingshuliang')}}</div>
							<div class="text-primary bold">{{subscription.issue_number}} {{subscription.currency_name}}</div>
						</div>
						<div class="flex between">
							<div class="">{{$t('subscription.shengyushuliang')}}</div>
							<div class="text-primary bold">{{subscription.issue_number - subscription.subscribed || ''}} {{subscription.currency_name}}</div>
						</div>
					</div>
					...
					<div class="flex mauto ptb50">
						...
						<div class="wt200 mr20">
							<el-select v-model="coin.active" class="white">
							    <el-option
						...
						<div class="flex1 bdbea2">
							<input type="number" :placeholder="$t('subscription.qsrje')" class="w100 h100 ft18 plr20 white" v-model="amount">
						</div>
						
					</div>
					<el-button type="primary" class="w100 mb50 bg-my-orange white" @click="submit">{{$t('subscription.lijishenggou')}}</el-button>
					
					<div v-if="subscription.currency && subscription.currency.desc" class="mb50">
						<div class="ft16 white mb20">{{$t('subscription.bizhongjieshao')}}</div>
