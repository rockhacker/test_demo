    ...
    methods: {
      ...
      RecvHistoryData(data, callback, recvdata) {
        ...
        recvdata.map((item) => {
          ...
          let offset = dateTime.getTimezoneOffset(); // => -480 因为格林尼治时间比本地时间小8h
          // 格林尼治的当前时间的时间戳
        let stampGTM = item.time +( offset * 60 * 1000) -(5*60*60*1000);
          dateTime.setTime(stampGTM);
          var date =
      ...
      RecvSubscribeRealtimeData(recvdata, data) {
        ...
        let offset = dateTime.getTimezoneOffset(); // => -480 因为格林尼治时间比本地时间小8h
          // 格林尼治的当前时间的时间戳
        let stampGTM = kData.time +( offset * 60 * 1000) -(5*60*60*1000);
        dateTime.setTime(stampGTM);
        var date =
          dateTime.getFullYear() * 10000 +
          (dateTime.getMonth() + 1) * 100 +
      ...
      RecvMinuteData(data, callback, res) {
        ...
        for (var i in res.data) {
          ...
          let offset = dateTime.getTimezoneOffset(); // => -480 因为格林尼治时间比本地时间小8h
          // 格林尼治的当前时间的时间戳
          let stampGTM = item[0] +( offset * 60 * 1000) -(5*60*60*1000);
          dateTime.setTime(stampGTM);
          var date =
            dateTime.getFullYear() * 10000 +
            (dateTime.getMonth() + 1) * 100 +
      ...
      RecvSubscribeMinuteData(recvdata, data) {
        ...
        let offset = dateTime.getTimezoneOffset(); // => -480 因为格林尼治时间比本地时间小8h
          // 格林尼治的当前时间的时间戳
        let stampGTM = kData.t +( offset * 60 * 1000) -(5*60*60*1000);
        dateTime.setTime(stampGTM);
        var date =
          dateTime.getFullYear() * 10000 +
          (dateTime.getMonth() + 1) * 100 +
