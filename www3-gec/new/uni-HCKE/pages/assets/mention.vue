	...
	<view :class="theme">
		<view class="text-white">
			...
			<view class="p-40">
				...
				<view class="mt-40 bg-my-gray p-20 rounded-xl" v-if="coinInfo.currency_protocols&&coinInfo.currency_protocols.length > 1">
					<view class="text-lg">{{$t('bind.liantype')}}</view>
					<view class="flex mt-20">
						<view v-for="(item, index) in coinInfo.currency_protocols" :key="index" class="rounded-2xl mr-20 px-30 py-12 border-2 border-solid"
							:class="name == item.chain_protocol.code ? 'text-my-orange border-my-orange' : 'text-my-gray2 border-my-gray2'" @tap="selectCharge(item)">
							{{ item.chain_protocol.code }}
						</view>
					</view>
				</view>
			</view>
			<view class="p-40 pt-0">
				<view class="mb-40 bg-my-gray p-20 rounded-xl">
					<view class="text-xl">{{ $t('assets.mentionaddr') }}</view>
					<view class="flex items-center justify-between">
						<input type="text" v-model="address" class="h-80 flex-1 text-base" disabled="" :placeholder="$t('assets.p_addr')" />
				...
				<view class="mb-40 bg-my-gray p-20 rounded-xl">
					...
					<view class="flex items-center justify-between ">
						<input type="number" class="h-80 flex-1 text-base" v-model="number" @input="numberChange" :placeholder="$t('assets.minnum') + (coinInfo.draw_min || '0.00')" />
						<view class="flex items-center">
							<text class="text-white text-lg pr-20 border-0 border-r-2 border-solid border-gray-200 dark:border-gray-700 ">{{ currencyName || '--' }}</text>
							<view class="text-lg px-20 text-my-orange" @tap="all">{{ $t('trade.all') }}</view>
						</view>
					</view>
				</view>
				...
				<view class="mb-40 bg-my-gray p-20 rounded-xl">
					...
					<input type="text" password="" class="text-base text- h-80 w-full" v-model="password" :placeholder="$t('login.e_pdeal')" />
				</view>
				...
				<view class="mt-20 rounded-10 h-80 flex justify-center items-center text-white text-lg text-center bg-my-orange" @tap="mention">{{ $t('assets.mention') }}</view>
			</view>
