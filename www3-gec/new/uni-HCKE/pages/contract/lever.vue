		...
		<view class="text-black dark:text-white">
				...
				<view class="w-full flex flex-col p-40 border-0 border-solid border-b-2 border-gray-100 dark:border-gray-900">
					<view class="w-full flex justify-center">
						<view class="flex w-500 h-60 bg-gray-100 dark:bg-gray-600 dark:bg-opacity-30 rounded-2xl overflow-hidden">
							<view class="flex-1 flex justify-center items-center" :class="active==0?'bg-primary text-black':''" @tap="setActive(0)">{{$t('new.leverLes')}}</view>
							<view class="flex-1 flex justify-center items-center" :class="active==1?'bg-primary text-black':''" @click="setActive(1)">{{$t('new.leverAll')}}</view>
						</view>
					</view>
				...
				<view class="flex justify-between p-20 bg-white dark:bg-gray-600 dark:bg-opacity-30">
					<view class="w-3_5">
						<view class="overflow-hidden flex justify-between rounded-10 bg-gray-100 dark:bg-gray-600 dark:bg-opacity-30 dark:text-white">
							<view class="trade-type buy-type  text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'buy' ? 'bg-success text-white' : ''" @tap="tradeType('buy')">
								{{ $t('trade.buy') }}
							</view>
							<view class="trade-type sell-type text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'sell' ? 'bg-danger text-white' : ''" @tap="tradeType('sell')">
								{{ $t('trade.sell') }}
							</view>
						</view>
						...
						<view class="">
							<view class="flex alcenter around tc tab-box gray7 input-title">
								<view :class="['bd_input flex1 radius2 py-8',{'bggreen white bd_green': percent==item&&leverTradeType=='buy'},{'bgred white bd_red': percent==item&&leverTradeType=='sell'},{'ml10':index>0}]" v-for="(item,index) in percentList" :key='index' @click="changePercent(item)">{{item}}%</view>
							</view>
						</view>
						<view class="mt-20 text-sm">{{ $t('trade.balance') }}：{{ leverBalance | toFixed4 }} {{ quoteCurrencyCode }}</view>
						<view class="h-70 flex justify-center items-center mt-30 rounded-10" :class="leverTradeType == 'buy' ? 'bg-success' : 'bg-danger'" @tap="submits">
				...
				<view class="mt-20 p-20 bg-white dark:bg-gray-600 dark:bg-opacity-30 pb-120 min-h-half">
					...
					<view class="mt-40">
						<view class="flex justify-between">
							...
							<view class="flex-2 text-center">{{ $t('trade.num') }}</view>
							<view class="flex-2 text-center">{{ $t('bigTrade.profitRate') }}</view>
							<view class="flex-2 text-center">{{$t('new.profit_amount')}}</view>
						...
						<view class="" v-if="orderList.length > 0">
							<view v-for="(item, index) in orderList" :key="index" class="flex justify-between items-center mt-20">
								<view class="flex-1 text-left">{{ item.type_name }}</view>
								...
								<view class="flex-2 text-center">{{((parseFloat(item.profits)/parseFloat(item.caution_money))*100).toFixed(2)}}%</view>
								<view class="flex-2 text-center">{{ item.profits }}</view>
			...
			<!-- 
				<view v-if="active==0 && !soon" >
				 <view v-if="false" class="pt80"></view>
				<web-view :src="url"></web-view>
				</view>
			-->

			<second ref="second" v-show="active == 0"></second>
...
export default {
	...
	onHide() {
		if(this.active==0){
			this.$refs.second.hide()
		}
		this.$ws.unSub([`MARKET_DEPTH.${this.symbol}`,'DAY_MARKET','LEVER_TRADE'])
	...
	computed: {
		...
		maxMult(){
			...
			if (this.priceStatus == 0) {
				prices = this.priceValue
			} else {
				prices = this.newPrice
			}
			var that = this;
			...
			var totalPrice = parseFloat(pricesTotal * 1 * shareNum); //保证金
			var bonds = parseFloat((totalPrice - 0) / (that.leverOrder.mult - 0)).toFixed(4);
			var fees = parseFloat(totalPrice * that.leverOrder.fee).toFixed(4); //合约市值
			
			const handFee = parseFloat(bonds)+parseFloat(fees)
			
			return parseInt(parseFloat(this.leverBalance)/handFee)
			// return parseInt(parseFloat(this.leverBalance)/ ((parseFloat(prices)/(this.leverOrder.mult)*parseFloat(this.leverOrder.fee))+(parseFloat(prices)/(this.leverOrder.mult))))
		}
	},
	methods: {
		changePercent(percent){//切换百分比
				...
				this.leverOrder.share = parseInt(this.maxMult*(this.percent)/100)
				// var newNum;
		...
		setActive(val){
			...
			if(val==0){
				this.$ws.unSub([`MARKET_DEPTH.${this.symbol}`])
				...
				this.$ws.unSub([`MARKET_DEPTH.${this.symbol}`,'DAY_MARKET'])
				this.$ws.unRegisterCallBack(this.handleCallBack)
				this.$refs.second.hide()
				this.init()
			}
		...
		chart(){
			this.$refs.second.show({
				currency_match_id:this.currencyMatchId,
				symbol:this.symbol,
				decimal:this.currencyMatch?.decimal || 4,
		...
		init() {
			...
			if (!that.linkStatus) {
				if (uni.getStorageSync('leverOrder')) {
					...
					that.currencyMatchId = leverOrderData.currencyMatchId
				}
			}
			that.$utils.initDataToken({ url: 'market/currency_matches' }, res => {
				...
				if (res.length > 0) {
					...
					console.log(that.quotationList,'that.quotationList');
					
					if (that.quoteCurrencyId) {
						that.quoteCurrencyIds = that.quoteCurrencyId;
						const index = datas.findIndex((item)=> item.id ==that.quoteCurrencyId)
						if(index!=-1){
							const target = datas[index].matches.findIndex((item)=>item.id ==that.currencyMatchId)
							if(target!=-1){
								const itm = datas[index].matches[target]
								that.currencyMatch = itm
								that.$store.commit("setCurrencyMatch",that.currencyMatch);
								that.baseIndex = index;
								that.quoteCurrencyCode = itm.quote_currency_code
		...
		selectCurrency(options) {
			...
			if(this.currencyMatch.id == options.id){
				console.log('xxx');
			...
			this.currencyMatch = options
			this.$store.commit("setCurrencyMatch",this.currencyMatch);
			var that = this;
			...
			var data = {
				...
				currencyMatchId: that.currencyMatchId
			};
			that.url=`/hybrid/html/index.html?legal_id=${that.quoteCurrencyId}&currency_id=${that.baseCurrencyId}&symbol=${that.baseCurrencyCode + '/' + that.quoteCurrencyCode}&currency_match_id=${that.currencyMatchId}&type=lever`
		...
		linkKline() {
			var that = this;
			var symbols = that.baseCurrencyCode + '/' + that.quoteCurrencyCode;
			const url = `/h5/hybrid/html/index.html?legal_id=${that.quoteCurrencyId}&currency_id=${that.baseCurrencyId}&symbol=${symbols}&currency_match_id=${that.currencyMatchId}`
		...
		handicap() {
			...
			this.$ws.Sub([`MARKET_DEPTH.${this.symbol}`,'DAY_MARKET','LEVER_TRADE'])
			this.$ws.registerCallBack(this.handleCallBack)
		},
		handleCallBack(msg){
			const that = this;
			if (msg.type == 'MARKET_DEPTH') {
					this.buyList = msg.depth.in;
					this.sellList = msg.depth.out.reverse();
				}
				if (msg.type == 'DAY_MARKET') {
					if (this.currencyMatchId == msg.currency_match_id) {
						this.newPrice = msg.quotation.close;
					}
					this.quotationList.forEach((item, index) => {
						var datas = item.matches;
						datas.forEach((itm, inx) => {
							if(itm.id == msg.currency_match_id){
								itm.currency_quotation = msg.quotation
							}
						});
					});
				}
...
.dark{
  ...
  &::before {
    ...
    top: 0;
    bottom: 0;
    border-top: 30rpx solid #262f39;
    border-bottom: 30rpx solid #262f39;
  }
}
}
.light{
...
.buy-type {
  padding-right: 10rpx;
  border-radius: 6rpx 0px 0rpx 6rpx;
  &::after {
    right: 0;
