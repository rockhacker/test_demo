<template>
  <view class="home" :class="theme">
	  <view>
		...
		<uni-nav-bar fixed>
			<view class="absolute left-0 z-10 h-100">
				<view class="flex flex-1 relative h-full flex items-center justify-center px-20">
					<!-- <image @click="toPath('/pages/mine/mine')" class="w-40 h-40" src="/static/image/icon_wd.png"></image> -->
					<!-- <view class="flex items-center">
						<text v-if="info.uid">UID: {{info.uid}}</text>
					</view> -->
				</view>
			</view>
			<view class="flex-1 flex justify-center items-center text-2xl h-100 text-white bg-main">
				{{ $t("market.home") }}
			</view>
			<view class="absolute right-0 z-10 h-100">
				<!-- <image @click="toPath('/pages/subscription/index')" class="w-40 h-40 mt-30 mr-20" src="/static/image/icon_kfdh.png"></image> -->
				<view class="flex flex-1 relative h-full flex items-center justify-center px-20">
		...
		<view class="relative">
		  ...
		  :autoplay="true" :interval="3000" :circular="true" indicator-color="#aaa" indicator-active-color="#eee">
			<swiper-item v-for="(item, index) in bannerList" :key="index">
				...
				<!--涨幅榜-->
			<view class="mx-30 relative rounded-xl overflow-hidden">
				<swiper display-multiple-items="3" class="w-full h-200 bg-my-black" v-if="quoList.length > 2">
		  		<swiper-item class="flex items-center justify-around bg-my-gray" v-for="(item, i) in quoList" :key="i">
				<view @click="toUrl(item)" class="flex flex-col items-center bg-my-gray">
					<text class="text-sm">{{ item.base_currency_code }}/{{ item.quote_currency_code }}</text>
					<templatev v-if="!isOnline(item.market_time)">
						<text  class="w-200">{{$t('trade.soon')}}</text>
					</templatev>
					<template v-else>
			...
			<swiper class="w-full h-200 bg-my-gray" v-else>
			<swiper-item class="flex items-center justify-around" v-for="(item, i) in quoList" :key="i" >
				...
				<view @click="toUrl(item)" class="flex column items-center bg-my-gray">
					<text class="text-sm">{{ item.base_currency_code }}/{{ item.quote_currency_code }}</text>
					<templatev v-if="!isOnline(item.market_time)">
						<text  class="w-200">{{$t('trade.soon')}}</text>
					</templatev>
		...
		<!-- <view class="flex py-40 box-border items-center justify-center bg-white dark:bg-floor text-gray-400">
		...
		<view class="mx-30 bg-my-gray rounded-xl mt-30 pt-40 pb-20 flex flex-wrap">
			<view class="flex flex-col items-center mb-20" style="width:25%" @click="toPath('/pages/bigTrade/index')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_yqjl.png"></image>
				<text class="text-center">{{$t('bigTrade.bigTrade')}}</text>
			</view>
			<view class="flex flex-col items-center mb-20" style="width:25%" @click="toPath('/pages/subscription/index')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_fb.png"></image>
				<text class="text-center">{{$t('subscription.new_subscription')}}</text>
			</view>
			<view class="flex flex-col items-center" style="width:25%" @click="toPath('/pages/staking/index')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_lc.png"></image>
				<text class="text-center">{{$t('home.aiwk')}}</text>
			</view>
			<view class="flex flex-col items-center mb-20" style="width:25%" @click="toPath('/pages/legal/legal')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_ccd.png"></image>
				<text class="text-center">{{$t('home.legal')}}</text>
			</view>
			<!-- <view class="flex flex-col items-center mb-20" style="width:25%" @click="toPath('/pages/home/turntable')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_hyjy.png"></image>
				<text class="text-center">{{$t('home.qy')}}</text>
			</view> -->
			<view class="flex flex-col items-center mb-20" style="width:25%" @click="toPath('/pages/contract/lever')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_hyjy.png"></image>
				<text class="text-center">{{$t('home.lever')}}</text>
			</view>
			<view class="flex flex-col items-center" style="width:25%" @click="toPath('/pages/mine/change_history')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_cbjl.png"></image>
				<text class="text-center">{{$t('assets.zbjl')}}</text>
			</view>
			<view class="flex flex-col items-center" style="width:25%" @click="toPath('/pages/mine/real_authentication')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_smrz.png"></image>
				<text class="text-center">{{$t('new.verified')}}</text>
			</view>
			<view class="flex flex-col items-center" style="width:25%" @click="toPath('/pages/home/chatBtn')">
				<image class="w-100 h-100 mb-20" src="/static/image/icon_bzzx.png"></image>
				<text class="text-center">{{$t('new.lianxikefu')}}</text>
			</view>
		</view>

		<view class="flex mt-30">
			<!-- <view @click="toPath('/pages/home/recharge')" class="flex w-350 h-200 bg-contain items-center pl-30 pr-40 bg-no-repeat" style="background:url(./static/image/bg_kscz.png)">
				<view class="flex flex-1 flex-col">
					<view class="font-bold text-lg">{{ $t("new.recharge") }}</view>
					<text class="pt-10 text-sm mt-20">{{ $t("new.clickre") }}</text>
				</view>
				<image class="w-120 h-120" src="/static/image/icon_kscz.png"></image>
			</view>
			<view @click="toPath('/pages/mine/withdraw')" class="flex w-350 h-200 bg-contain items-center pl-50 pr-20 bg-no-repeat" style="background:url(./static/image/bg_kstb.png)">
				<view class="flex flex-1 flex-col">
					<view class="font-bold text-lg">{{ $t("new.withdrawal") }}</view>
					<text class="pt-10 text-sm mt-20">{{ $t("new.clickwit") }}</text>
				</view>
				<image class="w-120 h-120" src="/static/image/icon_kstb.png"></image>
			</view> -->
		</view>
		...
		<!--行情-->
		<view class="pb-100 w-full bg-my-gray">
		  ...
		  <view class="flex px-40 pt-20">
			<view class="trading-item text-center mr-40 " v-for="(item, index) in allList" :key="index" @click="setItem(item)">
			  <view class="pb-10 text-lg" :class="itemCode == item.code ? 'text-primary' : ''">{{ item.code }}</view>
		  ...
		  <view @click="toUrl(item)" class="flex items-center text-sm text-gray-400 tableitem px-40 py-30" v-for="(item, i) in tabList" :key="i">
			<view class="flex items-center" style="flex: 1.5">
				...
				<view class="text-sm"><text class="text-2xl text-black dark:text-white">{{ item.base_currency_code }}</text>/{{ item.quote_currency_code }}</view>
				<!-- <view class="text-sm">24H量 {{item.volume}}</view> -->
			</view>
			<view v-if="!isOnline(item.market_time)" class="w-200">{{$t('trade.soon')}}</view>
			<template v-else>
				<view class="flex-1 text-white text-xl text-gray-400" v-if="item.currency_quotation">{{ parseFloat(item.currency_quotation.close) }}</view>
				<view class="flex-1 text-right flex justify-end" v-if="item.currency_quotation">
				  <view class="w-140 h-60 flex justify-center items-center rounded-8 text-center text-white" :class="{ 'bg-danger': parseFloat(item.currency_quotation.change) < 0, 'bg-success':parseFloat(item.currency_quotation.change) >= 0 ||item.currency_quotation.change == ''}">
					{{ item.currency_quotation.change.substr(0, 1) == "-" ? "" : "+"}}{{ (item.currency_quotation.change - 0) | toFixed2 }}%
				  </view>
				</view>
			</template>
		...
		<uni-popup :show="announcementModal.showModal" @hidePopup="hideBtn" :msg="announcementModal.title" :btnShow="announcementModal.tipbtnShow" :lang="lang">
			<!-- <text class="w-full p-40" v-html="announcementModal.content">
			 ...
			 <view class="w-full p-40 h-700 overflow-auto" v-html="announcementModal.content"></view>
		</uni-popup>
	</view>
  </view>
</template>
...
export default {
	...
	methods: {
			...
			try {
				...
				that.$utils.initDataToken({
					...
					if (res.data.length && res.data[0].content) {
						...
						console.log(this.announcementModal,'ss');
					}
				});
			} catch (e) {
				console.log(e);
		...
		topo(){
			...
			window.open(`https://${this.$utils.domain}/wp/MUT-${lang}.pdf`);
		...
		toPath(url){
			// if(url == '/pages/home/chat'){
			// 	return
			// }
			if(url=='/pages/bigTrade/index'||url=='/pages/contract/lever' || url=='/pages/assets/assets' || url=='/pages/financial/financial'){
      ...
      if(this.theme=='light'){
		...
		gotrade(){
			...
			this.$utils.initDataToken({ url: 'user/getUSDTAddress', data: {}, type: 'get' }, (res, msg) => {
