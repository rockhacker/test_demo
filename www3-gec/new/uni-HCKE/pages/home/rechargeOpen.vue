<template>
	<view :class="theme" class="rechargeOpen">
         <view class="flex justify-center items-center flex-col p-30">
			  ...
			  <button class="w-full rounded-3xl bg-my-orange text-white text-xl" @click="setCopy">{{$t('feed.copyadd')}}</button>
			  <view class="open-form w-full">
			       <text class="text">{{$t('feed.depositsAddress')}}</text>
			       <input class="uni-input bg-my-gray text-white"  v-model="depositsAddress" :placeholder="$t('feed.depositsAddressUp')"/>
				   <text class="text">{{$t('feed.deposits')}}</text>
				   <input class="uni-input bg-my-gray text-white"  v-model="deposits" :placeholder="$t('feed.depositsUp')"/>
			      <!-- <text class="text">chain address</text>
			        ...
			        <view class="bd_dashed radius4 tc mb10" @tap="uploadImg(1)">
						 <view v-if="!hasUp1">
							 <image :src="img" class="wt80 h80 mx-auto" ></image>
						 </view>
						 <image :src="img1" class="w95" mode="widthFix" v-else style="max-height: 100px;"></image>
					</view>
			        <view class="upload-text text-center">{{$t('feed.pictures')}}</view>
			  </view>
			  <button class="w-full rounded-3xl bg-my-orange text-white text-xl" @click="submit">{{$t('feed.submit_transaction_screenshot')}}</button>
