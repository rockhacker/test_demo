<template>
	<view class="bg-black" :class="theme" style="min-height:100%">
			...
			<view class="bg-black">
				<view class="flex justify-between items-center w-full py-30 px-30">
					<view class="flex items-baseline justify-center font-bold">
						<view class="mr-30" :class="Type=='sell'?'text-3xl':'text-gray-500 text-lg'" @click="changetype('sell')">{{$t('legal.ibuy')}}</view>
						<view :class="Type=='buy'?'text-3xl':'text-gray-500 text-lg'" @click="changetype('buy')">{{$t('legal.isell')}}</view>
					</view>
				...
				<view class="w-full border-0 border-b-2 border-solid border-gray-200 dark:border-gray-700">
					<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
						<view class="w-140 h-100 flex-col justify-center items-center text-lg inline-flex rounded-none" 
							...
							:class="{'text-primary dark:text-primary':coinindex==index}" v-for="(item,index) in legal_list" :key="index" 
				...
				<view class="gray75 flex column w100 plr20 ptb10 bdb27" v-for="item in orderlist" :key="item.id">
					<view class="flex justify-between mt5">
						<view class=" flex items-center">
							<view class="logo bgBlue2 ft12 flex items-center jscenter white" v-if="item.seller_name">{{item.seller_name || '' | chart0}}</view>
					...
					<view class="flex justify-between mt5">
						...
						<view class="flex items-center">
							<text class="blue21 ft16">{{item.price | toFixed2}}{{item.area_currency}}</text>
						</view>
					</view>
					<view class="flex justify-between mt5">
						<view class="flex items-center">
						...
						<view class="flex items-center jscenter h25 plr20 bgBlue2 radius2" @click="getinfo(item)">
				...
				<view class="layer_opeation bgHeader" @click.stop>
					...
					<view class="mtb10 tab flex column plr15">
						<view class="num_price flex items-center gray75 ft14">
							...
							<view v-if="lang!='vn'" @click="msg=1" :class="{'cur':msg==1}">{{$t('legal.annum')}}{{Type=='sell'?$t('legal.buy'):$t('legal.sell')}}</view>
							<view v-if="lang=='vn' && Type=='sell'" @click="msg=1" :class="{'cur':msg==1}">Theo số lượng mua</view>
							<view v-if="lang=='vn' && Type!='sell'" @click="msg=1" :class="{'cur':msg==1}">Theo số lượng bán</view>
						</view>
						...
						<view class="flex justify-between items-center Ipt ft13 pl10 mt10 bd3a" v-if="msg==1">
							<input v-if="lang!='vn'" type="number" class="flex1  h25" :placeholder="$t('legal.pl') + Type=='sell'?$t('legal.sell'):$t('legal.buy')+$t('legal.num')" value="" v-model='number' />
							<input v-if="lang=='vn' && Type=='sell'"  type="number" class="flex1  h25" placeholder="Số lượng" value="" v-model='number' />
							<input v-if="lang=='vn' && Type!='sell'"  type="number" class="flex1  h25" placeholder="Số lượng" value="" v-model='number' />
							<view class="ptb10 flex w40 jscenter w40">
								<text class="white coin_code flex1 pr10 tr bdr_input" v-if="legal_list.length && false">{{legal_list[coinindex].code}}</text>
								<view class="flex items-center bdr_input">
									<text class="white coin_code flex1 tr" @click="changeCoinType">{{getOptions[codeType]}}</text>
									<image src="/static/image/mores.png" style="transform:rotate(90deg)" class="wt20 h20 p-10"></image>
								</view>
								<text v-if="lang!='vn'" class="plr10 blue21 tc" @click="all(1)">{{$t('trade.all')}}{{Type=='sell'?$t('trade.buy'):$t('trade.sell')}}</text>
								<text v-if="lang=='vn' && Type=='sell'" class="plr10 blue21 tc" @click="all(1)">Mua tất cả</text>
								<text v-if="lang=='vn' && Type!='sell'" class="plr10 blue21 tc" @click="all(1)">Bán tất cả</text>
							</view>
						</view>
						<view class="gray75 mt5">
							<text>{{$t('legal.limit')}}</text>
							<text class="limit pl5" v-if="legal_list.length && false">{{Data.min_number}} {{legal_list[coinindex].code}}-{{Data.max_number}} {{legal_list[coinindex].code}}</text>
							<text  class="limit pl5"  v-if="legal_list.length">{{(Data.min_number*Data.price|toFixed2)}}-{{(Data.max_number*Data.price) | toFixed2 }}  {{Data.area_currency}}</text>
							</view>
						<view class="ft14 flex items-center justify-between mt10">
							<text class="gray75">{{$t('legal.allmoney')}}</text>
							<text v-show="codeType==0" class="blue21 ft16 total">{{totalPrice | toFixed2}} {{Data.area_currency}}</text>
							<text v-show="codeType==1" class="blue21 ft16 total">{{number || 0}} {{Data.area_currency}}</text>
...
<script>
	...
	export default {
		...
		computed:{
			...
			getOptions(){
				return [
					this.legal_list[this.coinindex].code,
					this.Data.area_currency
				]
			},
			...mapState(['theme']),
		},
		...
		methods: {
			changeCoinType(){
				...
				const that = this;
				uni.showActionSheet({
					itemList: this.getOptions,
					success: function (res) {
						that.codeType = res.tapIndex
			...
			all(msg) {
				...
				if (msg == 0) {
					...
					that.number = that.codeType==0? Number(that.Data.max_number): Number(this.Data.max_number*this.Data.price).toFixed(2);
				}
			},
			isshow() {
				let that = this;
			...
			pay(id) {				
				...
				that.box1 = false;
				that.$utils.initDataToken({ url: 'otc/match_master',data:{master_id:that.id,price:that.Data.price,number:value,type:that.codeType},type:'post' }, (res,msg)=> {
