	...
	<view :class="theme">
		<view class="">
			...
			<view class="flex items-center justify-between text-sm border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700">
				<view class="px-40 py-20" :class="status == item.id ? 'text-my-orange border-0 border-b-2 border-solid border-my-orange' : ''" v-for="item in tabs" :key="item.id" @tap="selectType(item.id)">{{ item.texts }}</view>
			</view>
			
			<view class="px-40">
				<view v-for="(item, index) in orderList" :key="index" class="bg-my-gray rounded-xl p-30 mt-30">
					...
					<view class="flex text-right mt-30 justify-end" v-if="status == 0 || status == 1">
						<view class="bg-my-orange px-30 py-20 rounded-8 text-white" @tap="loss(item,index)">{{$t('lever.setys')}}</view>
						<view class="bg-my-orange px-30 py-20 rounded-8 text-white ml-20" @tap="closes(item.id, index)">{{ status == 0 ? $t('trade.chedan') : $t('trade.pingcang') }}</view>
					</view>
				</view>
			</view>
