<template>
	<view :class="theme">
		<uni-nav-bar backIcon fixed>
			<view class="flex justify-between items-center py-20 pr-20 w-full bg-main">
				<view>
				...
				<view class="bg-my-orange text-white py-20 text-center rounded-10 w-200" @tap="allClose.shows=true">{{$t('lever.yijian')}}</view>
			</view>
		</uni-nav-bar>
		
		<view class="list text-xl px-30 bg-my-black mt-20" style="min-height: calc(100vh - 70px - var(--status-bar-height));">
			...
			<template v-else>
				<view v-for="(item, index) in orderList" :key="index" class="p-30 bg-my-gray rounded-lg mb-30 ">
					...
					<view class="flex text-right mt-30 justify-end">
						<view class="bg-my-orange px-40 py-20 rounded-8 text-white" @tap="loss(item,index)">{{$t('lever.setys')}}</view>
						<view class="bg-my-orange px-40 py-20 rounded-8 text-white ml-20" @tap="closes(item.id,index)">{{$t('trade.pingcang')}}</view>
					</view>
				</view>
			</template>
		</view>
