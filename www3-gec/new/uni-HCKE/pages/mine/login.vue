			...
			<view class="py-40 px-40">
				...
				<view class="my-20">
				  <view class="flex items-center flex1 border-2 border-solid px-20 border-my-orange rounded-xl">
					...
					<input type="text" v-model="password" class="h-100 flex-1" v-if="showPass == true"  :placeholder="$t('login').p_pwd"/>
					<input type="text" password="" v-model="password" class="h-100 flex-1" v-else  :placeholder="$t('login').p_pwd" />
					<image src="/static/image/a.png" class="w-40 h-40" v-if="showPass == true" @click="showPass=false"></image>
					<image src="/static/image/b.png" class="w-40 h-40" v-else @click="showPass=true"></image>
				...
				<view class="my-60 bg-my-orange rounded-3xl px-20 text-white text-lg text-center mt-30" style="height:88rpx;line-height: 88rpx;" @tap="toWallet" hover-class="opacity-50">{{ $t("wallet.login") }}</view>
