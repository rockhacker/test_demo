			...
			<view class="bg-white mt-10 flex-1">
					...
					<view class="w-full flex justify-center p-40">
						<view class="w-full flex flex-col p-40 rounded-10 bg-gray-100">
							...
							<view class="flex alcenter between mb30">
								...
								<view class="text-black text-xl">{{staking.lock_dividend_days}}</view>
							</view>
							<view class="flex alcenter between mb30">
				...
				<!-- 			<view class="flex alcenter between mb30">
							...
							<view class="flex items-center border-0 border-b-2 border-solid border-gray-300">
								<input type="text" v-model="amount" class="h-100 flex-1 text-black" :placeholder="$t('staking.suocangshuliang')" />
							</view>
							
							<view class="mt-20 rounded-3xl p-20 text-black text-sm text-center bg-primary my-40" hover-class="opacity-50" @tap="buyProduct">{{ $t("staking.shengou") }}</view>
							<view class="text-xl mb20 text-black">{{$t('staking.chanpinjieshao')}}</view>
							<view class="flex1 text-black">{{staking.desc}}</view>
						</view>
					</view>
