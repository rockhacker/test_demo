	...
	<view :class="theme">
		<uni-nav-bar fixed backIcon>
			...
			<slot name="right">
				...
				<navigator url="./orders" class="px-10 py-10 border-solid border-2 border-my-orange rounded-10 absolute top-24 right-20 text-yellow-500 z-10 dark:border-primary dark:text-primary" >{{$t('subscription.my_subscription')}}</navigator>
			</slot>
		</uni-nav-bar>
			
		<view class="p-30" v-if="list.length">
			<navigator class="rounded-10 bg-my-gray p-20 mb-20" :url="`./details?id=${item.id}`" hover-class="opacity-50" v-for="(item,index) in list" :key="index">
				<view class="rounded-10">
					<view class="flex justify-between items-center mb-10">
						...
						<view class="text-sm rounded-8 p-20 text-white bg-my-orange">
