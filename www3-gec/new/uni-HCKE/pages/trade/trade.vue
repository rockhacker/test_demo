<template>
	<view :class="theme">
		...
		<!-- 头部 -->
		<view class="header fixed flex alcenter jscenter plr15 zdx100 bg-main" style="height: 88rpx;">
			<view class="flex alcenter jscenter" @click="showLeft=true">
				<!-- <image src="/static/image/list.png" class="h20 wt20"></image> -->
				<text class="ft17 ml10 bold">{{currency_name}}/{{legal_name}}</text>
		...
		<template v-else>
			<view class="flex pt70 pb15 bg-my-black">
				<!-- 左侧 -->
				<view class="flex1 plr10">
					<view class="flex alcenter tc overxy   ">
						<view class="flex1 overflow-hidden relative" style="border-radius: 8rpx 0 0 8rpx;" @click="changeType('buy')">
							<view class="p-10 relative right-12" style="transform: skew(-30deg, 0deg);" :style="{background:type=='buy'?'#12b886':'rgba(79, 189, 53,0.2)'}">
								<view style="transform: skew(30deg, 0deg);">{{$t('trade').buy}}</view>
							</view>
						</view>
						<view class="flex1 overflow-hidden relative" style="border-radius: 8rpx 0rpx 0rpx 8rpx;transform: rotateY(180deg);" @click="changeType('sell')">
							<view class="p-10 relative right-12" style="transform: skew(30deg, 0deg);" :style="{background:type=='sell'?'#EA5F48':'rgba(234, 95, 72,0.2)'}">
								<view style="transform: skew(-30deg, 0deg)">
									<view style="transform:rotateY(180deg);">{{$t('trade').sell}}</view>
								</view>
							</view>
						</view>
						<!-- <view class="bor abstrot mt5 bgPart"></view> -->
						<!-- <view :class="['flex1  flex-after ft17 border-2 border-solid rounded-xl h-60 leading-56 ml-12 border-gray-500',{'border-my-orange':type=='sell'}]" @click="changeType('sell')">{{$t('trade').sell}}</view> -->
					</view>
					<view class="flex items-center justify-between">
						<view class="flex alcenter ptb15 ft14">
						<text class="" @click="togglePopup('bottom-share')" data-position="bottom">{{priceType=='limit' ? $t('trade').limit : $t('trade').shi}}</text>
						<image src="/static/image/wegfwe.png" class="h10 wt10 ml5 b-icon"></image>
						</view>
						<navigator :url="'/pages/market/kline?legal_id='+legal_id+'&currency_id='+currency_id+'&symbol='+currency_name+'/'+legal_name+'&currency_match_id='+match_id+'&from=trade'">
					...
					<uni-popup :show="popType === 'bottom-share'" position="bottom" @hidePopup="togglePopup('')">
						<view class="bottom-content safe-pb" >
					...
					<view class="">
						<view class="" v-show="priceType=='limit'">
							<view class="bd_input rounded-lg input-pd" style="border-color:#8A8A8A">
								<view class="input-title text-white">{{$t('trade.price')}}</view>
								<input type="number" v-model='price' :placeholder="$t('store.p_price')" class="h40 flex1 ft14" />
						...
						<view class="bd_input rounded-lg mt15 plr10 radius2 input-pd bdb_blue3" style="border-color:#8A8A8A">
							<view class="input-title">
								<text class="text-white">{{$t('trade.num')}}</text>
							</view>
							<input type="number" v-model="number" :placeholder="$t('trade.p_num')" class="h40 flex1" @input="inputNum" />
						...
						<view class="gray7 input-title ft12">{{$t('trade.use')}} {{balance}} {{type=='buy' ? legal_name :currency_name}} </view>
						...
						<view class="gray7 flex alcenter mt10 input-title ft12">
							<text>{{$t('trade.amout')}}</text>
							<text class="nwhite pl5 input-title">{{tradeNum || '--'}}{{legal_name}}</text>
							
						</view>
						<view class="gray7 flex alcenter mt10 input-title ft12">
							<text>{{$t('trade.fee')}}:{{parseFloat(fee)}}%</text>
						</view>
						<view style="visibility: hidden;" :class="['btn-default mt10 bgred',{'bggreen':type=='buy'}]" @click="submit">{{type=='buy'?$t('trade.buy'):$t('trade.sell')}}{{currency_name}}</view>
					</view>
				</view>
				<!-- 盘口 -->
				<view class="trade-left flex column border-2 border-solid mx-10 px-10 rounded-8" style="border-color:#424242">
					<view class="flex between  pr20 ft14 pt5 pb20">
					...
					<view class="ft14 flex flex1 mt5 column">
						<view class="flex1 flex column jsend asks" v-if="ptype!=1"><!-- 取最后的五条记录 -->
							<view class="flex ft14 between hg20 alcenter mtn gray7 pr20" v-if="ptype==0 ? (sellData.length>5 ? (index>(sellData.length-6)&&index<10) : index<5) : index<10" :style="'background-size:'+parseInt((item.amount/sellMax)*100)+'% 100%'" :key="index" v-for="(item,index) in sellData" @click="changeNum(item.price)">
						...
						<view class="ptb5">
							<view :class="['redColor ft14 flex between mr15',{'greenColor':change.substr(0,1)!='-'}]">
								<text>{{parseFloat(last_price)}}</text>
								<text>{{parseFloat(last_price)}}</text>
							</view>
							<!-- <text class="gray4">≈{{last_price * cny| toFixed2}}CNY</text> -->
						</view>
						<view class="flex1 flex column bids" v-if="ptype!=2">
							<view class="flex ft14 between hg20 alcenter mtn gray7 pr20" v-if="ptype==0 ? index<5 : index<10" :style="'background-size:'+parseInt((item.amount/buyMax)*100)+'% 100%'"  :key="index" v-for="(item,index) in buyData" @click="changeNum(item.price)">
			...
			<view class="bg-my-black pb-100">
		    <view class="flex between flex-btn">
				<!-- :class="['btn-default mt10 bgred',{'bggreen':type=='buy'}]"  -->
				<view class="btn-default rounded-3xl" :style="{background:type=='buy'?'#12b886':'#EA5F48'}" @click="submit">{{type=='buy'?$t('trade.buy'):$t('trade.sell')}}{{currency_name}}</view>
				<view class="btn-text text-my-orange" @click="toDeal">{{$t('new.deal')}} ></view>
			</view>
			<view class="flex alcenter between ptb10  plr15 footer">
				<view class="flex flexend">
					<text class="ft18 text-my-orange">{{$t('trade.delegate')}}</text>
				...
				<view class="flex alcenter gray7">
					...
					<navigator url="entrust">{{$t('new.newAll')}}</navigator>
				</view>
			</view>
			<!-- 委托类型切换弹窗 -->
		...
		<view class="left-mode">
			<uni-drawer :visible="showLeft" style="background: rgb(16, 32, 48,.5);" mode="left" @close="showLeft = false">
				<view class="flex column ht100 bg-my-black">
					<view class="pt20">
						<view class="ft18 pl15 ptb10">{{$t('trade.bibi')}}</view>
						<view class="flex plr15 bdb_1e bold gray7">
							<view :class="['mr20 pb5 ft14',{'text-my-orange bdb_active border-my-orange':current==index}]" v-for="(item,index) in quotationList" :key="item.id" @click="changeLegal(index,item.code)">{{item.code}}</view>
						</view>
					</view>
					<view class="flex1 overyscroll" v-for="(market,index) in marketList" :key="index" v-if="current==index">
						<view :class="['flex alcenter between ptb15 plr15  bdb_1e',{' bg-gray-800':legal_id==item.quote_currency_id && currency_id==item.base_currency_id}]" :key="idx" v-for="(item,idx) in market" v-if="item.open_change==1" @click="changeCurrency(idx)">
							<view class="flex flexend">
	...
	export default {
		...
		computed:{
			...
			tradeNum(){//交易额 单价*数量
				...
				return parseFloat(this.price*this.number) || '0.0000000';
			},
			buyMax(){//买盘最大数量
		...
		onShow() {
			...
			const type =  uni.getStorageSync('tradeType')
		...
		onHide() {
			...
			this.$ws.unSub([`MARKET_DEPTH.${symbol}`,'DAY_MARKET','TX_MATCHED'])
			// if(this.timer){
		...
		linkKline() {
			var that = this;
			var symbols = this.currency_name+'/'+this.legal_name;
			uni.navigateTo({
				url: '/pages/market/kline?from=trade&legal_id=' + that.legal_id + '&currency_id=' + that.quoteCurrencyId + '&symbol=' + symbols +'&currency_match_id='+that.match_id
			});
		},
			toDeal(){
				uni.navigateTo({
			...
			changeCurrency(idx){//交易对切换
				if(this.marketList[this.current][idx].id == this.match_id){
					return
				}
				...
				this.fee = data.change_fee_rate*100;
				this.soon = data.base_currency_code=='ETHFXX' && !this.isOnline(data.market_time)
				...
				that.$utils.initData({ url: 'market/currency_matches' }, res => {
					if (res.length > 0) {
						...
						if(localData){
							...
							that.fee = arrFrist.change_fee_rate*100;
			...
			socketData(){//socket连接
				...
				this.$ws.Sub([`MARKET_DEPTH.${symbol}`,'DAY_MARKET','TX_MATCHED'])
