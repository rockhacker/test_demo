<template>
	<view :class="theme">
		<view class="text-black dark:text-white">
			...
			<uni-nav-bar fixed>
				<view class="w-full h-100 flex justify-center items-center  text-center px-40 bg-main">
					<view class="flex h-60 justify-center items-center flex-1 rounded-10" :class="{'bg-my-orange text-white':index==tabIndex}" hover-class="opacity-50"
					 v-for="(item,index) in wallet_data" :key="index" @click="tapTab(index)">{{item.name}}</view>
				</view>
			</uni-nav-bar>
			
			<swiper :current="tabIndex" :duration="300" @change="changeTab" style="min-height: calc(100vh - 50px - var(--status-bar-height));">
				<swiper-item v-for="(item,index) in wallet_data" :key="index">
					<scroll-view scroll-y class="w-full h-full">
						<view class="py-30 px-40 text-xl text-gray-400 border-0 border-b-2 border-solid border-gray-200 dark:border-gray-900">
						...
						<view class="pt-40 px-20 pb-100">
							<navigator class="justify-between rounded-10 bg-my-gray px-20 py-30 mb-30" v-for="changeItem,indexs in wallet_data[index].accounts" :key="indexs"
							:url="'tradeAccount?id='+wallet_data[index].id+'&titleName='+wallet_data[index].name+'&type='+wallet_data[index].account_code+'&account_id='+changeItem.id+'&currency_id='+changeItem.currency_id">
								...
								<view class="flex justify-between">
									<view class="text-xl font-bold">{{changeItem.currency_code}}</view>
