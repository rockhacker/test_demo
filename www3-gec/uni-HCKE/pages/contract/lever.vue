<template>
	<view :class="theme">
		<view class="text-black dark:text-white">
				...
				<view class="w-full flex flex-col p-40 border-0 border-solid border-b-2 border-gray-100 dark:border-gray-900">
					<view class="w-full flex justify-center">
						<view class="flex w-500 h-60 bg-gray-100 dark:bg-gray-600 dark:bg-opacity-30 rounded-2xl overflow-hidden">
							<view class="flex-1 flex justify-center items-center" :class="active==0?'bg-my-orange':''" @tap="setActive(0)">{{$t('new.leverLes')}}</view>
							<view class="flex-1 flex justify-center items-center" :class="active==1?'bg-my-orange':''" @click="setActive(1)">{{$t('new.leverAll')}}</view>
						</view>
					</view>
			...
			<view v-if="active==1 && !soon" class="min-h-screen bg-my-black">
				<!-- <view class="flex justify-center items-center text-3xl h-600" v-if="soon">{{$t('trade.soon')}}</view> -->
				<view class="flex justify-between p-20">
					<view class="w-3_5">
						<view class="flex alcenter tc overxy   ">
							<view class="flex1 overflow-hidden relative" style="border-radius: 8rpx 0 0 8rpx;"  @tap="tradeType('buy')">
								<view class="p-12 relative right-12" style="transform: skew(-30deg, 0deg)" :style="{background:leverTradeType=='buy'?'#4FBD35':'rgba(79, 189, 53,0.2)'}">
									<view style="transform: skew(30deg, 0deg);">{{$t('trade').buy}}</view>
								</view>
							</view>
							<view class="faterclass flex1 overflow-hidden relative" style="border-radius: 8rpx 0rpx 0rpx 8rpx;transform: rotateY(180deg);" @tap="tradeType('sell')">
								<view class="p-12 relative right-12" style="transform: skew(30deg, 0deg);" :style="{background:leverTradeType=='sell'?'#EA5F48':'rgba(234, 95, 72,0.2)'}">
						...
						<view class="h-70 flex justify-center items-center mt-30 rounded-10" :style="{background:leverTradeType=='buy'?'#4FBD35':'#EA5F48'}"  @tap="submits">
							<view class="flex-1 text-center text-white text-xl">{{ leverTradeType == 'buy' ? $t('trade.buy_duo') : $t('trade.sell_kong') }}</view>
						</view>
					</view>
					<view class="w-2_5 flex flex-col ml-20 border-2 border-solid p-10 rounded-8" style="border-color:#424242">
				...
				<!-- 当前委托列表 -->
				<view class="mt-20 p-20 bg-my-black pb-120 min-h-half">
					<view class="flex justify-between">
					...
					<view class="mt-40">
						<view class="flex justify-between px-14">
						...
						<view class="" v-if="orderList.length > 0">
							<view v-for="(item, index) in orderList" :key="index" class="flex justify-between items-center mt-20 bg-my-gray p-14 rounded-8">
								...
								<view class="flex-1 text-center bg-my-orange py-10 rounded-8 ml-20 text-white" @tap="closeTrade(item.id)">{{ $t('trade.pingcang') }}</view>
							</view>
						</view>
					...
					<view class="pb-60">
						<view class="border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 text-black dark:text-white" v-for="(item, index) in quotationList[quoteIndex].matches" :key="index" v-if="item.open_lever == 1">
							...
							>
								...
								<view v-if="!isOnline(item.market_time)">{{$t('trade.soon')}}</view>
			...
			<uni-popup :show="mults.shows" @hidePopup="hideBtn" :type="mults.types" :btnShow="false" :bgColor="false" :lang="langs">
				<view class="w-full bg-white dark:bg-floor text-black dark:text-white fixed bottom-0 left-0 pb-150">
...
export default {
	...
	methods: {
			...
			that.$utils.initDataToken({ url: 'market/currency_matches' }, res => {
				...
				if (res.length > 0) {
					...
					if (that.quoteCurrencyId) {
								...
								if (lists.length > 0) {
									lists.forEach(function(itm, inx) {
										if (that.baseCurrencyId == itm.base_currency_id) {
											...
											that.soon = !that.isOnline(itm.market_time)
						...
						that.soon =  !this.isOnline(list[0].market_time)
		...
		selectCurrency(options) {
			...
			that.soon = !this.isOnline(options.market_time)
