			...
			<uni-nav-bar backIcon :left-text="$t('legal.list')" >
				<view class="flex-1 flex justify-end relative" slot="right">
					...
					<view class="absolute right-0 top-100 w-screen" v-show="show">
						<view class="bg-black relative z-20">
							<view class="p-30">
								<view class="text-lg mb-20">{{$t('trade.dealtype')}}</view>
								<view class="flex mb-30">
									<view class="w-200 h-60 flex justify-center items-center mr-40 rounded-10 text-center" :class="way=='BUY'?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changetype('BUY')">{{$t('legal.buy')}}</view>
									<view class="w-200 h-60 flex justify-center items-center mr-40 rounded-10 text-center" :class="way=='SELL'?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changetype('SELL')">{{$t('legal.sell')}}</view>
								</view>
								<view class="text-lg mb-20">{{$t('store.orderstatus')}}</view>
								<view class="flex mb-20">
									<view class="w-200 h-60 flex justify-center items-center mr-30 rounded-10 text-center" :class="status===0?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changestatu(0)">{{$t('store.notdone')}}</view>
									<view class="w-200 h-60 flex justify-center items-center mr-30 rounded-10 text-center" :class="status===1?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changestatu(1)">{{$t('trade.has_pay')}}</view>
									<view class="w-200 h-60 flex justify-center items-center mr-30 rounded-10 text-center" :class="status===2?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changestatu(2)">{{$t('store.yanqi')}}</view>
								</view>
								<view class="flex mb-20">
									<view class="w-200 h-60 flex justify-center items-center mr-30 rounded-10 text-center" :class="status===3?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changestatu(3)">{{$t('store.protection')}}</view>
									<view class="w-200 h-60 flex justify-center items-center mr-30 rounded-10 text-center" :class="status===4?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changestatu(4)">{{$t('trade.has_cancel')}}</view>
									<view class="w-200 h-60 flex justify-center items-center mr-30 rounded-10 text-center" :class="status===5?'bg-primary text-white':'bg-gray-100 dark:bg-gray-700'" @click="changestatu(5)">{{$t('store.done')}}</view>
								</view>
							</view>
							<view class="flex border-0 border-t-2 border-solid border-gray-200 dark:border-gray-700">
								<view class="flex-1 h-80 flex justify-center items-center text-lg bg-main" @click="reset()">{{$t('store.chongzhi')}}</view>
								<view class="flex-1 h-80 flex justify-center items-center bg-primary text-white text-lg" @click="confirm">{{$t('login.e_confrim2')}}</view>
							</view>
						</view>
					...
					<view class="gray75 flex column w100 plr15 ptb15 bdb27" v-for="(item,index) in orderlist" :key="index" @click="goDetail(item.id,item.is_sure)">
						...
						<view class="flex between mt10">
							...
							<view class="flex column flexstart">
								...
								<text class="b7c ft13 mt5">{{item.number | filterDecimals(4)}}</text>
							</view>
							<view class="flex column flexend">
								<text v-if="item.area_info">{{$t('legal.allmoney')}}({{item.area_info.currency}})</text>
								<text class="b7c ft13 mt5">{{item.amount | filterDecimals(4)}}</text>
							</view>
						</view>
