<template>
	<view class="bg-my-black">
		<view class="flex flexend between plr15 ptb10 w100 bg-my-black fixed lf0 zdx100">
			<view class="flex flexend">
				<view :class="['ft14 text-white',{'ft18 text-my-orange':current==0}]" @click="change(0)">{{$t('trade.all_delegate')}}</view>
			    <view :class="['pl10 ft14 text-white',{'ft18 text-my-orange':current==1}]" @click="change(1)">{{$t('trade.his_delegate')}}</view>
			</view>
			<view class="flex alcenter ml10 ft14" v-show="current==0">
				<text class="text-white" @click="togglePopup('bottom-entrust')" data-position="bottom">{{enType=='in' ? $t('trade.buy') : $t('trade.sell')}}</text>
		...
		<uni-popup :show="popType === 'bottom-entrust'" position="bottom" @hidePopup="togglePopup('')">
			<view class="bottom-content">
				<view class="bottom-content-box tc gray7">
				    <view :class="['bdb_1e ptb10 ft14 bg-my-gray text-white',{'text-my-orange':enType=='in'}]" @click="changeEntrustType('in')">{{$t('trade.buy')}}</view>
					<view :class="['ptb10 ft14 bg-my-gray text-white',{'text-my-orange':enType=='out'}]" @click="changeEntrustType('out')">{{$t('trade.sell')}}</view>
		            <view class="pt5 pb10">
		            	<view class="ptb10 ft14 bg-my-gray text-white" @click="togglePopup('')">{{$t('trade.cancel')}}</view>
		            </view>
				</view>
			</view>
		</uni-popup>
		...
		<view class="en-ul pl15 pt50 px-20" v-show="current==1">
			<view class="p-20 bg-my-gray rounded-xl mb-20 box-border" v-for="item in hisList" :key="item.id">
				<view class="flex between alcenter">
					<view class="flex flex flexend">
						...
						<text class="pl5 text-white">{{item.created_at.substring(10,16)}} {{item.created_at.substring(5,7)}}/{{item.created_at.substring(8,11)}}</text>
					</view>
					<view class="text-white ptb5">{{item.transacted_amount==0?$t('trade.revoked'):$t('trade.successed')}}</view>
				</view>
				<view class="mt15 flex">
					<view class="flex1">
						<text class="text-white">{{$t('trade.fee')}}</text>
						<view class="mt5">{{item.fee}}</view>
					</view>
					<view class="flex1 tc">
						<text class="text-white">{{$t('trade.delegate_price')}}({{item.currency_match.quote_currency_code}})</text>
						<view class="mt5">{{item.price}}</view>
					</view>
					<view class="flex1 tr">
						<text class="text-white">{{$t('trade.w_num')}}({{item.currency_match.base_currency_code}})</text>
						<view class="mt5">{{item.number}}</view>
					</view>
				</view>
				<view class="mt10 flex">
					<view class="flex1">
						<text class="text-white">{{$t('trade.cj_total')}}({{item.currency_match.quote_currency_code}})</text>
					...
					<view class="flex1 tr">
						<text class="text-white">{{$t('trade.cj_vol')}}({{item.currency_match.base_currency_code}})</text>
						<view class="mt5">{{item.transacted_amount}}</view>
					</view>
				</view>
			</view>
			...
			<view class="tc text-white ptb20" v-show="isBottom">{{$t('home.loading')}}...</view>
			<view class="tc text-white ptb20" v-show="!hasMore && hisList.length>10">{{$t('home.nomore')}}</view>
		</view>
	</view>
</template>
