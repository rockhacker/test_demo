	...
	<view :class="theme">
		...
		<template v-else>
				...
				<view class="flex1 plr10">
					<view class="flex alcenter tc overxy   ">
						<view class="flex1 overflow-hidden relative" style="border-radius: 8rpx 0 0 8rpx;" @click="changeType('buy')">
							<view class="p-10 relative right-12" style="transform: skew(-30deg, 0deg);" :style="{background:type=='buy'?'#4FBD35':'rgba(79, 189, 53,0.2)'}">
		    ...
		    <view class="flex between flex-btn">
				...
				<view class="btn-default rounded-3xl" :style="{background:type=='buy'?'#4FBD35':'#EA5F48'}" @click="submit">{{type=='buy'?$t('trade.buy'):$t('trade.sell')}}{{currency_name}}</view>
	...
	export default {
		...
		methods: {
			...
			changeCurrency(idx){//交易对切换
				...
				this.soon =  !this.isOnline(data.market_time)
				...
				that.$utils.initData({ url: 'market/currency_matches' }, res => {
					if (res.length > 0) {
						...
						if(localData){
							...
							that.soon = !that.isOnline(localData.market_time);
							...
							that.soon = !that.isOnline(arrFrist.market_time);
