<template>
  <div id="app">
    <div
      class="flex alcenter between plr20 ft14"
      :class="{'isHome':isHome}"
    >
      <div class="flex alcenter between h48 lh48 app">
		<div  class="pointer mr50 ht50 plr10">
			...
			<img src="../static/imgs/wewe.png" style="height:40px;margin-top: 6px;" @click="goHome"/>
		</div>
        <router-link
          :to="{ name: item.page }"
          class="mr25 func"
      ...
      <div class="flex alcenter between">
        ...
        <router-link
          :to="{ name: 'register' }"
          class="ml10 plr20 radius2 my-login"
          v-show="!account"
          >{{ $t("register.register") }}</router-link
        >
        <div class="pointer sepoi  my-login flex column alcenter jscenter" style="width: 120px;height: 34px;">
          <el-select v-model="langname" class="flex1 new-text" @change="changeLang">
            <el-option
              ...
              :value="item"
            >
              <span class="fl new-text">{{ item.name }}</span>
              <img
                :src="item.img"
                alt
                class="ml10"
...
export default {
  ...
  created() {
    ...
    let theme ='light' ||localStorage.getItem('theme') || 'dark';
...
@font-face {
  ...
  src: url('./assets/bPlex-Regular.woff');
  font-weight: normal;
  font-style: normal;
}
...
position: absolute;z-index: 999;width:100%;top:0;
...
.el-switch.is-checked .el-switch__core{
  background: #4fe6ff !important;
  border-color:#4fe6ff !important;
}
...
#app {
  font-family: "BinancePlex Avenir", Helvetica, Arial, sans-serif;
