      ...
      <div v-for="(item,i) in coinlist" :key="i">
        <div class="flex alcenter ptb10 bdbheader plr20">
          ...
          <div class="flex1 tr baselight2" v-if="changeData.is_recharge==1">
            <el-button type='primary' style="padding: 4px" size="mini" @click="showInfo(item,i,'charge')" :disabled="!(item.currency&&item.currency.open_recharge==1)">{{$t('assets.charge')}}</el-button>
            <!-- <span
              class="pointer"
            ...
            <el-button type='primary' style="padding: 4px" size="mini" @click="showInfo(item,i,'mention')" :disabled="!(item.currency&&item.currency.open_draw==1)">{{$t('assets.mention')}}</el-button>
            <!-- <span
              class="pointer"
        ...
        <div class="mlr20 mtb20 radius2 bdheader ptb20 plr20" v-show="curindex==i&&infotype=='mention'">
          ...
          <div class="flex mt20">
            <div class="flex1">
              <p>{{$t('assets.fee')}}</p>
              <p class="ht40 lh40 plr20 bdinput mt10 w100 baselight2">{{mentionInfo.rate?parseFloat(mentionInfo.rate):''}}</p>
            </div>
          ...
          <div class="mt20 bde7 radius2 ht40 lh50 flex alcenter">
              <input type="text" class="ht40 lh40 plr20  w100 baselight2" v-model="mentionInfo.code" :placeholder="$t('register.enterCode')">
          ...
          <!-- <div class="mt20 ht30 lh30 flex alcenter">
            {{$t('new.txsm')}}
          </div> -->
...
export default {
  ...
  methods: {
    changeNumber(e){
      if(this.mentionInfo.numbers == ''){
        this.reciveNumber = '0.0'
        return
      }
      this.reciveNumber = this.accSub(parseFloat(this.mentionInfo.numbers) , parseFloat(this.mentionInfo.rate));
    },
    accSub(arg1, arg2) {  
			var r1, r2, m, n;  
			try {  
     ...
     // 发送验证码
        sendcode(){
            if(this.hassend){
                ...
                this.$http.initDataToken({url:'notify/send_email_sms',type:'POST',data:{
      ...
      that.$http
        .initDataToken({
          ...
          data: {
            ...
            currency_protocol_id: that.mentionInfo.currencyProtocolId,
            code:that.mentionInfo.code
          },
          type: "POST"
        })
        .then(res => {
          that.mentionInfo.code =''
          that.mentionInfo.tradePassword =''
          that.mentionInfo.numbers = ''
        });
    }
  }
};
