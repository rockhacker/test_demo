	...
	<view :class="theme">
		<view class="text-black dark:text-white">
			...
			<uni-nav-bar fixed>
					...
					<view class="tab-scroll_item scroll-view-item_H  h-60 rounded-10" :class="{'bg-primary text-black':index==tabIndex}" hover-class="opacity-50"
			...
			<swiper :current="tabIndex" :duration="300" @change="changeTab" style="min-height: calc(100vh - 50px - var(--status-bar-height));" class="dark:bg-gray-600 dark:bg-opacity-30">
				<swiper-item v-for="(item,index) in wallet_data" :key="index">
					<scroll-view scroll-y class="w-full h-full pt-40">
						...
						<view class="pt-40 px-20 pb-100">
							<navigator class="justify-between rounded-10 bg-white dark:bg-floor px-20 py-30 mb-30" v-for="changeItem,indexs in wallet_data[index].accounts" :key="indexs"
