<template>
	<view :class="theme">
		<view class="text-black dark:text-white">
			...
			<view class="p-40 bg-white dark:bg-floor border-0 border-b-2 border-solid border-gray-200 dark:border-gray-700">
				...
				<view class="pt-40" v-if="coinInfo.currency_protocols&&coinInfo.currency_protocols.length > 1">
					...
					<view class="flex mt-20">
						<view v-for="(item, index) in coinInfo.currency_protocols" :key="index" class="mr-20 px-20 py-10 border-2 border-solid"
							:class="name == item.chain_protocol.code ? 'text-primary border-primary' : 'text-gray-500 border-gray-500'" @tap="selectCharge(item)">
			...
			<view class="p-40 bg-white dark:bg-floor">
				...
				<view class="mb-40 flex bgwhite alcenter bdb_myblue ">
				...
				<view class="mb-40">{{ $t('trade.fee') }}：{{ parseFloat(coinInfo.draw_rate )|| '--' }} {{ currencyName || '--' }}</view>

				<!-- <view class="mb-40">{{$t('new.txsm')}}</view> -->
				
				<view class="mt-20 rounded-10 h-80 flex justify-center items-center text-black text-sm text-center bg-primary" @tap="mention">{{ $t('assets.mention') }}</view>
			</view>
			
			<view class="p-40 bg-white dark:bg-gray-800">
...
export default {
	...
	methods: {
		//发送验证码
			send() {
				this.$utils.initDataToken({ url: 'notify/send_email_sms', data: {}, type: 'post' }, (res, msg) => {
		...
		all() {
			...
			if(this.number ==0){
				this.reciveNumber = '0.0'
				return 
			}
			// this.reciveNumber = this.balance* (1 - this.coinInfo.rate / 100);
			this.reciveNumber = this.accSub(this.balance , (this.coinInfo.draw_rate*1));
		},
		numberChange(e) {
			if(e.target.value==''){
				this.reciveNumber = '0.0'
			...
			this.reciveNumber = this.accSub(e.target.value , (this.coinInfo.draw_rate*1));
		},
		accSub(arg1, arg2) {  
			var r1, r2, m, n;  
			try {  
			...
			this.$utils.initDataToken(
				{
					url: 'account/draw',
					type: 'POST',
					data: { code:this.code,currency_id: this.currency, number: this.number, address: this.address, pay_password: this.password,memo:this.memoText,currency_protocol_id:this.currencyProtocolId }
				},
				(res, msg) => {
					// this.code = ''
					console.log(res);
