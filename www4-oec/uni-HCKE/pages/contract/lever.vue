		...
		<view class="text-black dark:text-white">
			<uni-nav-bar fixed>
				<view class="w-full flex flex-col p-40 border-0 border-solid border-b-2 border-gray-100 dark:border-gray-900">
					<view v-if="false" class="w-full flex justify-center">
				...
				<view class="flex justify-between p-20 bg-white dark:bg-gray-600 dark:bg-opacity-30">
					...
					<view class="w-2_5 flex flex-col ml-20">
						...
						<view class="flex-1 flex flex-col" v-if="ptype != 1">
							<view class="relative h-46 items-center flex justify-between text-black dark:text-white" v-for="(item,index) in []"
						...
						<view>
							<view v-if="false" class="text-2xl">{{ newPrice }}</view>
					...
					<view class="pb-60">
						<view class="border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 text-black dark:text-white" v-for="(item, index) in quotationList[quoteIndex].matches" :key="index" v-if="item.open_lever == 1">
							...
							>
								...
								<view v-if="item.base_currency_code=='oec' && !isOnline(item.market_time)">{{$t('trade.soon')}}</view>
			...
			<uni-popup :show="tradeTypes.shows" @hidePopup="hideBtn" :type="tradeTypes.types" :btnShow="false" :bgColor="false" :lang="langs">
				<view class="w-full bg-white dark:bg-floor text-black dark:text-white fixed bottom-0 left-0 pb-160">
...
export default {
	...
	methods: {
			...
			that.$utils.initDataToken({ url: 'market/currency_matches' }, res => {
				...
				if (res.length > 0) {
					...
					if (that.quoteCurrencyId) {
								...
								if (lists.length > 0) {
									lists.forEach(function(itm, inx) {
										if (that.baseCurrencyId == itm.base_currency_id) {
											...
											that.soon = itm.base_currency_code=='oec' && !that.isOnline(itm.market_time)
						...
						that.soon = list[0].base_currency_code=='oec' && !this.isOnline(list[0].market_time)
		...
		selectCurrency(options) {
			...
			that.soon = options.base_currency_code=='oec' && !this.isOnline(options.market_time)
