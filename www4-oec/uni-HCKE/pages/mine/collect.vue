<template>
  <view :class="theme">
    <navigator
      :url="'/pages/mine/collect_money?id=' + item.id + '&types=' + item.code"
      v-for="item in typeList"
      :key="item.id"
      class="bg-white dark:bg-floor p-40 text-lg flex items-center justify-between border-0 border-b-2 border-solid border-gray-200 dark:border-gray-700"
    >
      <view class="flex items-center">
        <image
          src="../../static/image/zhi.png"
          mode="aspectFit"
  ...
  methods: {
    ...
    getInfo() {
      ...
      that.$utils.initDataToken(
        { url: "pay_method/types", type: "GET", data: {} },
        (res, msg) => {
          that.typeList = res;
        }
      );
