<template xlang="wxml">
  <view :class="theme" class="h-full">
    <view
      class="bg-white dark:bg-floor flex flex-col items-center justify-center h-full"
    >
      <!-- <image :src="codeImg" class="wt160 h160"></image> -->
      <view class="p-40 bg-white rounded-xl">
        <tki-qrcode
          ref="qrcode"
          :val="val"
          :size="size"
      ...
      <view class="w-full p-40">
        <view
          class="bg-gray-100 rounded-xl flex justify-between items-center p-30 box-border text-primary"
        >
          <input type="text" :value="val" class="flex-1 text-black" disabled />
          <view
            class="text-warn px-20"
            @click="copyText(val, $t('assets.copy_success'))"
            >{{ $t("assets.coypaddr") }}</view
          >
        </view>
      </view>
      <view class="mb-40 text-center text-lg mt-80"
        >{{ $t("bind.codes") }}：{{ code }}</view
      >
