<template>
	<view class="plr20 bg-transparent  pb-120" :class="theme">
		<view class="mt30 ">
			...
			<view class="mt40 ">
				<view class="mb10 ">
					...
					<view class="mt45 radius4 ptb10 ft14 tc mb10 bg-primary text-black" @tap="register">{{$t('login.register')}}</view>
				</view>
			</view>
	...
	export default{
		...
		methods:{
			...
			getCode(){
				...
				this.$utils.initData({url:url,type:'POST',data:{to:this.loginActive == 0?`${this.array[this.index].global_code}${this.user_string}`: this.user_string,type:1,area_id:this.area_code}},(res,msg)=>{
			...
			send() {
				...
				var datas = {
					to: this.loginActive == 0?`${this.array[this.index].global_code}${this.user_string}`: this.user_string,
