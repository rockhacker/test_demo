<template>
	<view :class="theme">
		...
		<!-- 头部 -->
		<view class="header bgPart fixed flex alcenter jscenter plr15 zdx100">
		...
		<template v-else>
			<view class="flex pt70 pb15 bgPart" style="height:980rpx;">
				<!-- 左侧 -->
				<view class="plr15 left-flex" style="min-width:60%;max-width: 60%;">
					<view class="flex alcenter tc overxy posRelt  ">
						<view :class="['flex1  flex-after  ft17 ',{'active':type=='buy'}]" @click="changeType('buy')">{{$t('trade').buy}}</view>
						<view class="bor abstrot mt5 bgPart"></view>
						<view :class="['flex1  flex-after ft17 ',{'active':type=='sell'}]" @click="changeType('sell')">{{$t('trade').sell}}</view>
					</view>
					...
					<view class="">
						<view class="" v-show="priceType=='limit'">
							<view class="bd_input  radius2 input-pd">
								<view class="input-title">{{$t('trade.price')}}</view>
								<input type="number" v-model='price' class="h40 flex1 ft14" />
						...
						<view class="bd_input mt15 plr10 radius2 input-pd bdb_blue3">
						...
						<view class="gray7 input-title ft12">{{$t('trade.use')}}
							<view>{{balance}} {{type=='buy' ? legal_name :currency_name}} </view>
						</view>
						...
						<view class="gray7 alcenter mt10 input-title ft12">
							<text>{{$t('trade.amout')}}</text>
							<view class="nwhite input-title" style="word-wrap:break-word;">{{tradeNum || '--'}}{{legal_name}}</view>
						</view>
		    ...
		    <view class="flex between flex-btn">
				...
				<view class="btn-text" @click="toDeal">{{$t('new.deal')}} ></view>
			</view>
			<view class="flex alcenter between ptb10  plr15 footer">
				<view class="flex flexend">
					<text class="ft18 yellow">{{$t('trade.delegate')}}</text>
				...
				<view class="flex column ht100">
					...
					<view class="flex1 overyscroll" v-for="(market,index) in marketList" :key="index" v-if="current==index">
						<view :class="['flex alcenter between ptb15 plr15  bdb_1e',{' bggray':legal_id==item.quote_currency_id && currency_id==item.base_currency_id}]" :key="idx" v-for="(item,idx) in market" v-if="item.open_change==1" @click="changeCurrency(idx)">
							...
							<view v-if="item.base_currency_code=='oec' && !isOnline(item.market_time)" class="w-200">{{$t('trade.soon')}}</view>
							<template v-else>
	...
	export default {
		...
		methods: {
			...
			changeCurrency(idx){//交易对切换
				...
				this.soon = data.base_currency_code=='oec' && !this.isOnline(data.market_time)
				...
				that.$utils.initData({ url: 'market/currency_matches' }, res => {
					if (res.length > 0) {
						...
						if(localData){
							...
							that.soon = arrFrist.currency_name=='oec' && !that.isOnline(localData.market_time);
							...
							that.soon = arrFrist.base_currency_code=='oec' && !that.isOnline(arrFrist.market_time);
