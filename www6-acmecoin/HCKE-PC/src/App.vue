<template>
  <div id="app">
    <div
      class="flex alcenter between plr20  ft14 bgwhite bg-new-black text-gray1"
      style="height:60px;"
    >
      <div class="flex alcenter between h48 lh48 app">
		<div  class="pointer mr100 ht50 plr10 mt16 posRelt">
			<img src="../static/imgs/wewe.png" class="abstort" style="width:50px;top:-8px" @click="goHome"/>
		</div>

    <el-popover
      placement="bottom"
      trigger="hover"
      popper-class="white border-none"
      :visible-arrow="false"
      v-model="show1"
      >
  ...
  <div class="mr25 pointer flex alcenter text-gray1" :class="{'routeractive':selectA!=''}" slot="reference">
    <span>{{selectA || $t('add.jiaoyi')}}</span>
    <img class="pl10 wt20" :class="{'rotate':show1}" src="./assets/images/arrow-dowm.png" alt="" srcset="">
  </div>
</el-popover>
 ...
 <el-popover
  ...
  <div class="mr25 text-gray1 pointer flex alcenter" :class="{'routeractive':selectB!=''}" slot="reference">
    <span>{{selectB || $t('assets.lever')}}</span>
    <img class="pl10 wt20"  :class="{'rotate':show2}" src="./assets/images/arrow-dowm.png" alt="" srcset="">
  </div>
</el-popover>
      ...
      <div class="flex alcenter between ">
        ...
        <router-link
          :to="{ name: 'login' }"
          class="plr20 radius2 login-btn"
          v-if="!account"
          >{{ $t("login.login") }}</router-link
        >
        <div class="plr20 pointer" style="white-space:nowrap;" v-else>
          <el-dropdown class="text-gray1" @command="personalLink">
            <span class="el-dropdown-link">
              <i class="iconfont icon-zhanghao ft16 text-gray1" ></i>
              {{ account }}
              <i class="el-icon-arrow-down el-icon--right ft16 text-gray1"></i>
            </span>
        ...
        <router-link
          :to="{ name: 'register' }"
          class="ml10 plr20 radius2  login-btn"
          v-show="!account"
          >{{ $t("register.register") }}</router-link
        >
        <div class="pointer sepoi radius2  login-btn" style="width: 100px;">
          <el-select v-model="langname" class="flex1 lang-select" @change="changeLang">
            <el-option
...
export default {
  ...
  async created() {
    ...
    let theme = "dark" || localStorage.getItem('theme') 
...
.el-switch.is-checked .el-switch__core{
  background: #E5A013 !important;
  border-color:#E5A013 !important;
}
.el-loading-spinner i {
    color:#E5A013 !important;
}
...
.el-input__inner {
  border: 1px solid #747D99 !important;
}
