<template>
    <div>
        <div class="w100 mauto baselight2 mt20 bg-box2 ptb10 flex alcenter between ft16 plr20 radius2">
            <p class="ptb10">  <span class="pointer" @click="$router.go(-1)">{{$t('legal.trade')}} <span>/</span> </span > <span class="text-clip-green">{{$t('legal.order')}}</span></p>
            <!-- <p class="pointer ptb10" @click="$router.back(-1);">&lt;&lt; {{$t('store.returns')}}</p>
            <p class="ptb10 blue">{{$t('legal.order')}}</p> -->
        </div>
        <div class="mt10 bg-box2">
            <div class="ft16 flex alcenter between">
                <div class="flex baselight2 ">
                    <span class="bold pointer plr15 ptb15" @click="changeType('BUY')" :class="{'text-primary bg-box3':selectType=='BUY'}">{{$t('legal.buy')}}</span>
                    <span class="bold pointer plr15 ptb15" @click="changeType('SELL')" :class="{'text-primary bg-box3':selectType=='SELL'}">{{$t('legal.sell')}}</span>
                </div>
            </div>
            <div class="plr20 ptb20 flex alcenter baselight2 ft14 ">
                <div class="mr30 pointer bd2px ptb10" v-for="(item,i) in statusList"  :class="[status==item.status?'text-primary':'',status==item.status?(selectType=='text-primary'):'']" @click="changeStatu(item.status)" :key="i">{{item.name}}</div>
            </div>
            <div class="mt20 plr20">
                ...
                <div class="baselight2">
                    <div class="flex alcenter ptb10 bdb30 pointer" v-for="(item,i) in orderlist" :key="i">
                        <div class="flex1 ">
                            <span :class="['ft14 green',{'red':item.way=='SELL'}]">{{item.way=='SELL' ? $t('legal.sell'):$t('legal.buy')}}</span>
                            <span class="blue">{{item.currency_name}}</span>
                        </div>
                        <span class="flex1 tc" v-if="item.area_info">{{item.price | filterDecimals(4)}} {{item.area_info.currency}}</span>
                        <span class="flex1 tc">{{item.number | filterDecimals(4)}}</span>
                        <span class="flex1 tc green" v-if="item.area_info">{{item.amount | filterDecimals(4)}} {{item.area_info.currency}}</span>
                        <span class="flex1 tc">{{item.created_at}}</span>
                        <div class="flex1 tr" @click="goDetail(item.id)">
