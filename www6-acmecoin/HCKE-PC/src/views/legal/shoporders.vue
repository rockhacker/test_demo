<template>
    <div class="plr20 ptb20 bgwhite dark-bg-1C2532 dark-text-white">
        <div class="flex alcenter baselight2 ft14 ">
            <div class="mr30 pointer bd2px" v-for="(item,i) in statusList" :class="{'active2px':status==item.status}" @click="changeStatu(item.status)" :key="i">
                <span  :class="{'text-clip-green':status==item.status}">{{item.name}}</span>
            </div>
        </div>
        <div class="mt20">
            <div class="flex alcenter ptb10" v-if="orderlist.length>0">
                <span class="flex1">{{$t('store.orderNumber')}}</span>
                <span class="flex1">{{ type=='SELL'? $t('new.buyuser'):$t('new.selluser')}}</span>
            ...
            <div class="baselight2">
                <div class="flex alcenter ptb10 bdb30 pointer" v-for="(item,i) in orderlist" :key="i">
                    <div class="flex1 ">
                        ...
                        <span class="text-clip-green">{{item.currency_name}}</span>
                    </div>
                    <span class="flex1">{{ type=='SELL'? item.buy_user_account:item.sell_user_account}}</span>
        ...
        <div class="w100 tc lf0 btm20 pages mt40" >
            <el-pagination
                background
