<template>
    <div class="flex alcenter jscenter pt220 login-page">
        <div class="bg-box radius4 plr40 ptb40 mb40 w25 mwidth">
            <div class="tc ft20 white">{{$t('login.welcome')}}</div>
            <div class="mt20 ">
                <div class="bold">
                    <span class="mr20 ft14" v-show="i!=0"  :class="{'active2px':index==i}" v-for="(item,i) in infos" :key="i" @click='changeMethod(i)'><span>{{$t(item)}}</span></span>
                </div> 
                <div class="mt20 radius2 ht50 lh50 flex alcenter border-b-EEEEEE">
                    <!-- <div class="w20 bdr tc">+86</div> -->
                    <el-select v-model="country_code" filterable class="w25" @change='changeCountry' v-show="index==0">
                        <el-option 
                    ...
                    <input type="text" class="flex1 h100  pl20 white ft14" v-model="account" @change="changeAccount" :placeholder="index == 0 ? $t('register.enterPhone') : $t('register.enterEmail')">
                </div>
                <div class="mt20 border-b-EEEEEE radius2 ht50 lh50 flex alcenter">
                    <input type="password" class="flex1 h100  pl20 white ft14" v-model="password" :placeholder="$t('register.enterPassword')">
                </div>
                ...
                <div class="flex between alcenter mt10">
                    ...
                    <router-link to="/forgetPassword" class="ft16"><span class="text-primary">{{$t('login.forget_pwd')}}</span></router-link>
                </div>
                <div class="mt40 bg-primary white tc h48 lh48 radius2 pointer ft20" @click='login'>{{$t('login.login')}}</div>
            </div>
        </div>
    </div>
</template>
