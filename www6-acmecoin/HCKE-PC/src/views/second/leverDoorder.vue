<template>
  <div class="p-10">
    <div class="tc flex  bgwhite dark-bg-1C2532 ft14">
      <div class="flex1 hidden-xs bg-blue2 ptb20 radius6">
        <div class="flex jscenter alcenter ft16 white ft16">
          <!-- <img src="../../assets/images/money.png" alt> -->
          <p>{{$t('assets.microacc')}}</p>
        </div>
        <p class="pt15 ft22 bold white" v-if="wallet_list.length&&wallet_list[modelIndex]">{{wallet_list[modelIndex].micro_account.balance}} {{wallet_list[modelIndex].code}}</p>
        <!-- <ul>
    ...
    <div class="mt20">
      <div class="dark-bg-1C2532 flex alcenter ft14 ptb10">
        <!-- <img src="../../assets/images/tick.png" alt=""> -->
        <span class="text-primary">{{$t('second.TransactionMode')}}</span>
      </div>
      <ul class="flex wraps times-list tc mt10">
        <li v-show="modelIndex==i" class="mb10 bde9 w30 ptb10 mr10 dark-bg-1C2532 posRelt pointer gray9 ft14" v-for="(item,i) in wallet_list" :key="i">
          <span>{{item.code}}</span>
          <img src="../../assets/images/pitch_on.png" alt="" v-if="modelIndex==i" style="width: 30px;" class="abstort btm0 rt0">
        </li>
      </ul>
      <div class="dark-bg-1C2532 flex alcenter ft14 ptb10">
        <!-- <img src="../../assets/images/tick.png" alt=""> -->
        <span class="text-primary">{{$t('second.OpenQuantity')}}</span>
      </div>
      <ul class="flex wraps times-list tc mtb10" >
        <!-- <input type="number" class="dark-bg-1C2532 nwhite k_number bde9" v-model="kNumber" >
        <el-select v-model="kNumber" @change="changeLang" no-data-text=" " popper-class="selects" class="bde9 s_number dark-bg-1C2532" placeholder="">
          <el-option
    ...
    <div class="name">{{ parseFloat(item.number) }}</div>
        ...
        <!-- <el-input type="number" class="w100 dark-bg-1C2532 nwhite k_number bde9" style="margin-right:0" v-model="kNumber">
        ...
        <!-- <li class="mb10 bde9 w30 ptb10 mr10 dark-bg-1C2532 posRelt pointer gray9 ft14" v-for="(item,i) in wallet_list[modelIndex].micro_numbers" :key="i" @click="changeNumber(i)">
      ...
      <div class="dark-bg-1C2532 flex alcenter ft14 ptb10">
        <!-- <img src="../../assets/images/tick.png" alt="" /> -->
        <span class="text-primary">{{$t('second.OpenTime')}}</span>
      </div>
      <ul class="flex wraps times-list tc mt10" v-if="second_List">
        <li class="mb10 bde9 w30 ptb10 mr10 dark-bg-1C2532 posRelt pointer gray9 ft14" v-for="(item,i) in second_List" :key="i" @click="changeTimes(i)">
          <span>{{item.seconds}}{{$t('new.miao')}}</span>
          <img src="../../assets/images/pitch_on.png" alt="" v-if="timeIndex==i" style="width:30px" class="abstort btm0 rt0">
        </li>
      </ul>
      <div class="bg-blue2 ptb20 flex hidden-xs">
        <div class="flex flex1 alcenter jscenter">
          <!-- <img src="../../assets/images/miaoIcon.png" alt=""> -->
          <span class="white bold ft18">{{$t('second.profitRate')}}</span>
        </div>
       <div class="flex1 alcenter flex  jscenter gold ft36">
          <span class="bold white" v-if="second_List.length&&second_List[timeIndex].profit_ratio">{{parseFloat(Number(second_List[timeIndex].profit_ratio)*100)}}%</span>
          <span></span>
        </div>
      </div>
      ...
      <div class="bg-main-liner tc ht40 lh40 radius2 blue mt20 ft16" v-if="!token">
      ...
      <div class="mt20 nmb_mt20" v-else>
        <div class="flex1 bg-k-green tc ptb10 ft16 white radius40 flex alcenter jscenter pointer mb_btn" @click="submits(1)" >
          <span class="white">{{$t('second.buyUp')}}</span> 
          <img class="ml10" src="../../assets/images/miaoIcon_18.png" alt="">
        </div>
        <div class="flex1 bg-k-red tc ptb10 ft16 white radius40 flex alcenter jscenter pointer mb_btn mt20" @click="submits(2)" >
          <span class="white">{{$t('second.buyDown')}}</span> 
          <img class="ml10" src="../../assets/images/miaoIcon_21.png" alt="">
        </div>
      </div>
    </div>
    <!-- 提交弹窗 -->
