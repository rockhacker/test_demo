<template>
	<div class="minh100 ft16">
		<div class="mauto wt1200 ptb20">
			...
			<el-card class="ft22 mb20 dark-bg-1C2532 dark-text-white">
				{{$t('subscription.new_subscription')}}
				<el-link style="float: right; padding: 3px 0" @click="$router.push({path:'/subscriptionOrders'})"><div class="text-clip-green">{{$t('subscription.my_subscription')}}</div></el-link>
			</el-card>
			
			<el-card class="box-card mb20 dark-bg-1C2532" @click="$router.push({path:'/subscriptionDetails?id=0'})" hover-class="opacity-50" v-for="(item,index) in list" :key="index">
				<div class="flex between">
					<div class="ft22 bold dark-text-white">{{item.currency_name}} / {{$t('subscription.new_subscription')}}</div>
					<el-button class="wt100 mr10 bg-main-liner white" style="border-radius: 0;" @click="$router.push({path:`/subscriptionDetails?id=${item.id}`})">{{$t('subscription.canjiahuodong')}}</el-button>
				</div>
				...
				<div class="ft16 flex between ptb20 alcenter">
					<div class="flex">
						<span class="black bold dark-text-white">{{item.subscribed}} {{item.currency_name}} </span>
						<span class="mlr10 dark-text-white">/</span>
						<span class="gray7 dark-text-white">{{item.issue_number}} {{item.currency_name}}</span>  
					</div>
				...
				<div class="w100 ht30 radius40 bgf6 posRelt overxy">
					<div class="abstort lf0 h100 lh30 bg-main-liner white ft14 tr" :style="`width: ${remainingRatio(item.subscribed,item.issue_number)}%;`">
						<span class="mlr10">{{remainingRatio(item.subscribed,item.issue_number)}}%</span>
					</div>
				</div>
			</el-card>
			<el-card class="box-card mb20 tc ptb40" v-if="list.length==0">
