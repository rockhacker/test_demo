<template>
	<view :class="theme">
		<view class="text-333333 dark:text-white bg-my-grey dark:bg-my-black">
			...
			<uni-nav-bar fixed>
				<scroll-view  class="scroll-view_H h-100 leading-100 bg-box" scroll-x="true" scroll-with-animation :scroll-left="scrollLeft">
					<view class="tab-scroll_item scroll-view-item_H  h-60 rounded-10" :class="{'bg-primary text-white dark:text-white':index==tabIndex,'text-333333 dark:text-white':index!=tabIndex}" hover-class="opacity-50"
					 v-for="(item,index) in wallet_data" :key="index" @click="tapTab(index)" :id="item.name">{{item.name}}</view>
				</scroll-view>
			</uni-nav-bar>
			
			<swiper :current="tabIndex" :duration="300" @change="changeTab" style="min-height: calc(100vh - 50px - var(--status-bar-height));" class="dark:bg-my-black">
				<swiper-item v-for="(item,index) in wallet_data" :key="index">
					<scroll-view scroll-y class="w-full h-full">
						<view class="py-30 px-40 bg-white dark:bg-box text-xl">
							<view class="flex justify-between">
								<view>
									<text class="text-33333 dark:text-white text-lg mr-20">{{item.name}}</text>
									<text class="text-838B99 dark:text-gray1 text-xs">{{$t('assets.zhehecny')}}（{{sign2}}）</text>
								</view>
						...
						<view class="pt-40 px-20 pb-100">
							<navigator class="justify-between rounded-10 bg-white dark:bg-box px-20 py-30 mb-30" v-for="changeItem,indexs in wallet_data[index].accounts" :key="indexs"
							:url="'tradeAccount?id='+wallet_data[index].id+'&titleName='+wallet_data[index].name+'&type='+wallet_data[index].account_code+'&account_id='+changeItem.id+'&currency_id='+changeItem.currency_id">
								...
								<view class="mt-20 flex items-center">
									<view class="flex-1">
										<view class="text-838B99 dark:text-gray1 text-xs">{{$t('trade.use')}}</view>
										<view class="mt-10 text-333333 dark:text-white">{{isClose?closeAccount:changeItem.balance}}</view>
									</view>
									<view class="flex-1">
										<view class="text-838B99 dark:text-gray1 text-xs">{{$t('assets.zhehe')}}({{sign2}})</view>
								...
								<view class="mt-20 flex items-center">
									<view class="flex-1">
										<view class="text-838B99 dark:text-gray1 text-xs">{{$t('assets.underView')}}</view>
