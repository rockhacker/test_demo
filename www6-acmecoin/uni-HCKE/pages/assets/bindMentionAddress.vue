<template>
	<view :class="theme">
		<view class="bg-my-grey dark:bg-box">
			...
			<view class="px-30 py-20">
				...
				<scroll-view scroll-x class="w-full whitespace-nowrap" :show-scrollbar="false">
					<view class="px-20 py-10 mr-40 flex-col justify-center items-center text-lg inline-flex" hover-class="opacity-50"
						:class="{'bg-primary':index==i,'border-2 border-solid border-gray1':index!=i}" v-for="(item,i) in coinList" :key="i" 
						@click="pickChange({target:{value:i}})">
						<view :class="{'text-white':index==i,'text-black dark:text-white':index!=i}">{{item.code}}</view>
					</view>
				</scroll-view>
			</view>
			<view class="px-30 py-20">
					...
					<view v-if="coinList[index].currency_protocols.length > 1">
						...
						<view class="flex mb-30">
							<view v-for="(item, index) in coinList[index].currency_protocols" :key="index" 
								class="text-lg mr-20 py-10 px-20  border-2 border-solid border-gray1" :class="{'text-primary dark:border-primary': name == item.chain_protocol.code}"
				...
				<view class="flex items-center">
					...
					<input type="text" value="" v-model="address" class="flex1 plr15 h40 border-0 border-b-2 border-solid dark:border-gray1" :placeholder="$t('bind.p_addr')" />
				</view>
				<view class="mt-100 mb-60 bg-primary rounded-10 p-20 text-white text-lg text-center" @tap="bindAddress" hover-class="opacity-50">{{ $t('bind.bind') }}</view>
			</view>
			<!-- 验证码 -->
			<!-- <view class="flex items-center">
