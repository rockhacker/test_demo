<template>
	<view :class="theme">
		<view class="p-30 bg-white dark:bg-box">
		...
		<view class="px-20 pt-30 pb-140 mt-20 bg-white dark:bg-box" style="min-height: calc(100vh - 100px);">
		...
		<view class="fixed w-full flex items-center bottom-0 bg-white dark:bg-box">
			<!-- 币币账户 -->
			<block v-if="tradeType == 'change_account' &&coinInfo.account_type&& coinInfo.account_type.is_recharge == 1">
				<navigator :url="'transfer?accountTypeId=' + coinInfo.account_type_id+'&id='+coinInfo.id+'&account_code='+coinInfo.account_type.account_code" class="flex-1 text-center py-20" v-if="changeTransfer == 1">
					...
					<view class="text-primary">{{ $t('assets.transfer') }}</view>
				</navigator>
				<!-- 充币 -->
				<navigator
				...
				>
					...
					<view class="text-primary">{{ $t('assets.charge') }}</view>
				</navigator>
				<!-- 提币 -->
				<navigator
				...
				>
					...
					<view class="text-primary">{{ $t('assets.mention') }}</view>
				</navigator>
			</block>
			<!-- 法币账户 -->
