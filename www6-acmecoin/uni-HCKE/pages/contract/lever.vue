<template>
	<view :class="theme">
		<view class="bg-my-grey dark:bg-my-black">
			<uni-nav-bar bgClass="dark:bg-my-black" fixed>
				<view class="w-full flex flex-col p-40 dark:bg-my-black">
					<view class="w-full flex justify-center">
						<view class="flex w-full h-60 bg-F4F5F7 dark:bg-333F4A rounded-2xl overflow-hidden">
							<view class="flex-1 flex justify-center items-center text-center" :class="active==0?'bg-primary text-white':'text-333333 dark:text-white'" @tap="setActive(0)">{{$t('new.leverLes')}}</view>
							<view class="flex-1 flex justify-center items-center text-center" :class="active==1?'bg-primary text-white':'text-333333 dark:text-white'" @click="setActive(1)">{{$t('new.leverAll')}}</view>
							<view class="flex-1 flex justify-center items-center text-center" :class="active==2?'bg-primary text-white':'text-333333 dark:text-white'" @tap="setActive(2)">{{$t('qqjy.qqjy')}}</view>
						</view>
					</view>
			...
			<view v-if="active==1 && !soon" class="min-h-screen bg-gray-100 dark:bg-floor">
				<!-- <view class="flex justify-center items-center text-3xl h-600" v-if="soon">{{$t('trade.soon')}}</view> -->
				<view class="flex justify-between p-20 bg-white dark:bg-my-black">
					<view class="w-3_5">
						...
						<view class="flex item-center justify-between mt-20">
							<view class="flex items-center justify-between flex-2 h-60 pr-10 rounded-10 dark:bg-box" @tap="tradeTypes.shows = true">
							...
							<view class="flex-1 h-60 ml-10">
								<view class="rounded-10 dark:bg-box h-60 flex items-center justify-between pr-10" @tap="mults.shows = true">
						...
						<view class="rounded-10 dark:bg-box h-70 flex items-center mt-20 px-20">
						...
						<view class="mt-20 text-lg">
							...
							<view class="flex justify-between items-center my-20 rounded-10 dark:bg-box">
								<view class="flex-1 h-60 flex items-center">
									<input class="px-8 text-black dark:text-white text-sm" placeholder-class="text-gray-600 flex-1" type="number" v-model="leverOrder.share" :placeholder="$t('trade.p_handnum')" />
								</view>
				...
				<!-- 当前委托列表 -->
				<view class="p-20 bg-white dark:bg-my-black pb-120 min-h-half">
					<view class="flex justify-between">
				...
				<view class="pt-bar">
					<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
						<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
							...
							:class="{'text-primary':quoteCurrencyIds == item.id}" v-for="(item,i) in quotationList" :key="i" 
