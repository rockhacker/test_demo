<template>
	<view :class="theme">
		<uni-nav-bar backIcon fixed>
			<view class="flex justify-between items-center py-20 pr-20 w-full bg-white dark:bg-box">
				<view>
		...
		<view class="list text-xl px-30 bg-white dark:bg-my-black" style="min-height: calc(100vh - 70px - var(--status-bar-height));">
			...
			<template v-else>
				<view v-for="(item, index) in orderList" :key="index" class="py-30 border-0 border-solid border-b-2 border-gray-100 dark:border-gray-900">
					...
					<view class="flex text-right mt-30 justify-end">
						<view class="bg-primary px-40 py-20 rounded-8 text-white" @tap="loss(item,index)">{{$t('lever.setys')}}</view>
						<view class="bg-primary px-40 py-20 rounded-8 text-white ml-20" @tap="closes(item.id,index)">{{$t('trade.pingcang')}}</view>
					</view>
				</view>
			</template>
		</view>
