<template>
	<view :class="theme" class="flex flex-col">
		...
		<view class="w-full flex flex-col">
			<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
				<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
					...
					:class="isActive==i?'text-primary':'text-838B99'" v-for="(item,i) in marketLists" :key="i" 
					@click="changeTab(i,item.code,item)">
					{{item.code}}
					<view class="w-70 h-6 mt-10" :class="{'bg-primary':isActive==i}"></view>
				</view>
			</scroll-view>
			
			<view class="bg-white flex items-center text-gray1 text-lg mx-20 px-10 py-20 dark:bg-box">
		...
		<view class="pb-180 mt-18" style="min-height: calc(100vh - 100px);">
			<view @click="toPath(item)" class="flex items-center text-sm text-gray1 tableitem mx-20 px-18 py-30 bg-box" style="border-bottom: 2rpx dashed #111727;" v-for="(item,i) in matches" v-if="marketLists[isActive]" :key="i" v-show="marketLists&&marketLists[isActive]&&item.open_change==1">
				<view class="flex-2">
