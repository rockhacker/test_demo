<template>
	<view :class="theme">
		<view class="bg-white dark:bg-box pb-30">
			<!-- 支付宝 -->
			<view v-if="types=='alipay'">
				<view class="flex items-center pl-20 mt-20 border-0 border-b-2 border-solid border-gray-200 dark:border-my-black">
			...
			<view v-else>
				<template v-if="show">
					<view class="flex items-center pl-20 border-0 border-b-2 border-solid border-gray-200 dark:border-my-black">
					<text class="text-right text-gray1">{{$t('collect.name')}}：</text>
					<input type="text"  v-model="name" class="h-80 flex-1 pr-20 text-right" :placeholder="$t('collect.p_name')">
				</view>
				<view class="flex items-center pl-20 mt-20 border-0 border-b-2 border-solid border-gray-200 dark:border-my-black">
					<text class="text-right text-gray1">{{$t('collect.bank')}}：</text>
					<input type="text"  v-model="kaihu" class="h-80 flex-1 pr-20 text-right" :placeholder="$t('collect.p_bank')">
				</view>
				<view class="flex items-center pl-20 mt-20 border-0 border-b-2 border-solid border-gray-200 dark:border-my-black">
					<text class="text-right text-gray1">{{$t('collect.no')}}：</text>
					<input type="text"  v-model="card_no" class="h-80 flex-1 pr-20 text-right" :placeholder="$t('collect.p_no')">
				</view>
				<view class="flex items-center pl-20 mt-20 border-0 border-b-2 border-solid border-gray-200 dark:border-my-black">
					<text class="text-right text-gray1">{{$t('collect.z_bank')}}：</text>
					<input type="text"  v-model="bank_adress" class="h-80 flex-1 pr-20 text-right" :placeholder="$t('collect.p_bankname')">
				</view>
				<view class="flex items-center pl-20 mt-20 border-0 border-b-2 border-solid border-gray-200 dark:border-my-black">
					<text class="text-right text-gray1">{{$t('collect.bank_code')}}：</text>
			...
			<button v-if="!show" @click="show=true" class="mx-30 rounded-3xl bg-primary text-white text-xl" >
				<!-- <image src="../../static/image/card.png" mode="aspectFit" class="w-30 h-30"></image> -->
				{{$t('store.bank')}}	
			</button>

			<button v-if="show" class="rounded-3xl bg-primary text-white text-xl mt-60 mx-30" @tap="confirm">{{$t('login.e_confrim')}}</button>
		</view>
	</view>
</template>
