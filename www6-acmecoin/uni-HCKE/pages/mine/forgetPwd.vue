<template>
	<view :class="theme">
		<view class="p-40 bg-white dark:bg-box">
			...
			<view class="mt-80">
				...
				<view class="my-100 bg-primary rounded-3xl p-20 text-white text-lg text-center" @tap="goNext" hover-class="opacity-50">{{$t('login').r_next}}</view>
			</view>
		</view>
		<view class="fixed my_layer pos_l0b0 w100 ht100" v-if="isShowCodeLayer" @tap="isShowCodeLayer=false;">
			<view class="fixed pos_l0b0 bgHeader w100 pt20 pb10 plr10" @tap.stop>
				...
				<view class="mt30 bgBlue rounded-3xl ptb10 white ft14 tc mb10" @tap.stop="confirm">{{$t('login').r_next}}</view>
			</view>
		</view>
	</view>
</template>
