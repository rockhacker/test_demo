<template xlang="wxml">
	<view :class="theme" class="h-full">
		<view class="dark:bg-my-black min-h-full px-40" >
			...
			<view class="mt-30 rounded-xl bg-box">
				<view class=" flex flex-col justify-between items-center p-30 box-border text-primary">
					...
					<view class="mt-20  text-white rounded-3xl py-16 w-400 text-center text-lg bg-primary" @click="copyText(val,$t('assets.copy_success'))">{{$t('feed.copyadd')}}</view>
				</view>
			</view>
			<view class="mt-30 rounded-xl flex px-20 py-40 text-white bg-box">
			 	<text class="pr-10">{{$t('new.fybl')}}:</text>  {{$t('new.yj')}}/{{set_comm[1]}}%, {{$t('new.ej')}}/{{set_comm[2]}}%, {{$t('new.sj')}}/{{set_comm[3]}}%
			</view>
			<view class="mt-30 rounded-xl flex py-40 text-white bg-box">
				<view class="flex1">
			...
			<view class="mt-30 rounded-xl p-20 text-white bg-box">
...
.my-bg{
	...
	background-image:  linear-gradient(-90deg, #E5A013, #0049B3);
}
</style>
