<template>
  <view :class="theme">
	  <view class="bg-my-grey dark:bg-box text-black dark:text-white" style="min-height: 100vh;">
		...
		<view class="p-40">
			<view class="flex items-start justify-between">
				...
				<!-- <view class="flex flex-col justify-center items-center mr-20 border-0 border-solid border-primary" :class="{ 'text-primary border-b-2': loginActive == 0 }" @tap="loginActive = 0">
					  <view class="mb-30 text-xl">{{$t('login').l_mobile}}</view>
				</view>		 -->
				<view class="flex flex-col justify-center items-center  border-0 border-solid border-primary" :class="{ 'text-primary border-b-2': loginActive == 1 }" @tap="loginActive = 1">
			...
			<view class="py-40">
				<view class="flex items-center border-0 border-b-2 border-solid border-gray-200 dark:border-gray1">
				...
				<view class="flex items-center border-0 border-b-2 border-solid border-gray-200 dark:border-gray1 mb-20" v-if="isShowCode">
				...
				<view class="my-60 bg-primary rounded-3xl p-20 text-white text-lg text-center" @tap="login" hover-class="opacity-50">{{ $t("login").login }}</view>
				<view class="my-60 bg-primary rounded-3xl p-20 text-white text-lg text-center mt-30" @tap="toWallet" hover-class="opacity-50">{{ $t("wallet.login") }}</view>
			</view>
			  <view class="flex items-center justify-center">
				<view class="flex items-center">
				  <text>{{ $t("login").noaccount }}</text>
				  <navigator url="register" class="text-primary">{{$t("login").register}}</navigator>
				</view>
...
<script>
	...
	export default{
		...
		onLoad(e) {
			...
			if(e.from ==='register'){
				this.user_string = this.$store.state.user.account
