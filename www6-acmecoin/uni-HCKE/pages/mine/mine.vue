<template>
  <view :class="theme">
	<view class="bg-white dark:bg-my-black pb-180">

	<uni-nav-bar>
		<view class="w-full bg-my-grey dark:bg-my-black">
			<view class="w-full bg-white dark:bg-box p-30">
				<view class="w-full flex justify-between">
					<navigator class="" :url="`/pages/mine/${token?'login':'userCenter'}`">
						<view class="text-sm text-333333 dark:text-white">{{ token?(info.mobile || info.email || '- -'):$t("home.p_login") }}</view>
						<view class="text-sm text-333333 dark:text-white">{{ token?(`UID: ${info.uid || '- -'}`):$t("home.welcome") }}</view>
					</navigator>
					<view class="flex flex-col">
						<view class="text-sm text-primary">{{ info.level || '- -' }}</view>
						<view class="text-sm text-primary">{{ `${$t('home.credit')}: ${info.credit_score || '- -'}` }}</view>
					</view>
					<button type="primary" size="default" class="text-sm px-20 bg-primary text-white rounded-10" @click="logout">
			...
			<view class="mt-20 w-full px-20 pb-20">
				<view class="bg-primary rounded-lg p-40">
					<view class="flex justify-between">
						<view class="text-xl font-bold text-white dark:text-white">{{ $t("new.mine") }}</view>
						<image src="/static/image/a.png" class="w-40 h-30" v-if="active == 1 && token" @click="aetActive(2)"></image>
						<image src="/static/image/b.png" class="w-30 h-30" v-else @click="aetActive(1)"></image>
					</view>
					<view class="my-20 text-xs text-white dark:text-white">{{ $t("new.zichan") }} (USDT) </view>
					<view class="text-white dark:text-white text-lg my-20">{{ active==1 && token?(all?all.toFixed(2):all):'******' }} </view>
					<navigator url="/pages/assets/assets" open-type="switchTab" class="flex justify-between items-center">
						<view class="text-primary rounded-3xl px-20 py-10" :class="[theme=='dark'?'dark:bg-white':'bg-white']">
							{{active==1 && token?(`${$t("new.keyong")} : ${all - lock}`):'****'}}
						</view>
						<image src="/static/image/white-jiantou.png" class="w-16 h-28"></image>
					</navigator>
				</view>
			</view>
		</view>
	...
	<view class="bg-white dark:bg-1C2532 mt-20">
		<view class="px-30">
			<navigator v-if="token" url="/pages/home/recharge" class="flex items-center py-30 justify-between text-lg border-0 border-solid border-b-2 border-EEEEEE dark:border-my-black">
				<view class="flex items-center">
					<view class="w-6 h-26 mr-20 bg-primary"></view>
		...
		<view class="px-30">	
			...
			<navigator v-if="false" class="flex items-center py-30 justify-between text-lg border-0 border-solid border-b-2 border-EEEEEE dark:border-my-black" @click="topo">
				<view class="flex items-center">
					<view class="w-6 h-26 mr-20 bg-primary"></view>
			...
			<view v-if="download" class="flex items-center py-30 justify-between text-lg border-0 border-solid border-b-2 border-EEEEEE dark:border-my-black" @click="dow">
				<view class="flex items-center">
					<view class="w-6 h-26 mr-20 bg-primary"></view>
	...
	export default{
		...
		methods:{
			...
			topo(){
				...
				window.open(`${this.$utils._DOMAIN}/wp/WEZCUX-${lang}.pdf`);
			},
			aetActive(val){
				if(!this.token){
					return
