<template>
	<view :class="theme">
		 <view class="p-30 bg-my-grey dark:bg-box text-black dark:text-white" style="min-height: 100vh;">
			<view>
			<view class="text-2xl text-primary" v-if="loginActive==0">
				{{$t('login').r_mobile}}
			</view>
			<view class="text-2xl text-primary" v-else>
				{{$t('login').r_email}}
			</view>
			...
			<view class="mt40 ">
				<view class="mb10 ">
					...
					<view class="flex bgwhite alcenter bdb_myblue ">
						...
						<button size="mini" type="primary" class="px-40 bg-primary text-white"  :disabled="disable" :loading="load" @click="send">{{ codeText }}</button>
					</view>
					<view class="flex bgwhite alcenter bdb_myblue ">
					...
					<view class="mt20 flex alcenter">
						...
						<navigator :url="'/pages/home/agreement'">
							<view class="ml10 text-primary">《{{$t('login').p_private}}》</view>
						</navigator>
					</view>
					<view class="mt45 rounded-3xl ptb10 ft14 tc mb10 bg-primary text-white" @tap="register">{{$t('login.register')}}</view>
				</view>
			</view>
			...
			register(){
				...
				this.$utils.initData({url:'user/register',data,type:'POST'},(res,msg)=>{
					...
					if(res.jump&&this.from_invite){
						...
						this.$store.commit('setUser',{account:this.user_string,password:this.password})
						uni.reLaunch({
							url:'login?from=register'
						})
					}
				})
			}
