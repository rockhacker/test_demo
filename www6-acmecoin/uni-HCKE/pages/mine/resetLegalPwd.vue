<template>
	<view class="min-h-screen"  :class="theme">
		<view class="bg-my-grey dark:bg-1C2532 p-40 min-h-screen">
			...
			<view class="my-50">
				<view v-if="hiddenOld" class="flex items-center border-0 border-solid border-b-2 border-gray-200 dark:border-gray-700 ">
			...
			<view class="mt-100 bg-primary rounded-10 p-20 text-white text-lg text-center" @tap="register" hover-class="opacity-50">{{ $t('login').e_confrim }}</view>
		</view>
	</view>
</template>
		...
		methods: {
			...
			getUserInfo() {
				...
				this.$utils.initDataToken({
					...
					console.log(res);
					this.hiddenOld = res.pay_password !== ''
