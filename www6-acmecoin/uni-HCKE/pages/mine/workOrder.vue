<template>
	<view :class="theme">
		<view class="min-h-screen bg-my-grey dark:bg-my-black">
			<view class="ptb10 w100 tc flex alcenter between fixed  head pr10 bg-white dark:bg-box">
		...
		<view class="plr15 pt50">
			<view class="flex alcenter mt10 plr15 h40  between bg-white dark:bg-box rounded-10" @tap="actionSheetTap">
			...
			<input type="text" value="" class="h40 plr20  w100 bg-white dark:bg-box rounded-10" :placeholder="$t('feed.p_title')" v-model="title" />
			<view class="ptb10 ft13 mt15">{{$t('feed.content')}}</view>
			<view class="ptb20 bg-white dark:bg-box rounded-10">
			...
			<view class="mt-100 mb-60 bg-primary rounded-3xl p-20 text-white text-lg text-center" @click="confirm" hover-class="opacity-50">{{$t('login.e_confrim')}}</view>
		</view>
		</view>
	</view>
</template>
