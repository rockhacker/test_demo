<template>
	<view :class="theme">
		<view class="text-black dark:text-white dark:bg-my-black w-full min-h-screen">
			<uni-nav-bar fixed backIcon>
				<view class="flex1 flex items-center justify-between">
					<view class="flex-1 p-30 bg-white dark:bg-box flex justify-center items-center text-2xl">
					{{$t('staking.kuanjishangcheng')}}
				</view>

				<!-- <navigator url="/pages/staking/describle" class="text-base">{{$t('new.hdjs')}}</navigator> -->
				</view>
				<slot name="right">
			...
			<scroll-view scroll-y class="w-full">
				<view class="p-30 pt-0">
					<view class="justify-between rounded-10 bg-white dark:bg-box text-black dark:text-white px-20 py-30 mt-30"
						...
						<navigator :url="`/pages/staking/details?id=${item.id}`"
							class="mt-20 rounded-3xl p-20 text-white text-sm text-center bg-primary">
							{{ $t("staking.shengou") }}
						</navigator>
