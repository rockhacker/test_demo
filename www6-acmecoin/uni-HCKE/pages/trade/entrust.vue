<template>
	<view :class="theme">
		<view  class="min-h-screen dark:bg-box" >
			<view class="flex flexend between plr15 ptb10 w100 bdb_1e fixed lf0 dark:bg-box zdx100">
			<view class="flex flexend">
				<view :class="['ft14 gray7',{'ft18 text-primary':current==0}]" @click="change(0)">{{$t('trade.all_delegate')}}</view>
			    <view :class="['pl10 ft14 gray7',{'ft18 text-primary':current==1}]" @click="change(1)">{{$t('trade.his_delegate')}}</view>
			</view>
		...
		<uni-popup :show="popType === 'bottom-entrust'" position="bottom" @hidePopup="togglePopup('')">
			<view class="bottom-content">
				<view class="bottom-content-box tc gray7">
				    <view :class="['bdb_1e ptb10 ft14 bgPart',{'text-primary':enType=='in'}]" @click="changeEntrustType('in')">{{$t('trade.buy')}}</view>
					<view :class="['ptb10 ft14 bgPart',{'text-primary':enType=='out'}]" @click="changeEntrustType('out')">{{$t('trade.sell')}}</view>
		            <view class="">
			...
			<view class="ptb10 bdb_1e pr15" v-for="(item,index) in enList" :key="item.id">
				<view class="flex between alcenter">
					<view class="flex flex flexend">
						...
						<text class="ft14" v-if="item.currency_match">{{item.currency_match.base_currency_code}}/{{item.currency_match.quote_currency_code}}</text>
						<text class="pl5 ft12">{{item.created_at.substring(10,16)}} {{item.created_at.substring(5,7)}}/{{item.created_at.substring(8,11)}}</text>
					</view>
					<view class="bg-primary bgBlack radius2 plr20 ptb5" @click="revoke(index,item.id)">{{$t('trade.back')}}</view>
				</view>
			...
			<view class="ptb10 bdb_1e pr15" v-for="item in hisList" :key="item.id">
				<view class="flex between alcenter">
					<view class="flex flex flexend">
						...
						<text class="text-white pl5 ft14">{{item.currency_match.base_currency_code}}/{{item.currency_match.quote_currency_code}}</text>
