<template>
	<view class="pb-100" :class="theme">
		<view class="dark:bg-my-black">
			<!-- 头部 -->
		<view class="h-88 w-full bg-box fixed flex alcenter justify-between plr15 zdx100">
			<view class="flex alcenter" @click="showLeft=true">
				<image src="/static/image/list.png" mode="aspectFit" class="w-40 h-40 mr-10"></image>
		...
		<template v-else>
			<view class="flex pt-110 pb15 bg-white dark:bg-my-black">
				<!-- 左侧 -->
				<view class="flex1 plr15 left-flex">
					
					<view class="flex alcenter tc overxy posRelt  ">
						<view :class="['flex1  flex-after  text-xl',{'active text-primary':type=='buy'},{'text-333333 dark:text-999FAA':type!='buy'}]" @click="changeType('buy')">{{$t('trade').buy}}</view>
						<!-- <view class="bor abstrot mt5 bg-white dark:bg-my-black"></view> -->
						<view :class="['flex1  flex-after text-xl',{'active text-primary':type=='sell'},{'text-333333 dark:text-999FAA':type!='sell'}]" @click="changeType('sell')">{{$t('trade').sell}}</view>
					</view>
					<view class="flex alcenter text-lg justify-between py-10 dark:bg-box mb-10 px-10"  @click="togglePopup('bottom-share')" >
						<image @click.stop="showTip = true" src="/static/image/warning.png" class="w-26 h-26 ml-10"></image>
						<text class="dark:text-gray1 py-6" data-position="bottom">{{priceType=='limit' ? $t('trade').limit : $t('trade').shi}}</text>
						<image src="/static/image/down-arrow.png" class="w-20 h-14 mr-10"></image>
					</view>
					<!-- 限市价切换弹窗 -->
					<uni-popup :show="popType === 'bottom-share'" position="bottom" @hidePopup="togglePopup('')">
						<view class="bottom-content">
							<view class="bottom-content-box tc ">
								<view :class="['bdb_1e ptb10 ft14 bg-white dark:bg-my-black',{'text-primary':priceType=='limit'}]" @click="changePriceType('limit')">{{$t('trade').limit}}</view>
								<view :class="['ptb10 ft14 bg-white dark:bg-my-black',{'text-primary':priceType=='market'}]" @click="changePriceType('market')">{{$t('trade').shi}}</view>
								<view class="pt5 pb10">
									<view class="ptb10 ft14 bg-white dark:bg-my-black" @click="togglePopup('')">{{$t('trade').cancel}}</view>
								</view>
							</view>
						</view>
					</uni-popup>
					<view class="">
						<view class="" v-show="priceType=='limit'">
							<view class="dark:bg-box input-pd">
								<view class="text-838B99 text-lg">{{$t('trade.price')}}</view>
								<input type="number" v-model='price' :placeholder="$t('trade.p_price')" class="dark:text-white h40 flex1 ft14" />
						...
						<view class="mt-6 dark:bg-box input-pd">
							<view class="input-title">
								<text>{{$t('trade.num')}}</text>
								<text class="fee">{{$t('trade.fee')}}:{{parseFloat(fee)}}%</text>
							</view>
							<input type="number" v-model="number" :placeholder="$t('trade.p_num')" class="h40 flex1 dark:text-white" @input="inputNum" />
						...
						<view class="py-10">
							...
							<view class="flex alcenter around tc tab-box gray7 input-title">
								<view :class="['dark:bg-box flex1 radius2',{'bg-k-green text-white bd_green': percent==item&&type=='buy'},{'bg-k-red text-white bd_red': percent==item&&type=='sell'},{'ml10':index>0}]" v-for="(item,index) in percentList" :key='index' @click="changePercent(item)">{{item}}%</view>
							</view>
						</view>
						<view class="py-10">
				...
				<view class="trade-left flex column">
					...
					<view class="text-gray1 dark:bg-box p-10 rounded-8 text-center mt-10 mr-10" @click="toDeal">{{$t('new.deal')}}></view>
			...
			<view class="flex alcenter between ptb10  plr15 footer pb-10 border-0 border-solid border-b-2 border-E5E5E5 dark:border-999FAA">
				<view class="flex flexend">
					<text class="text-xl text-primary">{{$t('trade.delegate')}}</text>
				...
				<view class="flex alcenter gray7">
					...
					<navigator class="ml-20 text-lg text-gray1 dark:bg-box py-10 px-40 rounded-8 text-center" url="entrust">{{$t('new.newAll')}}</navigator>
				</view>
			</view>
			<!-- 委托类型切换弹窗 -->
			<uni-popup :show="popType === 'bottom-entrust'" position="bottom" @hidePopup="togglePopup('')">
				<view class="bottom-content">
					<view class="bottom-content-box tc gray7">
					    <view :class="['bdb_1e ptb10 ft14 bg-white dark:bg-my-black',{'text-primary':enType=='in'}]" @click="changeEntrustType('in')">{{$t('trade.buy')}}</view>
						<view :class="['ptb10 ft14 bg-white dark:bg-my-black',{'text-primary':enType=='out'}]" @click="changeEntrustType('out')">{{$t('trade.sell')}}</view>
			            <view class="pt5 pb10">
			            	<view class="ptb10 ft14 bg-white dark:bg-my-black" @click="togglePopup('')">{{$t('trade.cancel')}}</view>
			            </view>
					</view>
			...
			<view class="en-ul">
				<view class="ptb10 bdb_1e plr15" v-for="(item,index) in enList" :key="item.id">
					<view class="flex between alcenter">
						...
						<view class="text-primary bgBlack radius2 plr20 ptb5" @click="revoke(index,item.id)">{{$t('trade.back')}}</view>
					</view>
				...
				that.$utils.initData({ url: 'market/currency_matches' }, res => {
					if (res.length > 0) {
						...
						if(localData){
							...
							that.soon = arrFrist.currency_name=='ETHFXX' && !that.isOnline(localData.market_time);
							...
							that.soon = arrFrist.base_currency_code=='ETHFXX' && !that.isOnline(arrFrist.market_time);
