<template>
	<div class="minh100 ft16">
		<div class="mauto wt1200 ptb20">
			...
			<el-card class="ft22 mb20">
				{{$t('subscription.new_subscription')}}
				<el-link style="float: right; padding: 3px 0" type="warning" @click="$router.push({path:'/subscriptionOrders'})">{{$t('subscription.my_subscription')}}</el-link>
			</el-card>
			
			<el-card class="box-card mb20" @click="$router.push({path:'/subscriptionDetails?id=0'})" hover-class="opacity-50" v-for="(item,index) in list" :key="index">
				<div class="flex between">
					<div class="ft22 bold">{{item.currency_name}} / {{$t('subscription.new_subscription')}}</div>
					<el-button type="warning" class="wt100 mr10" style="border-radius: 0;" @click="$router.push({path:`/subscriptionDetails?id=${item.id}`})">{{$t('subscription.canjiahuodong')}}</el-button>
				</div>
				...
				<div class="ft16 flex between ptb20 alcenter">
					<div class="flex">
						<span class="black bold">{{item.subscribed}} {{item.currency_name}} </span>
				...
				<div class="w100 ht30 radius40 bgf6 posRelt overxy">
					<div class="abstort lf0 h100 lh30 bgyellow white ft14 tr" :style="`width: ${remainingRatio(item.subscribed,item.issue_number)}%;`">
