...
Vue.component('page-footer',{
    data(){
        ...
        hi:{
          "tranMode": "ट्रांसेक्शन मोड",
          "numOpen": "खोलने के स्थितियों की संख्या",
          "openTime": "समय खोल रहा है",
          "Profitability": "प्रोफिटीबिलिटी",
          "buy": "खरीद करें",
          "sell": "खरीद या पड़ें",
          "quantity": "मात्रा",
          "time": "समय",
          ...
          "ordered": "सफलतापूर्वक अनुक्रमित",
          "income": "आयात",
          "openNumber": "खुली स्थितियों की संख्या गुम है",
          "contract": "संपर्क खाता",
          "seleteNum": "कृपया खोलने की मात्रा भरें या चुनें",
          "tian": "दिन",
          "shi": "घंटा",
          "fenz": "मिनट",
          "miao": "सेकंड",
    ...
    methods:{
      setCoupon(){
        ...
        uni.navigateTo({
          url:`/pages/mine/coupon?currency_id=${this.currencyItem.id}&from=tab&id=${this.couponInfo?this.couponInfo.id:undefined}`
        })
      },
        secondsFormat(s) { 
    ...
    created(){
        ...
        if (window.plus) {
            ...
            that.couponInfo = plus.storage.getItem('couponInfo')
            plus.storage.removeItem('couponInfo')
        } else {
            that.lang = localStorage.getItem('lang') || 'zh';
            that.couponInfo = localStorage.getItem('couponInfo')
            localStorage.removeItem('couponInfo')
        }
        if(that.couponInfo && that.couponInfo !='-1'){
          that.couponInfo = JSON.parse(that.couponInfo)
          console.log(that.couponInfo,'couponInfo');
        }
    },
                            ...
                            <div v-show="isSubmit" class="sub-select">
                               <!-- <div class="icon-text">
                                ...
                                <div class="footer-btn slide-box" >
                                    <div class="btn-item slide-item dark:btn-item" :class="item.code==currencyItem.code?'active':''" v-for="(item,index) in currencyList" :key="index" @click="setCurrency(item)">
                                            <span>{{item.code}}</span>
                                            <img v-show="item.code==currencyItem.code" class="icon" src="children/image/icon.png" alt="" >
                                    </div>
                                </div>  
                                
                                <div v-if="false" class="icon-text" style="justify-content: space-between;" @click="setCoupon">
                                   <span>{{transwords[lang].xzzjq}}</span>
                                   <div style="display:flex;align-items: center;">
                                    <span style="margin-right:0px;" v-if="couponInfo">{{parseFloat(couponInfo.amount)}}{{couponInfo.currency.code}}</span>
                                    <img src="../../static/image/mores.png" />
                                   </div>
								...
								<div class="foot-btn">
								    <div class="sub-btn">
								        <button class="bottom" @click="submit('sell')">{{transwords[lang].sell}}</button>
								        <button class="top" @click="submit('buy')">{{transwords[lang].buy}}</button>
								    </div>
								</div>
                            </div>  
          ...
          <div>
            <div class="item" v-if="active==1" v-for="(item,index) in list" :key="index">
              ...
              <div class="item-center">
               ...
               <div class="item-center top" style="justify-content: flex-start;">
                  <p style="text-align: left;">
                    <span>{{transwords[lang].Countdown}}</span>
                    <span>{{secondsFormat(item.remain_milli_seconds)||0}}</span>
                  </p>
                  <p style="text-align: center;    padding-left: 20px;                  ">
                    <span>{{transwords[lang].dotime}}</span>
