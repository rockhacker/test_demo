<template>
	<view :class="theme">
		<view class="text-black dark:text-gray-400 bg-white dark:bg-new-black">
			...
			<uni-nav-bar fixed>
				...
				<view class="flex-1 flex justify-center items-center text-2xl h-100">{{ $t("market.home") }}</view>
				<view class="absolute right-0 top-0 z-10 h-100">
					<view class="flex flex-1 relative h-full flex items-center justify-center px-20">
						<image @click="toPath('/pages/home/chat')" src="/static/image/kefu.png" class="w-50 h-50 mr-20"></image>
						...
						<view class="absolute right-0 top-100 w-200" v-show="isshowlang"
							@tap="isshowlang = !isshowlang">
							<view class="bg-white dark:bg-gray-800 px-20 relative z-20">
								<view
									class="flex items-center py-20 border-0 border-b-2 border-solid border-gray-100 dark:border-gray-900 justify-between"
			...
			<!--轮播-->
			<xlbsh-swiper :imgList="bannerList.map((item)=>item.banner)"></xlbsh-swiper>

			<view class="mx-30 rounded-sm flex py-40 box-border items-center justify-center dark:text-white bg-gray-100 dark:bg-new-black2">
				<view class="flex items-center flex-1 justify-center flex-col text-center" @click="toPath(item.path)"
					v-for="item,index in functionModule" :key="index">
		  ...
		  <view class="flex items-center flex-1 justify-center flex-col text-center" @click="toPath('/pages/legal/legal')">
			...
			<view class="mx-30 rounded-sm flex py-20 box-border items-center justify-center text-white">
				<view class="h-160 flex-1 flex items-center justify-around px-10 bg-gray-100 dark:bg-new-black2 mr-20 rounded-sm"
					@click="toPath('/pages/home/recharge')">
					<view class="flex  flex-col">
						<text class="text-black dark:text-gray-400 font-bold text-lg">{{ $t("new.clickre") }}</text>
						<text class="text-black dark:text-gray-400 pt-8">{{$t('new.fl1')}} BTC,ETH,</text>
						<text class="text-black dark:text-gray-400 pt-8">USDC,USDT…</text>
					</view>
					<image :src="`../../static/image/${theme!='dark'?'light-':''}fast-charge.png`" class="w-80 h-80"></image>
				</view>
				<view class="h-160 flex-1 flex items-center justify-around px-10 bg-gray-100 dark:bg-new-black2 ml-20 rounded-sm"
					@click="toPath('/pages/trade/trade')">
					<view class="flex  flex-col">
						<view class="text-black dark:text-gray-400 font-bold text-lg">{{ $t("trade.trade") }}</view>
						<!-- <text class="text-black dark:text-gray-400 pt-8">{{$t('new.tdzh')}}</text>
						<text class="text-black dark:text-gray-400 pt-8">{{$t('new.qd')}}</text> -->
					</view>
					<image :src="`../../static/image/${theme!='dark'?'dark-':''}qiandao.png`" class="w-80 h-80"></image>
				</view>
			</view>

			<!-- <view class="bg-gray-100 dark:bg-new-black2 mx-30 rounded-sm flex px-30 py-10 text-white mb-20 items-center">
				<image :src="`../../static/image/laba.png`" class="w-60 h-60"></image>
				<view class="mx-20 text-xl text-black dark:text-white">{{$t('new.zx')}}</view>
				<view class="text-black dark:text-gray-500">{{$t('new.zygg')}}</view>
			</view> -->
			...
			<!--涨幅榜-->
			<swiper v-if="quoList.length" :display-multiple-items="quoList.length>2?3:quoList.length" class="w-full h-190 bg-white dark:bg-new-black px-20">
		  <swiper-item class="flex items-start justify-around"  v-for="(item, i) in quoList" :key="i">
			<view @click="toUrl(item,allList[1])" class="flex flex-col items-center w-200 h-170 justify-center rounded-sm relative bg-gray-100 dark:bg-new-black2">
				<text class="text-1A222B mb-20" style="font-size: 28rpx;">{{ item.base_currency_code }}/{{ item.quote_currency_code }}</text>
			...
			<!--行情-->
			<view class="pb-100 w-full bg-white dark:bg-new-black">
				<!-- <view class="ptb10 bdb_blue3 ft14 plr10 bold zhang">主流区</view> -->
				<view class="flex px-40 pt-20">
					<view class="trading-item text-center mr-40 " v-for="(item, index) in allList" :key="index"
						@click="setItem(item)">
						<view class="text-lg px-10 py-16 rounded-10" :class="itemCode == item.code ? 'bg-primary text-black' : 'text-primary'">{{ item.code }}
						</view>
					</view>
				</view>
				...
				<view @click="toUrl(item)" class="flex items-center text-sm text-gray-400 tableitem px-40 py-30"
					v-for="(item, i) in tabList" :key="i">
					<view class="flex items-center" style="flex: 1.5">
						<image class="w-70 h-70 mr-16 rounded-half bg-white" :src="host + item.base_currency.logo">
						</image>
						<view class="text-sm"><text
								class="text-2xl text-black dark:text-white">{{ item.base_currency_code }}</text>/{{ item.quote_currency_code }}
						</view>
						<!-- <view class="text-sm">24H量 {{item.volume}}</view> -->
					</view>
					<view v-if="item.base_currency_code=='oec' && !isOnline(item.market_time)" class="w-200">
						{{$t('trade.soon')}}</view>
					<template v-else>
						<view class="flex-1 text-white text-xl text-gray-400" v-if="item.currency_quotation">
							{{ parseFloat(item.currency_quotation.close) }}</view>
						<view class="flex-1 text-right flex justify-end" v-if="item.currency_quotation">
							<view class="w-140 h-60 flex justify-center items-center rounded-8 text-center text-white"
								:class="{ 'bg-danger': parseFloat(item.currency_quotation.change) < 0, 'bg-success':parseFloat(item.currency_quotation.change) >= 0 ||item.currency_quotation.change == ''}">
								{{ item.currency_quotation.change.substr(0, 1) == "-" ? "" : "+"}}{{ (item.currency_quotation.change - 0) | toFixed2 }}%
							</view>
						</view>
					</template>
			...
			<!-- 下单弹窗 -->
			<uni-popup :show="announcementModal.showModal" @hidePopup="hideBtn" :msg="announcementModal.title"
				:btnShow="announcementModal.tipbtnShow" :lang="lang">
	...
	export default {
		...
		data() {
			return {
				...
				languages: {
					...
					vn: {
						name: 'Tiếng Việt',
						img: '/static/image/vn.png'
					},
					th: {
						name: 'ไทย',
						img: '/static/image/th.png'
					},
				...
				functionModule: [{
					...
					// {
					// 	img: 'dsf.png',
					// 	path: '/pages/home/rechargeThirdParty',
					// 	// name: 'legal.legal'
					// 	name:'new.dsfcz'
					// },
					// {
		...
		methods: {
			...
			topo() {
				...
				window.open(`https://${this.$utils.domain}/wp/oec-${lang}.pdf`);
			...
			toOnline() {
				...
				let winOpen = window.open("", "_blank");
				let url =
					"https://chat.cointigerex.cc/index/index/home?visiter_id=&visiter_name=&avatar=&groupid=0&business_id=3"
				setTimeout(() => {
					winOpen.location = url;
				})
			},
			...
			toPath(url) {
				if (url == '/pages/contract/lever' || url == '/pages/assets/assets'  || url == '/pages/trade/trade') {
