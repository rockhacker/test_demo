<template>
	<view :class="theme" >
         <view class="bg-white dark:bg-floor flex justify-center items-center flex-col p-30" style="min-height:100%">
              <image class="w-300 h-300 p-30"  :src="qr" @click="TanPreviewImage(qr)" mode="" ></image>
              <view class="mb-10 text-sm text-center">
				<text>{{ $t('assets.c_tip2_1') }}</text>
				<text>{{ obj.currency_code || '--' }} </text>
				<text>{{ $t('assets.c_tip2_2') }}，</text>
				<text>{{ $t('assets.c_tip2_3') }}。</text>
			</view>
              <!-- <button class="btn" @click="save(qr)">{{$t('feed.qr')}}</button> -->
              <view class="text-xl my-40">{{obj.currency_code}}{{$t('feed.address')}}</view>
              <view class="text-lg mb-40">{{obj.address}}</view>
              
			  <button class="w-full rounded-10 bg-primary text-black text-xl mt-100" @click="setCopy">{{$t('feed.copyadd')}}</button>
