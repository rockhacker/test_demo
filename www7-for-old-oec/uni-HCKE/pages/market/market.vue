<template>
	<view :class="theme" class="flex flex-col">
		<view class="bg-white dark:bg-floor text-black">
			...
			<view class="w-full flex flex-col">
				<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
					<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
						...
						:class="{'text-primary dark:text-primary':isActive==i}" v-for="(item,i) in marketLists" :key="i" 
		...
		<view class="bg-white dark:bg-gray-600 dark:bg-opacity-30 dark:text-white pb-100" style="min-height: calc(100vh - 100px);">
			<view @click="toPath(item)" class="flex items-center px-40 h-120" v-for="(item,i) in matches" v-if="marketLists[isActive]" :key="i" v-show="marketLists&&marketLists[isActive]&&item.open_change==1">
				<view class="flex-2">
					<view><text class="text-lg font-bold">{{item.base_currency_code}}</text><text class="text-sm text-gray-400 pl-14"> / {{item.quote_currency_code}}</text></view>
