<template>
  <view :class="theme">
    <view v-if="authStatus == 0" class="bg-white dark:bg-floor px-40 mt-20" style="min-height: calc(100vh - 40px);">
	  ...
	  <view class="mt-30 text-lg">{{$t('new.zjsm')}}</view>
			<view class="mt-20">
				...
				<view class="w-full rounded-8 text-center py-40" @tap="uploadImg(2)">
      			...
      			<view class="w-full rounded-8 text-center py-40" @tap="uploadImg(3)">
			...
			<view class="mt-100 h-80 flex justify-center items-center text-center bg-primary text-black text-lg rounded-10" @tap="confirm">{{ $t("login.e_confrim") }}</view>
    </view>
    <view class="bg-white dark:bg-floor text-2xl text-center pt-200" v-if="authStatus == 1">
      {{ $t("authentication.ing") }}
    </view>
    <view class="bg-white dark:bg-floor text-2xl text-center pt-200" v-if="authStatus == 2">
