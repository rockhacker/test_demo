<template>
	<div class="minh100 ft16">
		<div class="mauto wt1200 ptb20">
			...
			<el-card class="ft22 mb20 f-item dark-text-white">
			...
			<el-card class="box-card mb20 f-item" @click="$router.push({path:'/subscriptionDetails?id=0'})" hover-class="opacity-50" v-for="(item,index) in list" :key="index">
				<div class="flex between">
					<div class="ft22 bold dark-text-white">{{item.currency_name}} / {{$t('subscription.new_subscription')}}</div>
					<el-button class="mr10 bg-main-liner white" style="border-radius: 0;" @click="$router.push({path:`/subscriptionDetails?id=${item.id}`})">{{$t('subscription.canjiahuodong')}}</el-button>
				</div>
