	...
	export default{
		data(){
			return {
				...
				name:'',
				coinInfo:{},
				changeName:[this.$t('assets.legalacc'),this.$t('assets.leveracc')],
				changeType:['legal','lever'],
				type:0,//法币兑合约
				hasChange:0,
				animateTab1:'',
				animateTab2:'',
		...
		methods: {
			getList(){
				this.$utils.initDataToken({url:'wallet/list',type:'POST'},res=>{
					...
					var select_lever_wallet=lever_wallet.filter(item=>item.currency_name==this.name)||[];
					if(select_lever_wallet.length>=0){
						this.currencyLever=select_lever_wallet[0];
						if(this.type==1){
							this.balance=select_lever_wallet[0].lever_balance;
						}
					}
			...
			tapChange(){
				...
				if(this.type==0){
					...
					this.balance=this.currencyLever.lever_balance;
				}
				this.hasChange++;
