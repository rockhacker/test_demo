<template>
  <view class="text-white" :class="theme">
    <view class="bgPart px-30 bg-white dark:bg-my-black min-h-screen" v-if="info.recharge_account">
      ...
      <view class="flex alcenter between" v-if="info.recharge_account">
        ...
        <view class="w-full flex items-center border-0 border-b-2 border-solid border-gray-200 dark:border-gray-600">
            <input class="flex-1 h-80 text-xl" type="number" v-model="number" placeholder="0.00">
            <view class="text-xl px-20 border-0 border-l-2 border-solid border-gray-200 dark:border-gray-600">{{type==1?info.recharge_account.Code:info.settle_account.CurrencyName}}</view>
        </view>
        <button class="w-full rounded-10 bg-main-linear text-black text-xl mt-100" @click="submit">{{$t('new.duih')}}</button>
    </view>
  </view>
</template>
