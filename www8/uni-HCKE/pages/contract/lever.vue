<template>
	<view class="text-white" :class="theme">
		<view class="bg-my-grey dark:bg-my-black">
			<uni-nav-bar fixed>
				<view class="w-full flex flex-col p-40 dark:bg-my-black">
					<view class="w-full flex justify-center">
						<view class="flex w-full h-60 bg-F4F5F7 dark:bg-gray-800 rounded-2xl overflow-hidden">
							<view class="flex-1 flex justify-center items-center text-center" :class="active==0?'bg-main-linear text-black':'text-333333 dark:text-white'" @tap="setActive(0)">{{$t('new.leverLes')}}</view>
							<view class="flex-1 flex justify-center items-center text-center" :class="active==1?'bg-main-linear text-black':'text-333333 dark:text-white'" @click="setActive(1)">{{$t('new.leverAll')}}</view>
							<!-- <view class="flex-1 flex justify-center items-center text-center" :class="active==2?'bg-main-linear text-black':'text-333333 dark:text-white'" @tap="setActive(2)">{{$t('qqjy.qqjy')}}</view> -->
						</view>
					</view>
					...
					<view class="flex justify-between items-center mt-20" v-if="active!=2">
						<view class="flex items-center" @tap="leftStatus = true">
							...
							<view class="mx-20 text-lg font-bold text-white">{{ baseCurrencyCode }} / {{ quoteCurrencyCode }}</view>
			...
			<view v-if="active==1 && !soon" class="min-h-screen bg-gray-100 dark:bg-my-black">
				<!-- <view class="flex justify-center items-center text-3xl h-600" v-if="soon">{{$t('trade.soon')}}</view> -->
				<view class="flex justify-between p-20 bg-white dark:bg-1C2532">
					<view class="w-3_5">
						<view class="overflow-hidden flex justify-between rounded-10 bg-gray-100 dark:bg-my-black dark:text-white">
							<view class="text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'buy' ? 'bg-success text-white' : ''" @tap="tradeType('buy')">
								...
								:style="`border-top: 16px solid #${ theme == 'light'?'f3f4f6':'0D081A' };border-left: 16px solid transparent;border-bottom: 18px solid #${ theme == 'light'?'f3f4f6':'0D081A' };`"></view>
							</view>
							<view class="text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'sell' ? 'bg-danger text-white' : ''" @tap="tradeType('sell')">
								...
								:style="`border-top: 16px solid #${ theme == 'light'?'f3f4f6':'0D081A' };border-right: 16px solid transparent;border-bottom: 18px solid #${ theme == 'light'?'f3f4f6':'0D081A' };`"></view>
							</view>
						</view>
						<view class="flex item-center justify-between mt-20">
							<view class="flex items-center justify-between border-2 border-solid border-B8C1CF dark:border-999FAA flex-2 h-60 pr-10 rounded-8" @tap="tradeTypes.shows = true">
								<view class="flex-1 text-center text-white">{{ priceStatus == 1 ? $t('trade.shi') : $t('trade.limit') }}</view>
							...
							<view class="flex-1 h-60 ml-10">
								<view class="border-2 border-solid  border-B8C1CF dark:border-999FAA h-60 flex items-center justify-between pr-10 rounded-8" @tap="mults.shows = true">
									<text class="flex-1 text-center text-white" >{{ leverOrder.mult }}</text>
									<image src="../../static/image/pull.png" mode="aspectFit" class="w-30 h-30"></image>
								</view>
							</view>
					...
					<view class="mt-40">
						...
						<view class="" v-if="orderList.length > 0">
							<view v-for="(item, index) in orderList" :key="index" class="flex justify-between items-center mt-20">
								...
								<view class="flex-1 text-center bg-main-linear py-10 rounded-8 ml-20 text-black" @tap="closeTrade(item.id)">{{ $t('trade.pingcang') }}</view>
							</view>
						</view>
				...
				<view class="pt-bar">
					<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
						<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
							...
							{{item.code}}
							<view class="w-60 h-6 mt-10" :class="{'bg-main-linear text-black':quoteCurrencyIds == item.id}"></view>
						</view>
					</scroll-view>
				</view>
