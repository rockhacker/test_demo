			...
			<view class="px-40">
				<view v-for="(item, index) in orderList" :key="index" class=" border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 py-20">
					...
					<view class="flex text-right mt-30 justify-end" v-if="status == 0 || status == 1">
						<view class="bg-main-linear px-30 py-20 rounded-8 text-black" @tap="loss(item,index)">{{$t('lever.setys')}}</view>
						<view class="bg-main-linear px-30 py-20 rounded-8 text-black ml-20" @tap="closes(item.id, index)">{{ status == 0 ? $t('trade.chedan') : $t('trade.pingcang') }}</view>
					</view>
				</view>
			</view>
