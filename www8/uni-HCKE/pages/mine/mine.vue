		...
		<view class="w-full bg-my-grey dark:bg-my-black">
			<view class="w-full bg-white dark:bg-1C2532 p-30">
				<view class="w-full flex justify-between">
					...
					<button type="primary" size="default" class="text-sm px-20 bg-main-linear text-black rounded-10" @click="logout">
			...
			<view class="mt-20 w-full px-20 pb-20">
				<view class="bg-main-linear rounded-lg p-40 text-black">
					<view class="flex justify-between">
						<view class="text-xl font-bold text-clip-yellow dark:text-black">{{ $t("new.mine") }}</view>
					...
					<view class="my-20 text-xs text-clip-yellow dark:text-black">{{ $t("new.zichan") }} (USDT) </view>
					<view class="text-clip-yellow dark:text-black text-lg my-20">{{ active==1 && token?(all?all.toFixed(2):all):'******' }} </view>
					<navigator url="/pages/assets/assets" class="flex justify-between items-center">
						<view class="text-black rounded-3xl px-20 py-10" :class="[theme=='dark'?'dark:bg-white':'bg-yellow-liner']">
	...
	<view class="bg-white dark:bg-1C2532 pb-180">
		...
		<!-- 	<navigator v-if="info.is_seller" url="/pages/legal/storeDetail" class="flex items-center py-30 justify-between text-lg border-0 border-solid border-b-2 border-EEEEEE dark:border-my-black">
			...
			<navigator v-if="token" :url="'/pages/mine/invite?code=' + info.invite_code" class="flex items-center py-30 justify-between text-lg border-0 border-solid border-b-2 border-EEEEEE dark:border-my-black">
				<view class="flex items-center">
					<view class="w-6 h-26 mr-20 bg-main-linear"></view>
					<view class="text-333333 dark:text-white text-sm">{{ $t("home.myshare") }}</view>
				</view>
				<image src="/static/image/mores.png" class="w-40 h-40"></image>
			</navigator>
