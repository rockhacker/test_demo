<template>
	<view class="min-h-screen" :class="theme">
		<view class="min-h-screen bg-my-grey dark:bg-my-black">
			...
			<view class="flex-1 p-30 bg-white dark:bg-1C2532 text-black dark:text-white flex justify-center items-center text-xl">
				{{$t('subscription.new_subscription')}}
			</view>
			<slot name="right">
				<navigator url="./orders" class="px-10 py-10 border-solid border-2 border-green-liner rounded-10 absolute top-24 right-20 z-10 text-clip-green" >{{$t('subscription.my_subscription')}}</navigator>
			</slot>
		</uni-nav-bar>
			
		<view class="p-30" v-if="list.length">
			<navigator class="rounded-10 new-bg dark:text-white p-20 mb-20" :url="`./details?id=${item.id}`" hover-class="opacity-50" v-for="(item,index) in list" :key="index">
				<view class="rounded-10">
					<view class="flex justify-between items-center mb-10">
						...
						<view class="text-sm rounded-8 px-20 py-10 text-black bg-main-linear">
					...
					<view class="w-full h-30 rounded-2xl bg-gray-200 relative overflow-hidden">
						<view class="absolute left-0 h-full leading-30 bg-main-linear text-black text-sm text-right" :style="`width: ${remainingRatio(item.subscribed,item.issue_number)}%;`">
