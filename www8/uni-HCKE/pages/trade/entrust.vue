		...
		<uni-popup :show="popType === 'bottom-entrust'" position="bottom" @hidePopup="togglePopup('')">
			<view class="bottom-content">
				<view class="bottom-content-box tc gray7">
				    <view :class="['bdb_1e ptb10 ft14 ',{'blueColor':enType=='in'}]" @click="changeEntrustType('in')">{{$t('trade.buy')}}</view>
					<view :class="['ptb10 ft14 ',{'blueColor':enType=='out'}]" @click="changeEntrustType('out')">{{$t('trade.sell')}}</view>
		            <view class="pt5 pb10">
		            	<view class="ptb10 ft14 " @click="togglePopup('')">{{$t('trade.cancel')}}</view>
		            </view>
				</view>
			</view>
		</uni-popup>
			...
			<view class="ptb10 bdb_1e pr15" v-for="(item,index) in enList" :key="item.id">
				<view class="flex between alcenter">
					<view class="flex flex flexend">
						...
						<text class="text-white pl5 ft12">{{item.created_at.substring(10,16)}} {{item.created_at.substring(5,7)}}/{{item.created_at.substring(8,11)}}</text>
					</view>
		...
		<view class="en-ul pl15 pt50" v-show="current==1">
			<view class="ptb10 bdb_1e pr15" v-for="item in hisList" :key="item.id">
				...
				<view class="mt10 flex">
					...
					<view class="flex1 tc">
						<text class="text-white">{{$t('trade.cj_price')}}({{item.currency_match.quote_currency_code}})</text>
