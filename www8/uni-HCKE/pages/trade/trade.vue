<template>
	<view class="pb-100" :class="theme">
		<!-- 头部 -->
		<view class="h-88 w-full fixed flex alcenter jscenter plr15 zdx100">
		...
		<template v-else>
			<view class="flex pt-110 pb15 bg-white dark:bg-1C2532">
				...
				<view class="flex1 plr15 left-flex">
					...
					<view class="flex alcenter text-lg justify-between py-10 bg-gray-800 text-white rounded-8 mb-10 px-10"  @click="togglePopup('bottom-share')" >
						<image @click.stop="showTip = true" src="/static/image/warning.png" class="w-40 h-40 mr-10"></image>
						<text class="py-6" data-position="bottom">{{priceType=='limit' ? $t('trade').limit : $t('trade').shi}}</text>
					...
					<uni-popup :show="popType === 'bottom-share'" position="bottom" @hidePopup="togglePopup('')">
						<view class="bottom-content">
							<view class="bottom-content-box tc ">
								<view :class="['bdb_1e ptb10 ft14 bg-white dark:bg-1C2532',{'blueColor':priceType=='limit','text-white':priceType!='limit'}]" @click="changePriceType('limit')">{{$t('trade').limit}}</view>
								<view :class="['ptb10 ft14 bg-white dark:bg-1C2532',{'blueColor':priceType=='market','text-white':priceType!='market'}]" @click="changePriceType('market')">{{$t('trade').shi}}</view>
								<view class="pt5 pb10">
									<view class="ptb10 ft14 bg-white dark:bg-1C2532 text-white" @click="togglePopup('')">{{$t('trade').cancel}}</view>
								</view>
							</view>
						</view>
					</uni-popup>
					<view class="">
						<view class="" v-show="priceType=='limit'">
							<view class="bg-gray-800 rounded-8 input-pd">
								<view class="text-white text-lg">{{$t('trade.price')}}</view>
						...
						<view class="mt-6 bg-gray-800 rounded-8 input-pd">
						...
						<view class="py-10">
							...
							<view class="flex alcenter around tc tab-box gray7 input-title">
								<view :class="['bg-gray-800 flex1 radius2',{'bg-my-k-green text-white bd_green': percent==item&&type=='buy'},{'bg-my-k-red text-white bd_red': percent==item&&type=='sell'},{'ml10':index>0}]" v-for="(item,index) in percentList" :key='index' @click="changePercent(item)">{{item}}%</view>
							</view>
						</view>
						<view class="py-10">
				...
				<view class="trade-left flex column">
					...
					<view class="text-white bg-gray-800 p-10 rounded-8 text-center mt-10 mr-10" @click="toDeal">{{$t('new.deal')}}></view>
			...
			<view class="flex alcenter between ptb10  plr15 footer pb-10 border-0 border-solid border-b-2 border-E5E5E5 dark:border-999FAA">
				...
				<view class="flex alcenter gray7">
					...
					<navigator class="text-lg text-white bg-gray-800 py-10 px-40 rounded-8 text-center" url="entrust">{{$t('new.newAll')}}</navigator>
				</view>
			</view>
			<!-- 委托类型切换弹窗 -->
			<uni-popup :show="popType === 'bottom-entrust'" position="bottom" @hidePopup="togglePopup('')">
				<view class="bottom-content">
					<view class="bottom-content-box tc gray7">
					    <view :class="['bdb_1e ptb10 ft14 bg-white dark:bg-1C2532 text-white',{'blueColor':enType=='in'}]" @click="changeEntrustType('in')">{{$t('trade.buy')}}</view>
						<view :class="['ptb10 ft14 bg-white dark:bg-1C2532 text-white',{'blueColor':enType=='out'}]" @click="changeEntrustType('out')">{{$t('trade.sell')}}</view>
			            <view class="pt5 pb10">
			            	<view class="ptb10 ft14 bg-white dark:bg-1C2532 text-white" @click="togglePopup('')">{{$t('trade.cancel')}}</view>
			            </view>
					</view>
			...
			<view class="en-ul">
				<view class="ptb10 bdb_1e plr15" v-for="(item,index) in enList" :key="item.id">
					...
					<view class="mt15 flex text-white">
						<view class="flex1">
							<text class="text-white" v-if="item.currency_match">{{$t('trade.price')}}({{item.currency_match.quote_currency_code}})</text>
							<view class="mt5">{{item.price}}</view>
						</view>
						<view class="flex1 tc">
							<text class="text-white" v-if="item.currency_match">{{$t('trade.num')}}({{item.currency_match.base_currency_code}})</text>
							<view class="mt5">{{item.number}}</view>
						</view>
						<view class="flex1 tr">
							<text class="text-white" v-if="item.currency_match">{{$t('trade.real_num')}}({{item.currency_match.base_currency_code}})</text>
		...
		<view class="left-mode">
			<uni-drawer :visible="showLeft" style="background: rgb(16, 32, 48,.5);" mode="left" @close="showLeft = false">
				<view class="text-white flex column ht100">
					<view class="pt20">
