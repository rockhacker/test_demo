  ...
  <div>
    ...
    <div class="flex ft12 pb10">
      ...
      <div class="flex1 plr10 mt10 bdr38">
        ...
        <div class="flex alcenter  posRelt">
          <span class="w15">{{$t('lever.hands')}}</span>
          <div class="flex bdinput lh30 radius2 ml10 flex1 white mult">
            <!-- <input type="number" v-model="buyOrder.shares" class="nwhite flex2 lh30" /> -->
            <el-input-number class="flex1" v-model="buyOrder.shares"  :min="1" ></el-input-number>
            <div
              class="flex1 bdl383"
        ...
        <div class="flex between mt10" v-if="token">
          <span>{{$t('add.kkzs')}}</span>
          <span>{{ Math.floor((order_data.user_lever / currency_matchInfo.lever_share_amount)* 1)}}</span>
        </div>
        <div class="flex alcenter mb20 posRelt">
          <span class="w15"></span>
          <div class="flex1 pl5  ml10 pr10">
            <el-slider
            @change="changeValue"
            v-model="value1"
        ...
        <div class="bg-main-liner tc ht30 lh30 white radius2" @click="submits(1)" v-else>{{$t('lever.buy')}}</div>
      </div>
      <!-- 卖盘 -->
      <div class="flex1 plr10 mt10">
        ...
        <div class="flex alcenter  posRelt">
          ...
          <div class="flex bdinput lh30  radius2 ml10 flex1 white mult">
            ...
            <el-input-number class="flex1" v-model="sellOrder.shares"  :min="1" ></el-input-number>
            <div
              class="flex1 bdl383"
              v-if="order_data.multiple&&order_data.multiple.share.length >0"
            >
        ...
        <div class="bgred tc ht30 lh30 white radius2" @click="submits(2)" v-else>{{$t('lever.sell')}}</div>
      </div>
    </div>
    ...
    <el-dialog :title="$t('lever.confrim_order')" :visible.sync="orderShow" width="30%" :before-close="handleClose">
      <div class="mb15">
        ...
        <div class="flex pb10 alcenter between">
          ...
          <p v-if="submitType==1">{{ buyOrder.shares * currency_matchInfo.lever_fee_rate }}</p>
          <p v-else>{{ sellOrder.shares * currency_matchInfo.lever_fee_rate }}</p>
        </div>
        <div class="flex pb10 alcenter between">
          <p>{{$t('lever.money')}}</p>
          <!-- <p>{{leverOrder.bonds || '0.00' |filterDecimals(6)}}</p> -->
          <p v-if="submitType==1">{{ buyOrder.shares * currency_matchInfo.lever_share_amount }}</p>
          <p v-else>{{ sellOrder.shares * currency_matchInfo.lever_share_amount }}</p>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button class="radius4" @click="orderShow=false">{{$t('lever.cannel')}}</el-button>
        <el-button type="primary" @click="orderSubmit" class="radius4 bg-main-liner white">{{$t('lever.e_confrim2')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    changeValue(val){
      console.log(val)
      this.buyOrder.shares = Math.floor((this.order_data.user_lever / this.currency_matchInfo.lever_share_amount)* (val/100))
    },
    changeValue2(val){
      console.log(val)
      this.sellOrder.shares = Math.floor((this.order_data.user_lever / this.currency_matchInfo.lever_share_amount)* (val/100))
    },
    formatTooltip(val) {
        return val + '%';
    },
