    ...
    <div class="bgwhite dark-bg-1C2532 plr15 pb10 h180 overyscroll">
      <ul>
        <li class="flex alcenter ptb10">
          ...
          <span class="flex1 tc" v-if="status == 3">{{$t('lever.loss')}}</span>
          <span class="flex1 tc" v-if="status == 3">{{$t('trade.time_start')}}</span>
          <span class="flex1 tr" v-if="status == 3">{{$t('lever.closedTime')}}</span>
        </li>
      </ul>
      <ul>
        <li v-if="historyData.currentPosition.length > 0" v-for="(item,index) in historyData.currentPosition":key="index" class="flex between ptb5">
          ...
          <span class="flex1 tc" v-if="status == 3" :class="item.fact_profits <0?'redColor':'greenColor'">{{item.fact_profits || '0' |tofixedFour}}</span>
          <span class="flex1 tc" v-if="status == 3" :class="item.fact_profits <0?'redColor':'greenColor'">{{ item.created_at }}</span>
          <span class="flex1 tr" v-if="status == 3" :class="item.fact_profits <0?'redColor':'greenColor'">{{ item.handled_at }}</span>
        </li>
