<template>
    <div class="flex h100">
        <div class="flex2 flex column mr5">
            <!-- 行情 -->
            <div class="flex2">
                <div class="bgwhite dark-bg-1C2532 flex alcenter plr10 ht40">
                    ...
                    <div v-if="tradeList.length>0 && tradeList[currentIndex]">
                        <div class="flex alcenter ptb5 markethover pointer" :class="{'bgshadow':itm.symbol==symbol}"  v-for="(itm,j) in tradeList[currentIndex].matches" v-if="itm.open_change==1"  :key="j" @click="$emit('changeSymbol',j)">
                            <div class="flex1 pl10 baselight2">{{itm.base_currency_code}}</div>
							<div v-if="itm.base_currency_code=='blescoin' && !isOnline(itm.market_time)" class="mlr10">{{$t('trade.soon')}}</div>
							<template v-else >
                        ...
                        <div class="baselight2 ft16 bold mr20 pointer" slot="reference">{{symbol}} <i class="el-icon-caret-bottom"></i></div>
                    </el-popover>
                    <div class="">
				...
				<div v-if="soon" style="width:100%;height: 90%" class="flex1 flex alcenter jscenter ft20">{{$t('trade.soon')}}</div>
				<div id="tv_chart_container" style="width:100%;height: 94.4%" v-show="!soon"></div>
            </div>
        ...
        <div class="flex column overy" style="width:300px;height:100%">
            <div class="flex">
                <div class="bgwhite dark-bg-1C2532 flex alcenter bold  flex1 pt7 pl5">
                    <div class="pb10 mr20 pointer ft14 bd2px" :class="{'active2px':optionIndex==i}" v-for="(item,i) in options" :key="i" @click='changeOption(i)'>{{item}}</div>
                </div>
                <!-- <div class="bgheader bdbheader flex1 plr10 ht40 lh40 baselight2 ft16">{{$t('trade.pankou')}}</div>
                <div class="bgwhite dark-bg-1C2532 flex1 plr10 ht40 lh40 baselight2 ft16 ml5">{{$t('trade.station')}} </div> -->
            </div>
            <div class="flex1 flex">
                <div class="flex column w100 flex1 bgwhite dark-bg-1C2532" v-show="optionIndex==0">
                    <div class="ptb10 flex1 flex column w100">
                        ...
                        <div class="mtb10 flex1 flex column jsend asks">
                            ...
                            <div v-show="outlistSocket.length>0"  class="ptb5 flex alcenter pointer h100 plr10 " :style="'background-size:'+parseInt((item.amount/sellMax)*100)+'% 100%'" style="transition:0.8s"  v-for='(item,i) in outlistSocket' :key="i"  @click="changeSell(item.price)">
                        ...
                        <!-- 买盘 -->
                        <div class="bgwhite dark-bg-1C2532 mt10 flex1 flex column bids">
                            ...
                            <div class="ptb5 flex alcenter pointer h100 plr10" v-show="inlistSocket.length>0" :style="'background-size:'+parseInt((item.amount/buyMax)*100)+'% 100%'" style="transition:0.8s" v-for='(item,i) in inlistSocket' :key="i" @click="changeBuy(item.price)">
                                <span class="flex1 green">{{$t('trade.buy1')}}{{i+1}}</span>
                                <span class="flex1 tc baselight2">{{item.price}}</span>
                                <span class="flex1 tr baselight2">{{item.amount}}</span>
                            </div>   
                ...
                <!-- 全站 -->
                <div class="flex1  bgpart overy flex" v-show="optionIndex==1">
                    <div class=" ptb10 flex1 flex column w100  " >
                        <div class="flex alcenter plr10">
			...
			<!-- 下单 -->
			<div class="bgwhite dark-bg-1C2532" style="width:290px;height:100%">
                <div class=" bdbheader flex alcenter plr10  baselight2 mb10" >
                    <div class='ptb5 mr10 bold ft14 pointer' style="margin-bottom:7px" :class="{'active2px':i==index}" v-for="(item,i) in list" :key="i" @click='changetype(i)'>{{item}}</div> 
                </div>
                <div class="flex mlr10 pointer" style="border-radius:2px;overflow: hidden;">
                    <!-- <el-button-group class="flex1">
                        <el-button @click="markType=0">{{$t('trade.buy')}}</el-button>
                        <el-button @click="markType=1">{{$t('trade.sell')}}</el-button>
                    </el-button-group> -->

                        <div class="flex1 posRelt bg-main-liner" @click="markType=0" style="height:34px;text-align: center;line-height: 34px;color:#fff;font-size:16px">{{$t('trade.buy')}}
                            <div class="abstort top0 rt0 btm0" :style="{'border-top': `16px solid `+(theme=='dark'?'#1C2532':'#fff'),'border-left': `16px solid transparent`, 'border-bottom':` 18px solid `+(theme=='dark'?'#1C2532':'#fff')}">

                            </div>
                        </div>
                        <div class="flex1 posRelt bgred" @click="markType=1" style="height:34px;text-align: center;line-height: 34px;color:#fff;font-size:16px">{{$t('trade.sell')}}
                            <div class="abstort top0 lf0 btm0" :style="{'border-top': `16px solid `+(theme=='dark'?'#1C2532':'#fff'),'border-right': `16px solid transparent`, 'border-bottom':` 18px solid `+(theme=='dark'?'#1C2532':'#fff')}">
                ...
                <div class=" flex  ft12">
                    <!-- ?? -->
                    <div class="flex1 plr10 mt10 bdr38" v-show="markType==0">
                        ...
                        <div class="flex alcenter mb10 posRelt">
                            ...
                            <input type="number" v-model="buyprice" class="ht30 bdinput lh30 plr5 radius2 ml10 flex1 nwhite" v-show="index==0" @input='buypriceInput' @keyup='buypriceInputKeyDown($event)'>
                            <input type="text" :placeholder="$t('trade.goodbuy')" class="trade ht30 bginput lh30 plr5 radius2 ml10 flex1 nwhite" v-show="index==1" disabled>
                            <span class="abstort ht30 lh30 plr5 rt0 btm0">{{legalName}}</span>
                        </div>
                        <div class="flex alcenter mb10 posRelt">
                            <span class="w15" >{{$t('trade.buyNumber')}}</span>
                            <input type="number" v-model="buynumber" class="ht30 bdinput lh30 pl5 radius2 ml10 flex1 nwhite"  @input='buynumberInput'>
                            <span class="abstort ht30 lh30 plr5 rt0 btm0">{{legalName}}</span>
                        ...
                        <el-slider v-model="value1" :step="25" show-stops   :class="loctheme=='dark'?' buyslider ml10':'buyslider2 ml10'" @change="sliderBuy"></el-slider>
                        <!-- <div class="flex alcenter mb10">{{$t('trade.amout')}}：{{totalAmountBuy}}{{legalName}}</div> -->
                        <div class="flex alcenter mb10">{{$t('trade.amout')}}：{{totalAmountBuy}}{{currencyName}}</div>
                        <div class="tc ht40 lh40  radius2 blue bg-main-liner ft16" v-if="!token">
                        ...
                        <div class="bg-main-liner tc ht30 lh30 white radius2 pointer" style="height: 34px;line-height: 34px;font-size: 16px;" @click="buyin" v-else>{{$t('trade.buy')}}</div>
                    </div>
                    <!-- 卖盘 -->
                    <div class="flex1 plr10 mt10" v-show="markType==1">
                        <div class="flex alcenter mb10">{{$t('trade.use')}} {{currencyBalance}} {{currencyName}}</div>
                        <div class="flex alcenter mb10 posRelt">
                            <span class="w15">{{$t('trade.sellPrice')}}</span>
                            <input type="number" v-model="sellprice" class="ht30 bdinput lh30 plr5 radius2 ml10 flex1 nwhite" v-show="index==0">
                            <input type="text" :placeholder="$t('trade.goodsell')" class="trade ht30 bginput lh30 plr5 radius2 ml10 flex1 nwhite" v-show="index==1" disabled>
                            <span class="abstort ht30 lh30 plr5 rt0 btm0">{{legalName}}</span>
                        </div>
                        <div class="flex alcenter mb10 posRelt">
                            <span class="w15">{{$t('trade.sellNumber')}}</span>
                            <input type="number" v-model="sellnumber" class="ht30 bdinput lh30 pl5 radius2 ml10 flex1 nwhite " @input='sellnumberInput'>
                        ...
                        <!-- <div class="flex alcenter mb10">
                         ...
                         <el-slider v-model="value2" :step="25 " show-stops  :class="loctheme=='dark'?' sellslider ml10':'sellslider2 ml10'" @change="sliderSell"></el-slider>
                        <div class="flex alcenter mb10">{{$t('trade.amout')}}：{{totalAmountSell}}{{currencyName}}</div>
                        <div class="bg-main-liner tc ht30 lh30  radius2 blue" v-if="!token">
                            <router-link :to="{name:'login'}">{{$t('login.login')}}</router-link>
                            <span class="baselight2">{{$t('trade.or')}}</span>
                            <router-link :to="{name:'register'}">{{$t('register.register')}}</router-link>
                        </div>
                        <div class="bgred tc ht30 lh30 white radius2 pointer" style="height: 34px;line-height: 34px;font-size: 16px;" @click="sellout" v-else>{{$t('trade.sell')}}</div>
                    </div>
                </div>
            </div>
		</template>
...
export default {
    props:['currentIndex','tradeList','symbol','mathid','legalName','currencyName','legalId','currencyId','cny_usd','usd_usd','currency_matchInfo','inlist','outlist','completelist','legalBalance1','currencyBalance1','soon'],
    data() {
        return {
            widget: null,
            symbolInfo: null,
            ...
            value2:0,
            basedigital:1000000,
            options:[this.$t('trade.pankou'),this.$t('trade.station')],
            optionIndex:0,
            markType:0, //记录买卖 0买 1卖
        }
    },
     watch:{
         theme:{
    ...
    computed:{
        ...mapState(['theme']),
         totalAmountBuy(){
            ...
            let total = new Big(this.buynumber);
            //  let total=Math.ceil(this.buyprice*this.buynumber*this.basedigital)/this.basedigital;
            // let total = this.accDiv(+this.buynumber,+this.buyprice)
            ...
            return parseFloat(total.div(+this.buyprice).toFixed(6))
        },
        totalAmountSell(){
            if(!this.sellnumber){
                return 0
            ...
            return parseFloat(new Big(this.sellnumber).div(this.sellprice).toFixed(6))
    ...
    methods: {
        adjustScale() {
      var that = this;
      console.log(that.currency_matchInfo.close)
      var str = that.currency_matchInfo.close;
      ...
      if (str) {
        ...
        buyin(){
            ...
            this.$http.initDataToken({url:'tx_in/add',data:{currency_match_id:this.mathid,number:this.totalAmountBuy,price:this.buyprice},type:'POST'})
            .then(res=>{
                this.initBalance(this.currencyId,0)
                this.initBalance(this.legalId,1)
            })
        ...
        sellout(){
            ...
            this.$http.initDataToken({url:'tx_out/add',data:{currency_match_id:this.mathid,number:this.totalAmountSell,price:this.sellprice},type:'POST'})
            .then(res=>{
                this.initBalance(this.currencyId,0)
                this.initBalance(this.legalId,1)
            })
        ...
        sliderBuy(e){
            if(this.buyprice){
                console.log(this.currency_matchInfo);
                // this.buynumber=this.legalBalance*e/100
                this.buynumber = this.accDiv(this.accMul(+this.legalBalance,parseFloat(e)),100)
        ...
        sliderSell(e){
            if(this.sellprice){
                ...
                this.sellnumber = new Big(this.sellprice).times(this.currencyBalance).times(e/100);
    ...
    //   更新下单
        ...
        connect(real) { //封装推送数据
            var that=this;
            that.$socket.send([
                {type: "login", params: that.token},
                {type: "sub",params: "MARKET_DEPTH."+that.symbol},
            ...
            Datafeed.Container.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
                ...
                Promise.resolve().then(() => {
                    ...
                    onSymbolResolvedCallback({
                        ...
                        "timezone": "Asia/Tokyo",
                         "pricescale": this_vue.adjustScale(),
