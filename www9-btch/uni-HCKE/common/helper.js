...
if (process.env.NODE_ENV === 'development') {
	 ...
	 */
	_SHUNT = 'coinsultra';
	_DOMAIN = `https://api2.${_SHUNT}.com`;
} else { 
	/**
	 * 正式环境
	 */
	// _DOMAIN = `https://api.${_SHUNT}.com`;
	_DOMAIN = window.location.origin.replace(/\/\/[a-zA-Z0-9]+\./g, "//api2.");
}
_API = `${_DOMAIN}/api/`;
...
function checkUser(emial) {
  var reg = /^[0-9_a-zA-Z]{6,16}$/;
  return reg.test(emial);
}
...
function initDataToken(params,callback,bool= false){
