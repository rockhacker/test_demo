				...
				<swiper-item v-for="(item,index) in wallet_data" :key="index">
					<scroll-view scroll-y class="w-full h-full">
						<view class="py-30 px-40 bg-white dark:bg-1C2532 text-xl">
							...
							<view v-if="!isClose" class="mt-20 text-#333333 text-xl dark:text-white">{{item.convert_usd}}   ≈  {{ item.convert_usd ? (item.convert_usd * currencys.rate).toFixed(2) : item.convert_usd}} <text class="bold  pl-6">{{ currencys.currency_unit }}</text> </view>
							<view v-else class="mt-20 text-#333333 text-xl dark:text-white">{{closeAccount}}</view>
						</view>
		...
		methods: {
			getUserInfo() {
				this.$utils.initDataToken({ url: 'user/info', data: {}, type: 'get' }, (res, msg) => {
					this.getRate(res.currency_unit)
					});
			},
