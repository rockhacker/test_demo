<template>
	<view :class="theme">
		<view class="bg-my-grey dark:bg-my-black">
			<uni-nav-bar fixed>
				<view class="w-full flex flex-col p-40">
					<view class="w-full flex justify-center">
						<view class="flex w-full h-60 bg-F4F5F7 dark:bg-333F4A rounded-2xl overflow-hidden">
						    <view class="flex-1 flex justify-center items-center text-center" :class="active==1?'bg-main-linear text-white':'text-333333 dark:text-white'" @click="setActive(1)">{{$t('new.leverAll')}}</view>
							<view class="flex-1 flex justify-center items-center text-center" :class="active==0?'bg-main-linear text-white':'text-333333 dark:text-white'" @tap="setActive(0)">{{$t('new.leverLes')}}</view>
						
							<!-- <view class="flex-1 flex justify-center items-center text-center" :class="active==2?'bg-main-linear text-white':'text-333333 dark:text-white'" @tap="setActive(2)">{{$t('qqjy.qqjy')}}</view> -->
						</view>
					</view>
			...
			<view v-if="active==1 && !soon" class="min-h-screen bg-gray-100 dark:bg-floor">
				<!-- <view class="flex justify-center items-center text-3xl h-600" v-if="soon">{{$t('trade.soon')}}</view> -->
				<view class="flex justify-between p-20 bg-white dark:bg-1C2532">
					<view class="w-3_5">
						<view class="overflow-hidden flex justify-between rounded-10 bg-gray-100 dark:bg-gray-600 dark:bg-opacity-30 dark:text-white">
							<view class="text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'buy' ? 'bg-my-k-green text-white' : ''" @tap="tradeType('buy')">
								{{ $t('trade.buy') }}
							...
							<view class="text-center flex justify-center items-center flex-1 h-66 relative" :class="leverTradeType == 'sell' ? 'bg-my-k-red text-white' : ''" @tap="tradeType('sell')">
								{{ $t('trade.sell') }}
						...
						<view class="flex item-center justify-between mt-20">
							<view class="flex items-center justify-between border-2 border-solid border-B8C1CF dark:border-999FAA flex-2 h-60 pr-10 rounded-8" @tap="tradeTypes.shows = true">
							...
							<view class="flex-1 h-60 ml-10">
								<view class="border-2 border-solid  border-B8C1CF dark:border-999FAA h-60 flex items-center justify-between pr-10 rounded-8" @tap="mults.shows = true">
						...
						<view class="border-2 border-solid  border-B8C1CF dark:border-999FAA h-70 flex items-center mt-20 px-20 rounded-8">
						...
						<view class="mt-20 text-lg">
							...
							<view class="flex justify-between items-center my-20 border-2 border-solid  border-B8C1CF dark:border-999FAA px-20 rounded-8">
								<view class="flex-1 h-60 flex between items-center">
									<image src="../../static/image/jian.png" @click="setShare(-1)" mode="aspectFit" class="w-30 h-30 ml-20"></image>
									<input class="text-black dark:text-white text-sm tc" placeholder-class="text-gray-600 flex-1" type="number" v-model="leverOrder.share" :placeholder="$t('trade.p_handnum')" />
									<image src="../../static/image/jia.png" @click="setShare(1)" mode="aspectFit" class="w-30 h-30 ml-20"></image>
								</view>
						...
						<view class="flex items-center justify-between">
							<view>{{ $t('new.kkzs') }}</view>
							<view>{{ maxValue }}</view>
						</view>
						<view class="px-20 py-40">
							<vue-slider @change="changeValue" :tooltip-style="tooltipStyle"  :tooltip-formatter="'{value}%'" v-model="value1" :marks="marks1"></vue-slider>
						</view>
						<view class="mt-20 text-sm">{{ $t('trade.balance') }}：{{ leverBalance | toFixed4 }} {{ quoteCurrencyCode }}</view>
						<view class="h-70 flex justify-center items-center mt-30 rounded-10" :class="leverTradeType == 'buy' ? 'bg-my-k-green' : 'bg-my-k-red'" @tap="submits">
							<view v-if="token" class="flex-1 text-center text-white text-xl">{{ leverTradeType == 'buy' ? $t('trade.buy_duo') : $t('trade.sell_kong') }}</view>
							<view v-if="!token" class="flex-1 text-center text-white text-xl">{{ $t('home.p_login') }}</view>
						</view>
					</view>
					<view class="w-2_5 flex flex-col ml-20">
						<view class="flex justify-between">
							<!-- <view class="">{{$t('trade.pankou')}}</view> -->
							<view class="text-838B99 dark:text-999FAA">{{ $t('trade.price') }}</view>
							<view class="text-838B99 dark:text-999FAA">{{ $t('trade.num') }}</view>
						</view>
				...
				<!-- 当前委托列表 -->
				<view class="mt-20 p-20 bg-white dark:bg-1C2532 pb-120 min-h-half">
					<view class="flex justify-between">
						<view class="text-333333 dark:text-white text-xl">{{ $t('trade.delegate') }}</view>
					...
					<view>
			            <order-list ref="order" :symbol="baseCurrencyCode + '/' +quoteCurrencyCode " :msg="msg" :currencyMatchId="currencyMatchId"></order-list>
						<!-- <view class="flex justify-between">
							<view class="flex-1 text-left text-91A3B0 dark:text-999FAA text-sm">{{ $t('trade.types') }}</view>
							<view class="flex-2 text-center text-91A3B0 dark:text-999FAA text-sm">{{ $t('trade.time') }}</view>
							<view class="flex-2 text-center text-91A3B0 dark:text-999FAA text-sm">{{ $t('trade.price') }}</view>
							<view class="flex-2 text-center text-91A3B0 dark:text-999FAA text-sm">{{ $t('trade.amout') }}</view>
							<view class="flex-1 text-center text-91A3B0 dark:text-999FAA text-sm">{{ $t('trade.operate') }}</view>
						</view>
						<view class="" v-if="orderList.length > 0">
							<view v-for="(item, index) in orderList" :key="index" class="flex justify-between items-center mt-20">
								<view class="flex-1 text-left text-333333 dark:text-999FAA">{{ item.type_name }}</view>
								<view class="flex-2 text-center text-333333 dark:text-999FAA">{{ item.time }}</view>
								<view class="flex-2 text-center text-333333 dark:text-999FAA">{{ item.price || '0.0000' | toFixed4 }}</view>
								<view class="flex-2 text-center text-333333 dark:text-999FAA">{{ item.number || '0.00' | toFixed2 }}</view>
								<view class="flex-1 text-center bg-main-linear py-10 rounded-8 ml-20 text-white" @tap="closeTrade(item.id)">{{ $t('trade.pingcang') }}</view>
							</view>
						</view>
						<view class="text-center p-200" v-if="orderList.length == 0">
							...
							<view class="text-center text-91A3B0 dark:text-999FAA">{{ $t('home.norecord') }}</view>
						</view> -->
				...
				<view class="pt-bar">
					<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
						<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
							...
							:class="{'text-clip-green':quoteCurrencyIds == item.id}" v-for="(item,i) in quotationList" :key="i" 
							@tap="selectedLegal(item.id)">
							{{item.code}}
							<view class="w-60 h-6 mt-10" :class="{'bg-main-linear':quoteCurrencyIds == item.id}"></view>
						</view>
					</scroll-view>
				</view>
					...
					<view class="pb-60">
						<view class="border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 text-black dark:text-white" v-for="(item, index) in quotationList[quoteIndex].matches" :key="index" v-if="item.open_lever == 1">
							...
							>
								...
								<view v-if="item.base_currency_code=='TEST' && !isOnline(item.market_time)">{{$t('trade.soon')}}</view>
				...
				<view class="w-full p-40 ">
					<view class="text-lg">
						...
						<view class="flex justify-between mt-20">
							...
							<view class="text-lg">{{ leverOrder.share * leverOrder.fee }}</view>
						</view>
						<view class="flex justify-between mt-20">
							<view class="text-lg">{{ $t('trade.money') }}</view>
							<view class="text-lg">{{ leverOrder.share * lever_share_amount }}</view>
						</view>
...
import orderList from '../lever/orderLists.vue';
export default {
	components: {
		uniDrawer,
		uniIcon,
	...
	onShow() {
		...
		if(this.priceStatus==1){
			this.priceValue = this.$t('trade.best');
		}
	...
	computed: {
		...
		maxValue(){
			if(!this.leverBalance || !this.lever_share_amount) return 0 
			return  Math.floor(this.leverBalance / this.lever_share_amount)
		}
	},
	methods: {
		changeValue(val){
           this.leverOrder.share = Math.floor((this.leverBalance / this.lever_share_amount)* (val/100))
		},
		setShare(val){

			 this.leverOrder.share = this.leverOrder.share*1 + val*1
			 if(this.leverOrder.share < 1){
				this.leverOrder.share = 1
			 }
		},
		setActive(val){
			let that = this
			...
			that.$utils.initDataToken({ url: 'market/currency_matches' }, res => {
				...
				if (res.length > 0) {
					...
					if (that.quoteCurrencyId) {
								...
								if (lists.length > 0) {
									lists.forEach(function(itm, inx) {
										if (that.baseCurrencyId == itm.base_currency_id) {
											...
											that.lever_share_amount = itm.lever_share_amount;
											that.soon = itm.base_currency_code=='TEST' && !that.isOnline(itm.market_time)
						...
						that.lever_share_amount = list[0].lever_share_amount;
						that.baseCurrencyCode = list[0].base_currency_code;
						that.soon = list[0].base_currency_code=='TEST' && !this.isOnline(list[0].market_time)
		...
		deals() {
			...
			that.$utils.initDataToken({ url: 'lever/deal', type: 'POST', data: { currency_match_id: that.currencyMatchId } }, res => {
				...
				if (res.multiple.muit.length > 0 && !that.loaded) {
		...
		selectCurrency(options) {
			...
			that.lever_share_amount = options.lever_share_amount;
		...
		handicap() {
			...
			var symbols = that.baseCurrencyCode + '/' + that.quoteCurrencyCode;
			var datas = {};
			that.$socket.listenFun([{ type: 'login', params: tokens }, { type: 'sub', params: 'MARKET_DEPTH.' + symbols }, { type: 'sub', params: 'DAY_MARKET' },{type: "sub",params: "LEVER_TRADE"},{type: "sub",params: "LEVER_CLOSED"}], res => {
				...
				if (msg.type == "LEVER_TRADE" || msg.type == "LEVER_CLOSED") {
					 that.msg = msg
				}
			});
		}
