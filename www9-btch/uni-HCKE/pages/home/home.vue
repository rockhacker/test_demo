<template>
  <view class="overflow-x-hidden" :class="theme">
	  <view class="bg-my-grey dark:bg-my-black">
			...
			<view class="absolute left-0 z-10 h-100">
				<view class="flex flex-1 relative h-full flex items-center justify-center px-20">
					<view class="flex items-center">
						<text v-if="info.uid" class="text-clip-yellow text-sm">{{info.email}}</text>
					</view>
				</view>
			</view>
			<view class="flex-1 flex justify-center items-center text-2xl h-100 bg-my-green"></view>
			<view class="absolute right-0 top-0 z-10 h-100">
				<view class="flex flex-1 relative h-full flex items-center justify-center px-20">
					<view class="flex items-center" @tap="showRight = true">
						<text class="text-clip-yellow">{{ languages[lang].name }}</text>
		...
		<view class="flex py-14 box-border items-center justify-center bg-white dark:bg-1C2532 w-full">
		  <view class="flex items-center flex-1 justify-center flex-col text-center" @click="toPath(item.path)" v-for="item,index in functionModule" :key="index">
			<image class="w-84 h-84" :src="`/static/image/${theme==='dark'?'dark-':''}${item.img}`"></image>
			<view class="text-sm mt-8 text-838B99 dark:999FAA">{{ $t(item.name) }}</view>
		  </view>
		  <view class="flex items-center flex-1 justify-center flex-col text-center" @click="topo">
			<view class="w-84 h-84 bg-main-linear rounded-base flex flex-col justify-center items-center">
				<image class="w-50 h-50" src="/static/image/bps.png"></image>
			</view>
			<view class="text-sm mt-8 text-838B99 dark:999FAA">{{ $t('new.baip') }}</view>
		  </view>
		...
		<!--涨幅榜-->
		<swiper v-if="quoList.length" :display-multiple-items="quoList.length>2?3:quoList.length" class="w-full h-170 bg-white dark:bg-1C2532 pl-20 pr-20 py-18">
		  <swiper-item class="flex items-center justify-around border-solid border-0 border-r-20 border-white dark:border-my-black" v-for="(item, i) in quoList" :key="i">
			<view @click="toUrl(item)" class="flex flex-col items-center">
				<text class="text-sm text-1A222B dark:text-999FAA font-bold">{{ item.base_currency_code }}/{{ item.quote_currency_code }}</text>
				<text class="text-3xl mt-10" v-if="item.currency_quotation" 
				:class="{'text-my-k-red': parseFloat(item.currency_quotation.change) < 0,'text-my-k-green':parseFloat(item.currency_quotation.change) >= 0 ||item.currency_quotation.change == '',}">
					{{ parseFloat(item.currency_quotation.close) }}
				</text>
				<text class="text-sm mt-10" v-if="item.currency_quotation" 
				:class="{'text-my-k-red': parseFloat(item.currency_quotation.change) < 0,'text-my-k-green':parseFloat(item.currency_quotation.change) >= 0 ||item.currency_quotation.change == '',}">
		...
		<view class="w-full my-20 h-100 flex justify-between px-20">
			<view class="flex-1 flex justify-between items-center bg-main-linear w-335 rounded-sm px-20 mr-20" @click="toPath('/pages/mine/withdraw')">
		...
		<!--行情-->
		<view class="px-20">
			<view class="pb-100 w-full bg-white dark:bg-1C2532 rounded-xl box-border">
		  <!-- <view class="ptb10 bdb_blue3 ft14 plr10 bold zhang">主流区</view> -->
		  <view class="flex px-40 pt-30">
			<view class="trading-item text-center mr-40 " v-for="(item, index) in allList" :key="index" @click="setItem(item)">
			  <view class="pb-10 text-lg" :class="itemCode == item.code ? 'text-clip-green' : 'text-838B99'">{{ item.code }}</view>
			  <view class="w-80 h-6" :class="itemCode == item.code ? 'bg-main-linear' : ''"></view>
			</view>
		  </view>
		  ...
		  <view @click="toUrl(item)" class="flex items-center text-sm text-gray-400 tableitem px-40 py-30" v-for="(item, i) in tabList" :key="i">
			...
			<template v-else>
				<view class="flex-1 text-1A222B text-lg dark:text-9BA4AD" v-if="item.currency_quotation">{{ parseFloat(item.currency_quotation.close) }}</view>
				<view class="flex-1 text-right flex justify-end" v-if="item.currency_quotation">
				  <view class="w-140 h-60 flex justify-center items-center rounded-6 text-center text-white" :class="{ 'bg-my-k-red': parseFloat(item.currency_quotation.change) < 0, 'bg-my-k-green':parseFloat(item.currency_quotation.change) >= 0 ||item.currency_quotation.change == ''}">
					{{ item.currency_quotation.change.substr(0, 1) == "-" ? "" : "+"}}{{ (item.currency_quotation.change - 0) | toFixed2 }}%
				  </view>
				</view>
			</template>
...
export default {
	...
	methods: {
		...
		topo(){
				...
				//  console.log()
				const lang = 'en'
				window.open(`${this.$utils._DOMAIN}/wp/TEST-en.pdf`);
			},
		toOnline(){
