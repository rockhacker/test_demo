<template>
	<view :class="theme">
		<view class="bg-white dark:bg-1C2532 min-h-screen text-gray-700 dark:text-gray-100">
			...
			<view class="flex items-center justify-between text-sm border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700">
				<view class="px-40 py-20" :class="status == item.id ? 'text-clip-green border-0 border-b-2 border-solid border-my-green' : ''" v-for="item in tabs" :key="item.id" @tap="selectType(item.id)">{{ item.texts }}</view>
			</view>
				...
				<view v-for="(item, index) in orderList" :key="index" class=" border-0 border-b-2 border-solid border-gray-100 dark:border-gray-700 py-20">
					<view class="flex justify-between mb-30">
						<view class="flex items-center">
							...
							<text class="ml-10 bold ft14">{{ item.symbol }}</text>
					...
					<view class="flex flex-col">
						...
						<view class="flex mb-20">
							<view class="flex-1">
								<view class="text-gray-500" v-if="status == 3">{{ $t('add.pcj') }}</view>
								<view class="text-gray-500" v-else>{{ $t('trade.price_cur') }}</view>
						...
						<view class="flex mb-20 justify-between">
							...
							<view class="items-center flex justify-center text-center" v-if="item.status == 1 || item.status == 3">
