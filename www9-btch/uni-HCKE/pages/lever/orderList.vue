			...
			<template v-else>
				<view v-for="(item, index) in orderList" :key="index" class="py-30 border-0 border-solid border-b-2 border-gray-100 dark:border-gray-900">
					...
					<view class="flex mb-20">
						...
						<view class="flex-1 items-center flex justify-end text-right">
					...
					<view class="flex text-right mt-30 justify-end">
						...
						<view class="bg-main-linear px-40 py-20 rounded-8 text-white ml-20" @tap="closes(item.id,index)">{{$t('trade.pingcang')}}</view>
					</view>
				</view>
			</template>
		</view>
