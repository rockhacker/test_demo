	...
	<view>
		<!-- <uni-nav-bar backIcon fixed>
			<view class="flex justify-between items-center py-20 pr-20 w-full bg-white dark:bg-floor">
				...
				<view class="bg-main-linear text-white py-20 text-center rounded-10 w-200" @tap="allClose.shows=true">{{$t('lever.yijian')}}</view>
			</view>
		</uni-nav-bar> -->
		
		<view class="list text-xl  bg-white dark:bg-floor mt-20">
			...
			<template v-else>
				<view v-for="(item, index) in orderList" :key="index" class="py-30 border-0 border-solid border-t-2 border-gray-100 dark:border-gray-900">
					<view class="flex justify-between mb-30">
						<view class="flex items-center">
							<view class="px-30 py-10 rounded-8 text-white" :class="item.type == 1 ? 'bg-my-k-green' : 'bg-my-k-red'">{{ item.type == 1 ? $t('trade.buy') : $t('trade.sell') }}</view>
							<text class="ml-10 bold ft14">{{ symbol }}</text>
					...
					<view class="flex mb-30">
						...
						<view class="flex-1 text-right">
							...
							<view class="text-black dark:text-white text-lg" :class="item.profits < 0 ? 'text-danger' : 'text-success'">
								{{ (item.profits / item.caution_money *100) | toFixed2 }}%
							</view>
						</view>
					</view>
					<view class="flex mb-30">	
					...
					<view class="flex">
						...
						<view class="flex-1 items-center flex justify-center text-center">
							<view  @tap="detalis(item)"  class="btn bg-F4F5F7 px-30 py-10 rounded-8 ml-20 blue2">{{$t('add.xq')}}</view>
						</view>
						<view class="flex-1 items-center text-right flex justify-end">
							<view :class="item.profits < 0 ? 'text-danger' : 'text-success'" class="btn bg-F4F5F7 px-30 py-10 rounded-8 ml-20" @tap="closes(item.id,index)">{{$t('trade.pingcang')}}</view>
						</view>
					</view>
					
					<!-- 按钮 -->
					<!-- <view class="flex text-right mt-30 justify-end">
						<view class="bg-main-linear px-40 py-20 rounded-8 text-white" @tap="loss(item,index)">{{$t('lever.setys')}}</view>
					</view> -->
				</view>
			</template>
		</view>
...
export default {
    ...
    props:['currencyMatchId','msg','symbol'],
	watch:{
		currencyMatchId(val){
			console.log(val)
				this.init();
	...
	methods: {
		detalis(item){
			uni.navigateTo({
					url:'/pages/lever/orderDetalis?item='+ JSON.stringify(item)
				})
		},
		init() {
			var that = this;
			that.$utils.initDataToken(
				...
				res => {
					console.log(res);
					if (res.order.data.length > 0 && that.page >1) {
						that.orderList = that.orderList.concat(res.order.data);
						// if(that.page == 1){
						// 	//that.orderTrade();
						// }
					}else{
						that.orderList = res.order.data
					}
				}
			);
		},
