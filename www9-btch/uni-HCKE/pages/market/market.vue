		...
		<view class="w-full flex flex-col">
			<scroll-view scroll-x class="w-full h-100 whitespace-nowrap" :show-scrollbar="false">
				<view class="w-180 h-100 flex-col justify-center items-center text-xl font-bold inline-flex rounded-none" 
					...
					:class="isActive==i?'text-clip-green':'text-838B99'" v-for="(item,i) in marketLists" :key="i" 
					@click="changeTab(i,item.code,item)">
					{{item.code}}
					<view class="w-70 h-6 mt-10" :class="{'bg-main-linear':isActive==i}"></view>
				</view>
			</scroll-view>
			...
			<view @click="toPath(item)" class="flex items-center px-40 h-120" v-for="(item,i) in matches" v-if="marketLists[isActive]" :key="i" v-show="marketLists&&marketLists[isActive]&&item.open_change==1">
				...
				<template v-else>
					...
					<view class="flex flex-1 justify-end text-white">
						<view class="flex justify-center items-center w-140 h-60 rounded-8"  v-if="item.currency_quotation" :class="[(item.currency_quotation.change.substr(0,1))=='-'?'bg-my-k-red':'bg-my-k-green']">{{(item.currency_quotation.change.substr(0,1))=='-' ? '' : '+'}}{{(item.currency_quotation.change-0) | toFixed2}}%</view>
					</view>	
				</template>
			</view>
		</view>
