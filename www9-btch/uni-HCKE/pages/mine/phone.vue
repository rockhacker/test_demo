<template>
	<view :class="theme">
		 <view class="px-40 bg-white flex between wraps mt30" v-if="!verified">
			...
			<view class="w-full flex items-center border-0 border-b-2 border-solid border-gray-200 dark:border-gray-700">
				  <view class="flex items-center flex1">
					<view class="flex  alcenter between mr10" @click="showpopup">
								<view>+{{item.global_code}}</view>
								<image src="/static/image/trade_arrow_down.png" class="wt10 h8 ml5"></image>
					</view>
					<input type="text" v-model="user_string" class="h-100 flex-1" :placeholder="$t('login').p_mobile" />
				  </view>
				</view>
        ...
        <view class="flex items-center jscenter">
           <image src="../../static/image/sj2.png" style="width: 200rpx;height: 200rpx;"  mode="widthFix"></image>
        </view>
        <view class="tc mt20 ft14 color1">{{$t('add.sjtt')}}</view>
      </view>
...
<script>
	...
	export default{
		...
		methods:{
			submit(){
				if(!this.user_string){
					this.$utils.showLayer(this.$t('login.p_mobile'))
                    return;
				}
				if(!this.code){
				...
				this.$utils.initDataToken({url:'user/bind_verify_method',type:'POST',data:{
					type:'mobile',
					code:this.code,
					account:this.item.global_code + this.user_string
				}},(res,msg)=>{
